...
if grep -nE '(^|[^[:alnum:]_])SET_[A-Z0-9_]+([^[:alnum:]_]|$)|BLOCKED_UNTIL_|\$\{[^}]+\}|pilot-local-token|localhost|127\.0\.0\.1|model-relay\.example\.com|CONNECTOR_FIXTURE_MODE[=:][[:space:]]*true|ALLOW_WILDCARD_WORKSPACE_GRANT[=:][[:space:]]*true' "$config_path" >/dev/null; then
...
if ! grep -Eq '^[[:space:]]*OPS_AUTH_MODE:[[:space:]]*"?password"?([[:space:]]*)$' "$config_path"; then
  echo 'production ops console must use Store Nova account/password authentication' >&2
  exit 1
fi
if ! grep -Eq '^[[:space:]]*ops_base_url:[[:space:]]*"?https://ops\.yxsona\.com"?[[:space:]]*$' "$config_path"; then
  echo 'production ops_base_url must equal https://ops.yxsona.com' >&2
  exit 1
fi
...
grep -Eq 'merchant_ui_workspace_id_ref:[[:space:]]*[^"'"'"' ]+' "$config_path" || { echo 'merchant_ui_workspace_id_ref must be configured' >&2; exit 1; }
...
vector_index_enabled=$(ruby -ryaml -e 'doc=YAML.safe_load(File.read(ARGV.fetch(0)), aliases: false); value=doc.is_a?(Hash) ? doc["knowledge_vector_index_enabled"] : nil; puts(value == true ? "true" : "false")' "$config_path")
if [ "$vector_index_enabled" = true ]; then
  ruby -ryaml -e 'c=YAML.safe_load(File.read(ARGV.fetch(0)), aliases: false); model=c["embedding_model"]; dimensions=c["embedding_dimensions"]; cost=c["embedding_max_request_cny"]; abort unless model.is_a?(String) && !model.strip.empty? && !%w[false null ~].include?(model.strip.downcase); abort unless dimensions.to_s.match?(/\A[1-9][0-9]*\z/); amount=Float(cost) rescue nil; abort unless amount && amount > 0' "$config_path" || { echo 'embedding_model, embedding_dimensions and embedding_max_request_cny must be valid when vector indexing is enabled' >&2; exit 1; }
fi
