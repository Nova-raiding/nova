...
for required_key in \
  plugin_enabled merchant_bearer_hostname auth_enforcement session_id_hash_secret_ref \
  ...
  object_storage_versioning lifecycle_policy_ref \
  asset_quarantine_retention_days asset_clean_retention_days deletion_request_grace_days backup_retention_days \
  alert_channel_secret_ref point_in_time_recovery_enabled database_pooler_enabled \
  database_max_backend_connections database_connection_utilization_alert_percent secret_provider worker_api_signing_secret \
  merchant_ui_api_token_ref payment_mode payment_provider_adapters payment_checkout_base_url \
...
grep -Eq 'asset_quarantine_retention_days:[[:space:]]*(7|[1-9][0-9]|[12][0-9][0-9])$' "$config_path" || { echo 'asset_quarantine_retention_days must be at least 7 days' >&2; exit 1; }
grep -Eq 'asset_clean_retention_days:[[:space:]]*(3[0-9]|[4-9][0-9]|[12][0-9][0-9])$' "$config_path" || { echo 'asset_clean_retention_days must be at least 30 days' >&2; exit 1; }
...
grep -q 'worker_api_signing_secret:' "$config_path" || { echo 'worker workspace signing secret must be configured' >&2; exit 1; }
