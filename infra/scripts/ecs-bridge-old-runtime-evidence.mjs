...
async function main(args) {
  ...
  if (mode === 'freeze') {
    ...
    const input = options(args.slice(1), ['--evidence', '--archive'])
    assert(input['--evidence'] && input['--archive'], 'verify requires protected evidence and Docker save archive')
    ...
    const runtime = collectRuntime(evidence.runtime?.gateway?.id, evidence.runtime?.source_git_sha)
