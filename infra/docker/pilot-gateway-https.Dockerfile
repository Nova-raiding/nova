...
EXPOSE 8080 8443
HEALTHCHECK --interval=10s --timeout=3s --retries=5 CMD wget -qO- http://127.0.0.1:8080/healthz >/dev/null || wget --no-check-certificate -qO- https://127.0.0.1:8443/healthz >/dev/null
