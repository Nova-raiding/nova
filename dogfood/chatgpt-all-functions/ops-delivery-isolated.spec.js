...
async function rpcAfter(page, method, action, evidence, expectedStatus = 200) {
  const captureIndex = await page.evaluate(() => {
    if (!window.__deliveryRpcCaptures) {
      window.__deliveryRpcCaptures = []
      ...
      window.fetch = async (...args) => {
        ...
        try {
          ...
          const requestText = typeof init?.body === 'string' ? init.body : input instanceof Request ? await input.clone().text() : ''
          const request = JSON.parse(requestText)
          if (new URL(url, location.href).pathname === '/api/mcp' && typeof request?.method === 'string') {
            window.__deliveryRpcCaptures.push({ method: request.method, status: response.status, body: await response.clone().text() })
          }
        } catch { /* only the browser/CDP evidence layer; application fetch is unchanged */ }
        return response
      }
    }
    return window.__deliveryRpcCaptures.length
  })
  let started = false
  const requestPromise = page.waitForRequest(request => {
    const payload = rpcPayload(request)
    return started && payload?.method === method
  })
  const responsePromise = page.waitForResponse(response => started && rpcPayload(response.request())?.method === method, { timeout: 45_000 })
  const [request, response] = await Promise.all([requestPromise, responsePromise, (async () => { started = true; await action() })()])
  const captured = await page.waitForFunction(({ start, rpcMethod }) => window.__deliveryRpcCaptures?.slice(start).find(item => item.method === rpcMethod), { start: captureIndex, rpcMethod: method }, { timeout: 45_000 }).then(handle => handle.jsonValue())
  const body = parse(captured.body)
  const payload = rpcPayload(request)
  const error = body?.error ?? body?.data?.error
  const result = body?.result ?? body?.data?.result
  evidence.events.push({ event: 'rpc', method, status: response.status(), request: safeParams(payload?.params), error_code: error?.code ?? null, response_body_unavailable: false })
  expect(response.status(), `${method} status`).toBe(expectedStatus)
  expect(captured.status, `${method} captured response status`).toBe(expectedStatus)
...
test('isolated customer delivery end-to-end fields, gates, checklists and scanned assets', async ({ page, browser }, testInfo) => {
  ...
  try {
    ...
    const fillChecklist = async (items, checklistKey) => {
      ...
      let loaded = await rpcAfter(page, 'ops.customer-delivery.checklist-items.list', () => cell.getByRole('button').click(), evidence)
      for (let attempt = 1; !loaded.result?.items && attempt < 3; attempt++) {
        await closeDrawer(page)
        loaded = await rpcAfter(page, 'ops.customer-delivery.checklist-items.list', () => cell.getByRole('button').click(), evidence)
      }
    ...
    await expect(evidenceTags(trainingEvidence, '已上传培训凭证')).toHaveText([training.assetRef])
    await trainingEvidence.getByRole('button', { name: '收起', exact: true }).click()
