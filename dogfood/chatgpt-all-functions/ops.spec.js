...
test('renders the real workspace brand tree with revision and store navigation', async () => {
  ...
  await expect(page.getByRole('heading', { name: 'Release QA Brand', exact: true })).toBeVisible({ timeout: 20_000 })
  await expect(page.getByText('brand_release_qa', { exact: true })).toBeVisible({ timeout: 20_000 })
