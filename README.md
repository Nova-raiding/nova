...
发布元数据同步基线以 `release-metadata.json`、共享注册表和运行态校验为准；固定工具数量和迁移版本不在说明文档中重复维护。
...
当前发布验收基线以 `release-metadata.json` 和运行时门禁为唯一权威；版本、工具表面、运营域和迁移尾部均由门禁从仓库实时核对。正式发布仍须由 metadata、真实宿主证据和生产发布门禁共同通过。
