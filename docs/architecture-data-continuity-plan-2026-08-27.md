...
| 断链 | 当前证据 | 用户影响 |
|---|---|---|
| 品权限未执行 | `brand_access_grants` 只在迁移中出现，API/仓储没有授权查询 | 工作区成员可能读取未授权品 |
| 成员校验默认放行 | 生产 `enforceActiveWorkspaceMember` 只在找到成员记录时校验状态；认证主体在当前工作区没有成员记录时仍继续执行 | SSO grant 与应用成员表不一致时，未建档成员可能访问工作区 |
| 上下文快照未落地 | `context_snapshots` 只有迁移和迁移测试，没有 repository/assembler/调用方 | 每次重复拼接上下文，Token 浪费，无法还原生成依据 |
| 上下文无预算与检索 | `knowledgeContextProvider` 读取工作区全部知识资产并携带完整 `content`；生成器直接序列化整个 input，结构修复重试还会附带上一份完整输出 | 品和素材增长后请求变慢、Token 成本非线性增加，甚至超过模型窗口 |
| 批次项未持久化 | `PostgresBrandUnitRepository.createCampaign` 只写 `batch_campaigns.data` | 单项状态、重试、费用和审计无法成为数据库事实 |
| manifest hash 无效 | 创建批次固定写入 `"0".repeat(64)` | 幂等不能证明同一意图，确认快照不可信 |
| 批次生成残留本地状态 | `campaignGeneratedTasks` 仍为 API 进程 Map fallback | 进程重启/多副本可能产生不同判断 |
...
- 用迁移 043+（当前最新为 042）完成默认品回填、旧 Product→CanonicalProduct/Listing 映射、品级资产/规则绑定和 model usage 关联。
