...
- 浏览器：43/43 项综合回归在单 worker 正式端口上一次性通过。商家端覆盖发布成功、500、超时、双击、幂等重放、后台轮询与任务辅助 API 失败恢复、审核理由失败重试及弹窗键盘可访问性；运营台覆盖 6 页、三种移动视口、8 项深链/浏览器历史、数据删除审批/取消和批量发布暂停/恢复/失败项新确认重试，正确同源代理配置下无非预期 console、HTTP 或 RPC 错误。
