...
1. 正式身份源采用外部 OIDC/OAuth 2.1 服务，不在业务 API 内自建密码库。
2. 商家账号采用邀请制；平台管理员只允许后台预配，不开放公共注册。
...
| 能力 | 当前状态 | 证据与缺口 |
|---|---|---|
| 任意金额钱包充值 | 已实现但属于历史兼容链路 | `billing.recharge.create`、支付 provider 回调和钱包流水存在；共享商业注册表明确不把它作为 ChatGPT 原生商业准入工具。 |
| 服务端 SKU 下单 | 已实现 | `commercial.order.create` 只接受批准、可执行、服务端定价的 SKU，不允许客户端提交金额、点数或权益。 |
| 支付到账后发放创意点 | 底层已实现 | `recordVerifiedPaymentAndGrant` 校验 provider、金额、币种和不可变订单快照后，原子写入支付事件、权益快照、创意点 grant、账本与 access revision。 |
...
1. 用户在 ChatGPT 或 `ops.yxsona.com` 点击“连接大麦”，完成一次 OIDC 登录。
...
6. 后续充值只购买批准的创意点包；使用前展示本次预计扣点，执行时预占，成功后按批准费率结算，失败释放，余额不足时只给充值恢复动作。
...
- 无静态测试 token 时，真实 OIDC 用户可完成登录、刷新、退出和重新登录。
...
- 每次模型执行都有预估点数、预占、provider usage/cost、最终扣点或释放证据；余额不可为负，未知 provider 结果不可重复执行。
