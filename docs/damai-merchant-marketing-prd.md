...
3. 插件跳转大麦 OAuth/OIDC，不收集或保存商家密码。
...
- 生产商家后台：企业 OAuth/OIDC。
- 生产 ChatGPT App：同一身份提供商的 OAuth/OIDC，并以 subject 映射 Workspace member。
- 生产平台运营后台：企业 SSO/OIDC，独立 platform scope。
- 本机 API 自动连接、Bearer Token 表单和 fixture 登录只允许 development/test，生产 UI 不得展示。
...
- 首次打开进入商家 OAuth 注册/登录，不出现本机 API 配置。
...
仓库已经具备较多基础：MCP bridge、Workspace scope、知识检索、素材扫描、创意点预留/结算、图片/视频生成、发布预览和运营账务读模型。现有模型适配器也已经以 `MODEL_RELAY_BASE_URL`、allowlist、provider request id、usage/cost evidence 和 fail-closed 门禁为主线。
