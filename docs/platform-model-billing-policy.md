...
- `MODEL_RELAY_BASE_URL`、`MODEL_RELAY_API_KEY`、`AI_MODEL`、`IMAGE_MODEL`、`IMAGE_EDIT_MODEL`、`OCR_MODEL` 和 `VIDEO_MODEL` 只允许由部署密钥管理系统注入；视频如需独立计费分组，可额外注入 `VIDEO_MODEL_RELAY_API_KEY`，未配置时回退到共享 Key。所有环境都只允许经 HTTPS 自有中转站，未配置时必须 fail-closed。`AI_BASE_URL`、`IMAGE_BASE_URL`、`VIDEO_BASE_URL` 及各类直连 Key 不再作为有效出口。
