...
`fixture`、`simulated`、`fixture_verified` 不得升级为 `production_canary`。真实账号/沙箱准备仍是外部门禁，需由平台开发者账号、测试店铺、回调域名、最小 scope 和脱敏回执补齐后，才能按 `docs/platform-capability-preflight.md` 验收九项能力（含媒体上传）。
