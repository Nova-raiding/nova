...
审计范围：`docs/PRD-merchant-marketing-codex-final.md` 中 FR-06～FR-10。
...
| 证据 | 位置 |
|---|---|
| PRD 原文 | `docs/PRD-merchant-marketing-codex-final.md:690-794` |
...
2. **无数据处理不统一。** 部分字段会显示“暂无已确认的结构化卖点，需由商家补充”，但没有统一的 `pending`/`omitted` 结构和验收；不能证明所有无数据模块都不会生成空泛或推断性内容。
