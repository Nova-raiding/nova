...
- 首批代码已作为 `91d4b461` 提交并推送到 `main`。上文提及的 `85dd26aa…` 与未提交状态是本次最初核对时的快照，不是当前候选身份。后续修复仍在工作目录中，正式 release ID、镜像与证据必须在最终提交后重新绑定。
- 发现正式升级的桥接缺口：当前迁移尾号为 254，243–254 的 SQL 和隔离恢复工具已存在，但现有 Bridge B 的 API、worker 和恢复校验只接受 242/244。正式部署器要求与 254 兼容的回滚目标及从现场版本到 254 每个迁移前缀的批准摘要。缺少经真实 PG17 恢复和两端运行验收的桥接镜像、回滚 capsule，不能直接迁移或切流。
- 人工运营证据采集此前只查询最新 20 条记录；当目标记录较旧时会误报缺失。已在后续工作目录中改为逐页查找，定向测试 19/19 通过。此改动尚未构成当前线上证据。
- 当前自动化无法操作本机 ChatGPT.app，仍需在可访问宿主的发布环境完成候选版 15 场景和安装包签名验收。
