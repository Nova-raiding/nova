...
结论：合同与交付视频的直接上传、隔离扫描、可信回执接纳及登记回显已在独立真实运行环境通过；不代表共享部署已升级或账号已实际启用。

### 本轮实现

- 合同支持 PDF、DOCX、PNG、JPEG，视频支持 MP4、WebM；每个文件上限 50 MiB。前后端校验用途、格式、实际字节大小与 SHA256，拒绝伪造扩展名、损坏的视频结构及不规范 Base64。
- 页面明确区分上传中、扫描中、可登记、失败和已取消；失败逐项重试，扫描中的重试复用已有素材，不虚构进度。多视频逐项登记，已成功项不因后续失败重复写入。
- 新增仅平台运营可用的上传/状态 MCP 契约。扫描采用独立运营授权事件，worker 在执行时重新核对真实签名、租户、交付档案、用途、素材版本、对象键与摘要；不借用商家商业调用或授予点数。
- 同一干净文件在不同交付档案复用时生成各自的绑定事件，修复第三次复用时持久化序号冲突。扫描回执的重复接纳只允许已有同一份可信回执，不接受任意终态资产。
- MIME/容器结构识别和病毒扫描不等于合同内容语义解析；HTTPS 外部凭证也不被冒充成平台已扫描文件。

### 运行与测试证据

- 验收源码快照 `/tmp/customer-delivery-upload-verify.AOH7AN`：全量 TypeScript 检查通过；20 个定向文件、458 项后端/worker/契约/隔离脚本测试通过；3 个前端文件、29 项测试通过。
- 桌面 1440×900、`Asia/Shanghai`，真实生产构建 UI → 签名 OIDC → API/MCP → PostgreSQL 17/Redis → 原生 worker → ClamAV，完整浏览器用例通过，48.7 秒。之后只读数据库/对象取证通过，整体进程退出码为 0。
- 真实 ClamAV 1.4.6、病毒库 28123（发布时间 `2026-09-14T06:24:19Z`）；普通内容探针和 EICAR 自检均通过，没有合成扫描结论。独立扫描容器顺序运行病毒库更新与引擎启动，避免并行加载引发内存不足。
- 1 份真实 PDF、2 段真实 MP4/WebM 文件完成隔离区入库、扫描尝试持久化、签名回执被 API 接纳、干净对象入库、登记及刷新。三份对象逐一核对实际字节 SHA256 和元数据；每份回调接纳一次、worker 已完成、无前次失败。
- 上传、档案更新及视频登记均有持久化审计。没有点数授予、商业操作、预占或流水；未初始化余额明确为 `null`，不宣称已有确定的零余额。
- 覆盖缺失合同/视频 409、18 项验收清单、培训独立确认、上传待扫描时取消并切换档案不串数据，以及同一干净文件在三个新档案复用后的各自可见性。签名、角色、租户、用途、摘要和通用素材越权拒绝有定向测试。
- 大载荷边界同时覆盖 40 MiB 网关转发、50 MiB 文件解析及 70 MiB MCP 传输上限，保留普通请求的独立限制。
- 使用 shot-scraper 实际操作培训开关和视频抽屉，保留 PNG/WebM。仅停止按容器 ID/运行标签确认属于本轮的 PostgreSQL、Redis、ClamAV，清理报告 `leftRunning: []`；未修改共享容器或删除业务数据，也未输出凭据、文件正文或回执签名。

可复核证据：`artifacts/customer-delivery-acceptance/run-f6677f9a/`，运行 ID `f6677f9a-ca5b-4a36-86a3-490b6f606dbe`。

- `scan-result.json`：三份文件的对象摘要、真实回执接纳、worker 完成、审计与无点数消耗证据。
- `runtime.json`、`scanner-readiness.json`：真实隔离运行环境及扫描器自检。
- `fixture-disposal.json`、`scanner-disposal.json`：仅本轮临时资源的清理结果。
- `browser-result.json`、`video-registration-shot-scraper.png`、`training-confirmed-shot-scraper.png`、`training-inline.webm`：桌面交互结果。

可复跑（需已缓存的固定 ClamAV 镜像、本地 Docker、Chrome 和 shot-scraper；扫描器会校验病毒库新鲜度）：
...
后续工程项：新扫描事件执行查询仍沿用按素材读取有限历史事件的接口，长期大量重扫应改为按事件 ID 精确读取。客户档案完成与账号/权限实际启用仍是独立未验收项；不得据此自动恢复已被管理员停用的主体。本轮也不构成真实模型、中转成本、支付或生产发布门禁的全面验收。

## 集成复核与当前阻断

本轮整合时，共享 `main` 从 `f98c69da` 前进至 `dc72e177`，且未提交的交付源文件继续被改写。已保留最新商业提交和其他并行修改；没有 reset、全量 stash、覆盖恢复他人代码或停止共享容器。

### 商业读取权限修复

- 最新商业自读策略未对 V2 支付订单执行创建者过滤。新增 `readOwnCommercialPaymentStatus`，同时核对工作区、订单 ID 和创建者；本人支付查询保留，其他账号或未知订单均不可见。此 self 接口对管理员也不隐式放宽。
- V2 点数流水包含企业全量操作与凭据，恢复 `billing.workspace.read`，不把企业流水伪装成本人数据。余额、目录和访问概览继续可读；HTTP/MCP 精确策略断言按实际范围分别校验。
- helper 20 项与真实 loopback HTTP/MCP 流水 6 项测试通过。独立真实 PostgreSQL 17、严格签名 OIDC、durable authz、HTTP 与原生 MCP 又完成 33 项检查：创建者可读，同租户其他管理员、跨租户、错误 ID、无创建者均不可读；无认证拒绝。测试使用明确的待支付合成订单，没有调用真实支付。
- 真实运行证据：`artifacts/commercial-read-boundaries/run-IhoaUG/result.json`，运行 ID `cd63151f-090a-4307-a3e7-6ce95b1e8352`。读取使用 `merchant_app`、READ ONLY、非超级用户且不绕过 RLS；订单与快照未变化，支付事件、点数授予、点数操作及流水均为 0。API 和两个本轮临时容器已清理，`leftRunning: []`。

### 清单绕过与并行改写

- 独立基线发现普通 patch 与 scalar 完成接口可直接修改系统接入/功能验收汇总，绕过 18 项清单。已在 `/tmp/customer-delivery-upload-verify.AOH7AN` 修复两处入口：派生状态不能普通 patch，非档案清单不能 scalar 完成，只由真实逐项/批量记录推导。
- 快照新增 `apps/api/src/customer-delivery-checklist-boundary.e2e.test.ts`，真实 HTTP/MCP 从 8 失败、6 通过变为 14/14 通过；拒绝后 revision、状态、清单不变，合法 18 项可回读，培训不联动。此处是独立内存仓储 API 证据，不是新的真实扫描或 PostgreSQL验收。该修复和测试暂保留在独立快照，尚未覆盖集成到共享源码。
- owner 整合复跑：同一独立快照全量 TypeScript 检查通过；21 个后端/worker/契约/隔离脚本文件、472 项测试全部通过（包含新 14 项）；原 3 个前端文件、29 项通过的范围未被这两处 API 保护修改。
- 同期共享目录的上传用途扩展为付款、培训、接入和验收附件，并新增付款/培训/逐项扫描凭证门禁；替换证据函数期间出现类型检查失败和 HTTP 500，已有成功测试的准备条件也被改变。未用合成扫描结果填平这些新门禁，未削弱断言宣称全绿。
- 因共享代码仍在持续变化，必须先统一稳定的集成基线，再合入上述清单入口修复、确认新凭证字段的需求及迁移、重新执行类型检查/API/桌面/真实扫描全链路。当前记录不构成共享 `main` 已通过验收的证据。

### 生效联动尚不能宣称完成

交付档案只有工作区引用，同工作区允许多份公司档案，没有唯一绑定到真实账号或商业订单；人工标记付款也不是支付核验。账号启用仍未联动。后续需明确控制对象，使用独立交付业务门禁与原有身份、管理员停用、商业权限叠加；保留登录、付款、授权、上传和受控验收入口，不可把交付未完成简单等同于停用企业，更不能自动恢复管理员停用。
