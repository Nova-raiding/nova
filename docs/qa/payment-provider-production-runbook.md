...
同一受保护目录还会保存签名 checkout 参数、支付宝已签名查单响应（仅已付款）、退款提交响应及最终退款查单的脱敏来源收据；退款的 `processing` 结果绝不等于退款成功。最终退款查单收据只在已验签响应精确包含 `code=10000`、`refund_status=REFUND_SUCCESS`，且订单号、退款请求号、金额与请求一致并有 provider 交易号时写入，同时记录已验签原始响应的 SHA-256。网关对这些路径仅保存哈希、金额、时间和结果，不保存原始参数、交易号、密钥或付款人资料。网关没有数据库账本访问权，收据明确标记 `ledger_state_observed=false`、`final_evidence=false`；它不能证明钱包预留已释放或订单已关单。收据保留窗口建议为 7 天，至少覆盖证据签发、退款对账及回滚观察期；到期清理须按具体文件先核对证据引用、审批和可恢复性并留审计记录，禁止对整个目录执行盲目递归删除。当前代码尚未实现自动保留清理、回调重放/账本前后状态采集或六阶段最终签发，因此不能据此宣布支付上线门禁通过。
...
6. 最后执行已授权的 reconciliation，保存真实 provider 查询与数据库账本前后状态、租户边界和审计 ID。只有无 pending/unknown/金额差异，且六阶段各自的来源收据/数据库记录均与同一订单相符，才允许离线受保护签发器生成最终 Ed25519 证据。当前第 5 步仅新增网关来源收据，仍缺数据库账本前后状态的受保护采集；第 3、6 步与最终签发器也未完成，生产门禁仍为 NO-GO；不得手填 `pass` JSON 或使用本地 fixture 替代。
