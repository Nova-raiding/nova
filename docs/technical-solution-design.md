...
版本：v1.0 RC  
日期：2026-08-22  
产品基线：[PRD-merchant-marketing-codex-final.md](./PRD-merchant-marketing-codex-final.md)  
部署基线：[cloud-resources-and-deployment.md](./cloud-resources-and-deployment.md)  
...
docs/                      # ADR、runbook、API 与 readiness evidence
```
...
完整字段见 [production-config.example.yaml](./production-config.example.yaml)。示例中的 `SET_*`、关闭的 feature flag 和未验证额度都属于上线阻断项。
