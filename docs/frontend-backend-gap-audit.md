...
Ops 专用 runner 最终结果为 7 通过、1 跳过；覆盖全平台页面巡检、模型状态失败关闭、无凭据连接诊断、401 重新认证、平台导航和用户目录治理。跳过项是需要独立 workspace token fixture 的工作区成员治理。当前 Ops 验收使用隔离账号密码登录和服务端 Cookie 会话；历史 OIDC runner 环境变量不再是启动条件。该历史结果不代表当前密码登录链路的验收证据，当前结果以 `npm run test:browser:ops` 最近一次记录为准。
