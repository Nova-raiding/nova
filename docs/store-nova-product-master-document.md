...
平台规则由平台运营从官方来源人工整理上传，或由受控 Worker 从签名清单同步；不接受插件或商家上传的“规则截图”作为权威版本。人工上传只生成 `draft`，必须经过规则管理员独立审批并激活，才能成为所有商家可读取的公共规则。两条路径都必须保留来源 URL、版本、checksum、发布时间/生效时间、适用平台和审批审计，生成不可变 `rule_snapshot`；导入失败、版本过期、来源或审批证据缺失时，该平台状态为 `not_configured/blocked`，不能降级为搜索摘要或模型常识。
