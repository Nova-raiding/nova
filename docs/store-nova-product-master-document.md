...
Store Nova是安装在桌面 ChatGPT 中的电商商品营销助手。商家用自然语言选择商品和 SKU，系统从商家自己的工作区读取已确认的商品事实、素材、品牌和规则，通过Store Nova自己的模型中转站生成文案、主图、详情页和视频，商家审核后可以下载或发布到已授权店铺。

ChatGPT 只是交互入口。权限、商品事实、账务、模型调用、规则、发布状态和审计都以Store Nova服务端为准。
...
1. 用户可在 ChatGPT App 市场搜索并安装“Store Nova”。
2. 用户可用Store Nova账号和密码登录插件及商家后台。
3. 平台运营可审核商家开通、绑定企业、开通套餐和创意点。
...
11. 所有Store Nova业务模型调用必须经过Store Nova自有模型中转站。
...
| GEN-01 | 文案、主图、详情页、视频脚本/成片通过Store Nova relay 生成 | P0 | provider request/usage/cost/错误证据完整 |
...
flowchart LR
  U[商家] --> C[桌面 ChatGPT]
  C --> P[Store Nova插件 / MCP Bridge]
  P --> A[Store Nova API / MCP Gateway]
...
下面的链路是产品验收的主线。它严格按商家实际操作顺序展开；“把账号给平台运营”只指提供登录账号，绝不提供密码。平台 Owner 的账号预置属于后台初始化，不是商家每次使用的前置动作。每一步都必须有明确输入、输出、责任边界和失败恢复动作。

```text
① 在 ChatGPT App 市场搜索并安装“Store Nova”
   → 插件首屏打开Store Nova账号登录页（账号 + 密码）
② 平台运营创建商家账号，提交企业资料
   → 生成商家待开通记录并进入 merchant_pending
③ 商家把Store Nova登录账号提交给平台运营
...
⑥ 商家选择第三方平台并走官方 OAuth 授权店铺
   → 同步商品、SKU、库存和图片（第三方店铺密码不交给Store Nova）
⑦ 商家在后台上传 Excel/CSV 和素材
   → 扫描、解析、事实/权益确认、规则检查、知识索引
⑧ 商家在 ChatGPT 输入商品名/SKU
   → 插件只从当前工作区取已批准事实和知识，调用Store Nova中转生成
...
| 商家账号开通 | 平台运营 | 登录账号、初始密码、企业资料、工作区、套餐 | 创建 merchant identity、账号和工作区成员关系 | `active` / `invited` | 商家无公开注册入口；账号由平台运营创建后交付 |
| 开通交接 | 商家 → 平台运营 | 登录账号（不是密码） | 平台运营按账号检索并绑定企业、合同、套餐 | `platform_review` | 找不到或资料不一致时保持待审核 |
...
| 生成 | 商家/插件 | 商品、SKU、目标、已确认方案 | 预占点数，调用Store Nova relay，保存结果和成本 | `review_required` 版本 | 释放点数或进入对账，不重复调用 |
...
```text
平台运营填写商家账号、初始密码和企业资料
 → 验证账号唯一性与工作区有效性
 → 创建 merchant identity 和 active 账号
 → 创建工作区成员关系并绑定角色
 → 商家登录后台并完成 ChatGPT 官方授权绑定
```

运营侧创建账号：登录账号（唯一）、初始密码、企业名称和联系人等。账号创建完成后，商家使用该账号登录后台并在插件首用流程完成官方授权；不对商家提供申请入口，不返回 token、密码哈希或内部密钥。

商家向平台运营提交的是Store Nova登录账号。平台运营只能查看待处理开通记录、绑定企业和批准商业订单，不能代填密码、模拟商家会话或直接把 `active` 开关写入数据库。
...
| 接口 | 用途 | 关键约束 |
| --- | --- | --- |
| `POST /v1/auth/register` | 兼容历史/测试环境的商家开通入口（自助调用兼容） | 默认关闭；生产和正常本地环境返回 `AUTH_PUBLIC_REGISTRATION_DISABLED`，商家账号必须由平台运营创建 |
...
| --- | --- | --- |
| 加载中 | 正在验证账号 | 加载业务列表 |
| 未登录 | Store Nova账号、密码和登录按钮 | 显示空业务页或旧工作区 |
...
平台规则的正式生效版本由受控 Worker 从核验的官方来源签名清单同步；不接受插件或商家上传的“规则截图”作为权威版本。运营人工整理上传只生成 `manual://` 未验证内部草稿，普通审批不能激活，也不会进入商家规则、生成预检或发布前复检。正式同步必须保留来源 URL、版本、checksum、发布时间/生效时间、适用平台和审批审计，生成不可变 `rule_snapshot`；导入失败、版本过期、来源或签名证据缺失时，该平台状态为 `not_configured/blocked`，不能降级为搜索摘要或模型常识。
...
宿主 ChatGPT 模型只负责理解对话和选择工具。宿主模型的回答不能当成Store Nova模型产物，不能进入Store Nova用量账本，也不能替代 relay 证据。
...
平台运营后台使用Store Nova账号密码登录，默认进入 `platform` 工作台。
...
| --- | --- | --- |
| 账号与平台/商家隔离 | `apps/api` auth/RBAC tests、桌面 Playwright | 真实账号开通、密码重置、平台角色开通和会话撤销 |
...
- [现有商家营销 PRD](store-nova-merchant-marketing-prd.md)
