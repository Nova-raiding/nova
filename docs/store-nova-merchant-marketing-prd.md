# Store Nova商家营销 ChatGPT App 产品 PRD
...
ChatGPT 是交互入口，不是权限、账务、模型、店铺授权或发布的事实来源。所有真实事实必须由Store Nova API/MCP、数据库/RLS、worker 和平台回执确认。

## 2. 目标与非目标

### 2.1 目标

1. 商家可以在 ChatGPT App 市场安装“Store Nova商家营销”，完成登录并由平台开通。
...
9. 所有业务模型请求必须经过Store Nova自有模型中转站，并保留真实鉴权、请求、用量、成本和错误证据。
...
- 不把 ChatGPT 宿主模型替换成Store Nova业务模型；宿主模型和Store Nova业务模型是两层边界。

## 3. 用户与角色

### 3.1 商家 Owner/管理员

- 商家账号由平台运营创建并分配企业工作区；商家使用该账号登录商家后台，再通过 ChatGPT 官方授权页绑定插件。
...
- Store Nova模型中转站：唯一业务模型出口，负责供应商适配、鉴权、限流和用量/成本回执。
...
## 11. Store Nova自有模型中转站硬约束

### 11.1 唯一出口

所有Store Nova业务模型调用必须经过Store Nova自有中转站：
...
这条约束适用于所有正式商家业务环境（pilot、staging、preview、production）。development/test 可以保留依赖注入的 deterministic fixture 供自动化测试，但任何 fixture 结果都必须标记 `simulated=true`、`provider_executed=false`，不能进入商家真实账本、发布回执或上线证据。若环境需要真实业务产物，必须配置并调用Store Nova中转站；不存在“先用宿主模型生成、之后再补 relay 证据”的过渡路径。

### 11.2 Store Nova中转站职责
...
MODEL_RELAY_BASE_URL 为 HTTPS
  ...
  AND Store Nova relay API key 可用
...
ChatGPT 自身的对话编排模型由 ChatGPT 宿主管理，插件无法拦截或替换宿主模型请求。Store Nova硬约束适用于所有“Store Nova业务能力”调用；业务模型不得出现在 ChatGPT 模型选择器中，也不得要求商家提供个人模型 Key。

因此产品文案必须始终区分两层：ChatGPT 宿主模型负责理解对话和选择工具；Store Nova自有中转站负责所有商品业务模型调用和计量。宿主模型生成的自然语言建议不能被记录为Store Nova模型产物，也不能触发点数结算或发布。
...
1. **商家与企业**：商家账号开通、企业绑定、Owner、状态和审批证据。
...
- 首次打开进入Store Nova账号密码登录；OAuth/MCP 只作为宿主授权协议，不出现本机 API 配置。
- 平台运营能在后台创建商家账号、绑定企业和角色，并开通对应商业权益。
...
- 文案、图片、编辑、OCR、视频全部产生Store Nova relay request/usage/cost 证据。
...
- Store Nova relay 是唯一业务模型出口，外部供应商密钥只在 relay 侧存在。
