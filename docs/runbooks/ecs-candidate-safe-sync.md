...
| 旧环境状态保全与接管 | 记录旧公网 `/readyz` 的实际结果、当前运行容器、镜像/配置/网络/卷及数据库版本；完成旧容器归属接管和数据保全 | 旧版不健康时进入故障修复发布流程，不把旧版恢复健康设为修复版部署的成功条件 |
...
隔离候选启动与 ChatGPT 宿主取证见 [候选宿主路由](chatgpt-candidate-host-route.md)。正式部署脚本的完整证据预检发生在生产容器启动**之前**；因此“先部署、后验证”在本项目应指先启动 **101 隔离候选运行时**、在该运行时验证，然后生产切流并再次验收。现有正式部署脚本还在迁移前强制旧生产 `/readyz` 为 200；当旧版已发生故障时，这是修复发布的流程阻断，**不得要求先证明旧版正确才能发布修复版**。同样，回到已知故障的旧版不能算成功恢复。故障修复发布需要单独实现并验收：记录旧版故障及身份，保全数据库和运行配置，验证候选在隔离环境中可用，核对迁移兼容和流量切换方案，并在生产切流后以新版本公网业务验收作为成功条件。该路径目前尚未实现，不得把修改本文当作可以运行现有部署器的依据；旧容器缺少目标 Compose 归属的问题仍需处理。Bridge B 的现有约束见 [Bridge B 手册](ecs-bridge-b-transition.md)。
...
运营台镜像与 API 固定使用 `password` 登录。构建把 `password` 写入摘要固定的 Ops UI 镜像标签；ECS preflight 拒绝其他 Ops 登录模式，并检查镜像标签、API 配置和固定摘要一致。后台账号使用服务端持久会话，不使用静态 token 冒充登录。上线验收需从匿名登录页完成真实账号密码登录，再验证平台角色、会话和退出，不能用健康端点代替。
...
旧 API/Worker 的已知数据库前缀为 242；执行发布前仍须重新读取并核实实时数据库历史。不能让旧运行时代码继续承载流量时直接把共享库迁到 246：旧镜像既无法通过 246 完整链校验，也不是合法的 246 回滚目标。候选 C 的普通 `deploy-verified-ecs-compose.sh` 在消费 nonce 前要求回滚 capsule 的目标镜像迁移链精确覆盖 C 的 `EXPECTED_MIGRATION_VERSION`，并为计划中的实时版本到目标版本之间的每个前缀提供受审查摘要；若要升级数据库，还要求公网 `/releasez` 已经显示回滚桥 B 的完整身份。预部署签名观测中的数据库版本也必须与计划值相同。任一条件失败均为 NO-GO，不得编辑计划版本或迁移记录以绕过。

正确的发布顺序是：在隔离的 PG17 恢复库验证 242→246 的候选迁移链及受保护回滚 capsule；在生产数据库经重新核实仍精确处于 242 前缀、且旧运行时接管与恢复证据完备时，用独立受保护的“只切代码、不运行迁移”步骤安装只兼容 242 的桥 B，并验明公网 `/releasez` 身份；随后以目标迁移链精确覆盖 246 的候选 C、完整迁移前缀摘要和受保护回滚 capsule 发布 C，迁移 242→246 后才切 C。桥 B 是固定在数据库 242 前缀的 runtime-only 阶段，不运行迁移，也不兼容 245 前缀。桥 B 的安装、独立阶段日志和故障恢复执行器尚未完成真实运行验收，因此本说明不是批准对 101 执行过渡发布。未具备这些条件时保持现网，不得把普通一键部署当作桥 B 的第一阶段。
