...
本次已观察到：公网 `release-74c42084` 的 `ready=true`，manifest SHA-256 为 `07c2dc3535a10ad9f7b888e6cf6932d5ef4208f4e6c48a3bd9026e4dd61ccf67`，image-set digest 为 `sha256:fec88c5e2b801660a719643d76d290579e16fe63c9e4f2c9ee39a561c0b0d870`；API 双副本的运行镜像均为 `127.0.0.1:5000/storenova/merchant-api@sha256:1c134f84baf57102eeae76dabc4edba3f1a9dc664dd8bcc866bbec34953361ab`，Ops UI、gateway 和业务 worker 健康，两个公网健康探针返回 200。部署后通过真实商家账号验证登录、`ws_guirenniaoniao` MCP token 签发及刷新均为 200，刷新后仍绑定原工作区；错误身份调用 `workspace.bootstrap` 返回 403，正确身份返回 200、`reused=true` 且工作区不变，随后撤销测试用刷新凭据。此前已通过的 REST/MCP 上传、客户交付和文本生成未重复执行；那次文本生成由 Qwen 中转返回 750 tokens、成本 ¥0.003511、1 个创意点已结算。此前一次上游结果未知的预留仍待核对，不能擅自释放。

客户安装尚未达到正式发布条件：Mac 单文件候选包已从干净提交 `74c42084` 重建，包含最新 token 刷新修复、官方 ChatGPT.app、Node 和插件，SHA-256 为 `cf84e6511439f127a5dcf533705a2ee63d199075fa3cf89fbe6c25a21fc3fdf5`；但它仍标记 `unsigned_candidate`、`ready_to_install=false`，缺 Developer ID 签名、公证和 ChatGPT 图形界面绑定验收。Windows `192.168.1.104` 当前不可达，尚无 Windows 包和目标机安装证据。图片生成、图片编辑、OCR、视频虽有中转配置，但该工作区没有经确认的真实商品图片与资料，也没有四种模态的 provider 用量回执；OCR 创意点费率尚未获批准。这些项不得以配置就绪或健康探针通过替代上线证据。
