...
本次已观察到：公网 `release-4c711dd4` 的 `ready=true`，manifest SHA-256 为 `0f1739a06c4a3c8af9452cce373137842573b1bf5889e335e5e9e0acec91a264`，image-set digest 为 `sha256:57ff59983a716e90a11021c56e196b69ae9a527f51026d9cf30a0a1302137075`；API 双副本、Ops UI、gateway 和业务 worker 健康，两个公网健康探针返回 200。商家/运营登录、REST/MCP 上传和客户交付已通过；公网 `merchant.first_value` 未绑定候选生成返回 200、`provider_executed=true`，对应 Qwen 中转 750 tokens、成本 ¥0.003511、1 个创意点已结算。`4c711dd4` 只修改提供商已成功但后续事件持久化失败时的钱包退款处理，部署后已验证身份与健康，未再次发起付费生成。此前一次上游结果未知的预留仍待核对，不能擅自释放。

客户安装尚未达到正式发布条件：Mac 单文件包从干净提交 `6d7d3c73` 构建，包含官方 ChatGPT.app、Node、插件及安装脚本，但缺 Developer ID 签名、公证和 ChatGPT 图形界面绑定验收；Windows `192.168.1.104` 当前不可达，尚无 Windows 包和目标机安装证据。图片生成、图片编辑、OCR、视频虽有中转配置，但该工作区没有经确认的真实商品图片与资料，也没有四种模态的 provider 用量回执；OCR 创意点费率尚未获批准。这些项不得以配置就绪或健康探针通过替代上线证据。
