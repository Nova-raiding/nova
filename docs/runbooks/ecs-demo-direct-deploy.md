...
本次已观察到：公网 `release-109ec223` 的 `ready=true`，manifest SHA-256 为 `23557ea1de6d6f84d6805493cc4e4202d7c464d8cc2f77bdc2a87dfbfadb9636`，image-set digest 为 `sha256:9ba641ede1b616209bb447f428a986d44a60e1259ae5d13c65e7e18f76549dd6`；API 双副本的运行镜像均为 `127.0.0.1:5000/storenova/merchant-api@sha256:6bcb89817b795bd664c2437e8cf39ff5eb18d9d98c00733518fd632bd0c9e22d`，两个公网健康探针返回 200。`109ec223` 增加结果未知的模型请求关联记录；部署后已在运行 API 容器中验证 `unknown:` 关联标识和 `outcome=unknown` 的代码产物，未人为制造付费请求的不确定结果。上一版部署后已通过真实商家账号验证登录、`ws_guirenniaoniao` MCP token 签发及刷新均为 200，刷新后仍绑定原工作区；错误身份调用 `workspace.bootstrap` 返回 403，正确身份返回 200、`reused=true` 且工作区不变，随后撤销测试用刷新凭据。本版未改动这些授权路径，也未重复已通过的 REST/MCP 上传、客户交付和文本生成；那次文本生成由 Qwen 中转返回 750 tokens、成本 ¥0.003511、1 个创意点已结算。此前一次上游结果未知的预留仍待中转站权威账单核对，不能擅自释放。
