...
客户安装尚未达到正式发布条件：Mac 单文件包需包含最新 token 刷新修复，且仍缺 Developer ID 签名、公证和 ChatGPT 图形界面绑定验收；Windows `192.168.1.104` 当前不可达，尚无 Windows 包和目标机安装证据。图片生成、图片编辑、OCR、视频虽有中转配置，但该工作区没有经确认的真实商品图片与资料，也没有四种模态的 provider 用量回执；OCR 创意点费率尚未获批准。这些项不得以配置就绪或健康探针通过替代上线证据。
