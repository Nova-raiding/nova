...
3. Provision the fixed root-owned directories, protected Node runtime, Ed25519 trust material, and the shared one-use deployment nonce consumer using [ecs-production-nonce-consumer.md](ecs-production-nonce-consumer.md). Run its isolated smoke test against the reviewed source before first installation. The consumer digest in `/run/release-security/evidence-trust/production-evidence-nonce-consumer-sha256` must match the installed consumer. Its SQLite ledger is durable state; never replace, truncate, or delete it.
