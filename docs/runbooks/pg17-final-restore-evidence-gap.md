...
当前 worker 的正式入口要求 `WORKER_WORKSPACES`；生产环境中的 `sync` 等回调角色还要求 `WORKER_API_BASE_URL`、`WORKER_API_TOKEN`、`WORKER_API_SIGNING_SECRET`。现有预检故意不允许工作区和回调密钥进入恢复环境。即使补齐这些变量，`WORKER_ONCE=true` 仍会执行一次 `pollOnce`，可能领取真实 outbox 事件；空队列时成功退出也不能证明执行了恢复任务。因此不能用该开关冒充无派发烟测，不能仅靠恢复库只读角色拦截写入后宣称成功。后续实现需要在候选 worker 镜像中提供独立、明确只读的恢复探针入口；固定入口应读取恢复目标的迁移链和一个预先定义的业务只读查询，核对隔离 Redis 身份，并在无外网的内部网络运行。保护端须独立检查该入口、镜像摘要、只读角色、实际容器和网络身份，保留不可变原始输出；任何凭据缺失、目标偏离或业务查询失败均保持 NO-GO。此设计尚未实现或在 101 上验收。
