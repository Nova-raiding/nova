...
正式 worker 的 `WORKER_ONCE=true` 仍会执行一次 `pollOnce`，可能领取 outbox 事件，因此恢复入口只接受 `RESTORE_SMOKE_MODE=isolated_read_only`、一个已审核的 `RESTORE_SMOKE_WORKSPACE_ID` 与专用只读数据库/隔离 Redis URL。独立探针使用只读事务核对 1..候选目标迁移链、`restore_app` 非超级用户/非 BYPASSRLS 身份、该工作区的 `active` 行及 Redis `PING`；不导入或调用正式 worker poll loop。调用者还须提供 `--worker-output`，路径在受保护的 `preview-restores` 目录，且不得预先存在。恢复容器中 `restore_app` 凭据、独立 Redis 与真实角色观察仍须在 101 受保护地供应并实测；当前没有这些生产输入或运行结果。单项 worker 观察不能证明 API、真实 ChatGPT Desktop 或完整数据恢复，最终签名器仍缺失，故整项 restore gate 保持 NO-GO。

候选镜像构建后，在无网络容器中核对编译入口确实存在：`docker run --rm --pull=never --network none --entrypoint test <审查过的worker镜像@sha256:...> -f /app/dist/apps/worker/src/restore-smoke.js`。这条命令只检查镜像文件，不执行恢复探针或数据库查询。正式探针只能由受保护预检在确认隔离网络、容器、角色与镜像身份后调用；其退出非零与 `status=incomplete` 原始记录均不能改写为最终恢复通过。
