...
export function DeliveryReadinessPanel({ baseUrl }: { baseUrl?: string }) {
  ...
  const mediaRows = capabilities ? mediaSpecReadiness(capabilities) : []
  ...
  const announced = loading ? '正在读取平台交付状态' : error ? '平台交付状态读取不完整，所有缺失能力保持阻断' : `已读取 ${mediaRows.length} 项媒体规格、${mappings.length} 项字段检查、${bundles.length} 个交付包`
  return <section ref={panelRef} tabIndex={-1} className="panel delivery-readiness" aria-labelledby="delivery-readiness-title" aria-busy={loading}>
    ...
    {!loading && baseUrl && <div className="delivery-readiness-grid">
      <article><h4>媒体规格</h4>{mediaRows.length ? <div className="delivery-status-list">{mediaRows.map(row => <div key={row.id} className={`delivery-status ${row.readiness}`}><div><b>{platformLabel[row.platform] ?? row.platform} · {row.capability}</b><span>来源 {row.source ?? '未提供'} · 版本 {row.version ?? '未提供'} · 有效期 {row.validUntil ?? '未提供'}</span><small>校验依据：{row.evidence ? '已绑定服务端依据' : '未提供'}</small><small>下一步：{row.nextAction}</small></div><strong>{row.readiness === 'approved' ? '已批准' : row.readiness === 'expired' ? '已过期' : '未验证'}</strong></div>)}</div> : <Empty label="API 未返回平台媒体规格" detail="发布保持阻断；请由运营端登记并批准规格。"/>}</article>
