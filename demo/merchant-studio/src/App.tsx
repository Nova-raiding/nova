...
type Target = { productId: string; platform: PlatformId; title: string; remoteId?: string; accountId?: string; taskId?: string }
...
function Products({ baseUrl, initialQuery = '', onSelectTarget }: { baseUrl?: string; initialQuery?: string; onSelectTarget: (target: Target) => void }) {
  ...
  const loadProducts = () => {
    if (!baseUrl) return
    setLoading(true); setError('')
    fetchProducts(baseUrl).then(setRemoteProducts).catch((cause: Error) => setError(describeApiError(cause))).finally(() => setLoading(false))
  }
  const loadAccounts = () => {
    const requestId = ++accountsRequestId.current
  ...
  const rows = remoteProducts?.map(product => ({ id: product.id, platformId: product.platform, accountId: product.accountId, title: product.title, remoteId: product.remoteId, name: product.title, sku: `${product.skuCount} SKU`, platform: platformLabel[product.platform] ?? product.platform, source: product.source === 'official_api' ? '官方 API' : '演示数据', status: product.factsConfirmed ? '事实已确认' : '待确认', stock: product.stock, issue: product.factsConfirmed ? 0 : 1 })) ?? products.map((product, index) => ({ ...product, id: `prod_fixture_${index + 1}`, platformId: (Object.entries(platformLabel).find(([, label]) => label === product.platform)?.[0] ?? 'taobao') as PlatformId, title: product.name, remoteId: undefined, accountId: undefined }))
  const visible = useMemo(() => rows.filter(p => {
    if (!(p.name.includes(query) || p.platform.includes(query))) return false
  ...
  return <div className="page-stack">
    ...
    <section className="panel table-panel">
      ...
      {selectedTargets.length > 0 && <div data-testid="task-group-selection" className="task-group-selection"><div><b>多平台任务组选择</b><small>每个平台只能选择一个商品；重新勾选同平台商品会替换原选择。</small></div>{selectedTargets.map(item => <span key={item.platform}><StatusChip tone="green">{platformNames[item.platform]}</StatusChip>{item.title}</span>)}</div>}
      <div className="table-wrap">{loading ? <LoadingState label="正在读取商品事实…" /> : visible.length ? <table><thead><tr><th>任务组</th><th>商品</th><th>平台</th><th>事实来源</th><th>可售库存</th><th>状态</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{visible.map(product => { const target = { productId: product.id, platform: product.platformId, title: product.title, remoteId: product.remoteId, accountId: product.accountId }; return <tr key={product.id}><td><input type="checkbox" aria-label={`选择${product.name}加入任务组`} checked={selectedTargets.some(item => item.productId === product.id)} onChange={() => toggleTarget(target)} disabled={!baseUrl} /></td><td><div className="product-cell"><div className="product-thumb"><ShoppingBag size={20} /></div><div><b>{product.name}</b><span>{product.sku}</span></div></div></td><td>{product.platform}</td><td><StatusChip tone="neutral"><Link2 size={12} />{product.source}</StatusChip></td><td>{product.stock.toLocaleString()}</td><td><StatusChip tone={product.issue ? 'amber' : 'green'}>{product.issue ? <AlertCircle size={12} /> : <Check size={12} />}{product.status}</StatusChip></td><td><button className="text-button" onClick={() => checkImages(product.id)} disabled={!baseUrl}>主图检查</button><button className="text-button" onClick={() => onSelectTarget(target)} disabled={!baseUrl} title={!baseUrl ? '配置 API 后可创建真实任务' : undefined}>创建任务 <ArrowRight size={14} /></button></td></tr>})}</tbody></table> : <div className="empty-state"><PackageSearch size={22} /><b>没有匹配商品</b><span>调整搜索条件，或重新同步平台商品。</span></div>}</div>
...
function TaskWorkspace({ openPublish, baseUrl, target, onContext, onSelectTarget, onBack }: { openPublish: () => void; baseUrl?: string; target?: Target; onContext: (context: TaskContext | null) => void; onSelectTarget: (target: Target) => void; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true); setError(''); setOperation(target.taskId ? '恢复原任务…' : '创建任务…'); setContent(null); setContentVersions([]); setFindings([]); setReviewCategories([]); setReviewTab('findings'); onContext(null)
    fetchProducts(baseUrl).then(items => { if (!cancelled) setProduct(items.find(item => item.id === targetProductId) ?? null) }).catch(() => { if (!cancelled) setProduct(null) })
    const initialTask = target.taskId ? fetchTask(baseUrl, target.taskId) : createTask(baseUrl, { product_id: targetProductId, platform: targetPlatform, ...(target?.accountId ? { account_id: target.accountId } : {}) })
    initialTask
      .then(async current => {
  ...
  return <div className="task-shell">
    ...
    <div className="task-titlebar"><div><button className="back-link" onClick={onBack}><ArrowRight size={16} />所有任务</button><h2>{targetTitle} · {platformNames[targetPlatform]}详情页</h2><div className="task-meta"><StatusChip tone={approved ? 'green' : 'blue'}>{approved ? '已批准' : loading ? '准备中' : taskStateLabel(task?.state ?? '')}</StatusChip><span>内容版本 v{content?.version ?? 0}</span><span>{operation || '已保存'}</span></div></div><div className="button-row compact"><button className="secondary" onClick={() => { if (baseUrl && task) fetchTaskTimeline(baseUrl, task.id).then(setTimeline).catch(() => undefined); setTimelineOpen(true) }} disabled={!task}><History size={16} />历史</button><button className="primary" disabled={!approved || Boolean(operation)} onClick={openPublish}><Rocket size={16} />进入发布</button></div></div>
    ...
    <div className="workspace-grid">
      <aside className={`context-panel ${contextCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="context-product"><div className="product-visual"><ShoppingBag size={32} /></div><div><StatusChip tone={platformTone[targetPlatform]}>{platformNames[targetPlatform]}</StatusChip><b>{targetTitle}</b><span>{target?.remoteId ? `远端商品 ${target.remoteId}` : '等待平台商品标识'}</span></div></div>
...
export default function App() {
  ...
  const openPublish = async () => {
    ...
    try { setPublishPreview(apiBaseUrl ? await preparePublish(apiBaseUrl, taskContext.task.id) : null); setPublishModal(true) }
    catch (cause) { setToast(`发布预览失败：${describeApiError(cause)}`); window.setTimeout(() => setToast(''), 5000) }
  }
  const submitPublish = async () => {
    if (!apiBaseUrl) return 'REQ-88214'
    if (!taskContext?.task || !taskContext.version) throw new Error('请先完成目标商品的内容审核，再进入发布。')
    const task = taskContext.task
    const draft = taskContext.version
    const preview = publishPreview ?? await preparePublish(apiBaseUrl, task.id)
    const job = await confirmPublish(apiBaseUrl, { task_id: task.id, content_version_id: draft.id, confirmation_hash: preview.confirmationHash, remote_snapshot_hash: preview.remoteSnapshotHash, ...(task.accountId ? { account_id: task.accountId } : target?.accountId ? { account_id: target.accountId } : {}) }, `ui-${task.id}-${draft.id}`)
    return job.id
  }
  ...
  const openCorrection = async (job: PublishJob) => {
    ...
    try {
      ...
      setTarget({ productId: rejectedTask.productId, platform: rejectedTask.platform, title: '被平台驳回的商品', accountId: rejectedTask.accountId, taskId: rejectedTask.id })
      setTaskContext(null)
      setPage('task')
  ...
  return <div className="app-shell">
    ...
    {publishModal && <PublishModal close={() => setPublishModal(false)} preview={publishPreview} target={target ?? (taskContext ? { productId: taskContext.task.productId, platform: taskContext.task.platform, title: '当前任务商品', accountId: taskContext.task.accountId } : undefined)} onSubmit={submitPublish} onComplete={completePublish} returnFocus={publishTrigger.current} />}
