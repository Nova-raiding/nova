...
function ImageGenerationJobPanel({ baseUrl, jobId }: { baseUrl?: string; jobId: string }) {
  ...
  const toggleVisual = (visualRef: string, allowed: boolean) => {
    if (!allowed) return
    setSelectedVisualRefs(current => current.includes(visualRef) ? current.filter(item => item !== visualRef) : current.length >= 6 ? current : [...current, visualRef])
  ...
  return <section className="panel image-generation-job-panel" aria-labelledby="image-job-title" aria-busy={loading}>
    ...
    {job?.images?.length ? <div className="image-candidate-grid" aria-label="已归档图片候选">{job.images.map((src, index) => {
