...
export function recoveryCopy(message: string) {
  if (message.includes('FORBIDDEN') || message.includes('授权决策拒绝') || message.includes('没有权限') || message.includes('无权访问')) return { title: '当前会话无权读取这项任务', body: '服务端拒绝了当前身份对任务或商品事实的读取请求。页面不会用演示数据替代；请切换到有权访问该工作区的商家账号后重试。', primary: '返回商品与素材范围' }
