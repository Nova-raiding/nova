  ...
  try {
    ...
    if (!response.ok || envelope.error) {
      ...
      const details = envelope.error?.details && typeof envelope.error.details === 'object' && !Array.isArray(envelope.error.details)
        ...
        : undefined
      if (path !== '/mcp' && isSessionAuthFailure(response.status, errorCode, errorMessage)) {
        authExpired = true
...
export function describeApiError(error: unknown) {
  ...
  if ([
    ...
    'IMAGE_GENERATION_CALLBACK_CONFIG_MISSING',
...
export const fetchImageGenerationJob = (baseUrl: string, jobId: string) => requestApi<ImageGenerationJobWire>(baseUrl, `/v1/image-generation-jobs/${encodeURIComponent(jobId)}`, {}, undefined, MAX_IMAGE_JOB_RESPONSE_BYTES).then(value => { const preferred = value.preferred_candidate ? { visualRef: value.preferred_candidate.visual_ref, selectedAt: value.preferred_candidate.selected_at, status: value.preferred_candidate.status, reviewRequired: value.preferred_candidate.review_required, approvalRequired: value.preferred_candidate.approval_required, platformPublished: value.preferred_candidate.platform_published } : value.preferredCandidate ?? null; return { jobId: value.job_id, revision: value.revision, state: value.state, archiveState: value.archive_state, productId: value.product_id, taskId: value.task_id, contentVersionId: value.content_version_id, imageMode: value.image_mode, direction: value.direction, requestedCount: value.requested_count, sourceAssetIds: value.source_asset_ids, sourceProductVersion: value.source_product_version, intentHash: value.intent_hash, executionState: value.execution_state, providerRequestId: value.provider_request_id, executionAttempt: value.execution_attempt, reconciliationRequired: value.reconciliation_required, errorCode: value.error_code, errorMessage: value.error_message, updatedAt: value.updated_at, createdAt: value.created_at, preferredCandidate: preferred, outputs: value.outputs.map(output => ({ visualRef: output.visual_ref, ordinal: output.ordinal, assetId: output.asset_id, archiveReceiptId: output.archive_receipt_id, archiveReceiptDigest: output.archive_receipt_digest, storageKey: output.storage_key, mimeType: output.mime_type, sizeBytes: output.size_bytes, sha256: output.sha256, createdAt: output.created_at, reviewStatus: output.review_status, gate: output.gate })), images: value.images, availabilityWarning: value.availability_warning, nextAction: value.next_action } })
