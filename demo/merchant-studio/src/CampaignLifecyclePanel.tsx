...
export function CampaignLifecyclePanel({ baseUrl }: { baseUrl?: string }) {
  ...
  if (!showControls) return <section className="panel campaign-lifecycle campaign-lifecycle-collapsed" aria-labelledby="campaign-lifecycle-title"><div className="panel-heading"><div><span className="section-kicker">ADVANCED CONTROL</span><h3 id="campaign-lifecycle-title">批量任务控制</h3><p className="panel-subtitle">暂停、恢复或重试失败项属于高级操作；打开后从当前工作区的批量计划列表选择。</p></div><div className="button-row compact"><button className="secondary" onClick={openProductSelection}>选择商品开始任务</button><button className="text-button" onClick={() => { setShowControls(true); void loadCampaignOptions() }} disabled={!baseUrl}>打开高级控制</button></div></div></section>
  return <section className="panel campaign-lifecycle" aria-labelledby="campaign-lifecycle-title" aria-busy={loading || submitting}>
    <div className="panel-heading"><div><span className="section-kicker">BATCH CONTROL</span><h3 id="campaign-lifecycle-title">批量任务控制</h3><p className="panel-subtitle">读取任务批次后，可暂停、恢复或重试失败项；任何状态冲突都会阻断。</p></div><button className="text-button" onClick={() => setShowControls(false)} disabled={loading || submitting}>收起高级控制</button></div>
    <div className="campaign-lookup"><label htmlFor="merchant-campaign-select">选择批量计划<select id="merchant-campaign-select" value={campaignId} onChange={event => { setCampaignId(event.target.value); setCampaign(undefined) }} disabled={loading || submitting || !campaignOptions.length}><option value="">{loading ? '正在读取批量计划…' : campaignOptions.length ? '请选择一个批量计划' : '当前没有可操作的批量计划'}</option>{campaignOptions.map(option => <option key={option.id} value={option.id}>{campaignStateLabel(option.state)} · {option.itemCount} 项 · {new Date(option.updatedAt).toLocaleString('zh-CN', { hour12: false })}</option>)}</select></label><button className="secondary" onClick={() => void load()} disabled={!baseUrl || !campaignId.trim() || loading || submitting}>{loading ? <RefreshCw className="spin" size={16}/> : null}{loading ? '读取中…' : '读取所选计划'}</button><button className="text-button" onClick={() => void loadCampaignOptions()} disabled={!baseUrl || loading || submitting}>重新读取列表</button></div>
    {!baseUrl && <div className="info-notice">API 未配置，campaign 控制保持关闭。</div>}
    {error && !action && <div ref={errorRef} tabIndex={-1} className="inline-error" role="alert"><AlertCircle size={16}/><span>{error}</span><button className="text-button" onClick={() => void (errorSource === 'list' ? loadCampaignOptions() : load())}>重新读取</button></div>}
    ...
    {campaign && <div ref={statusRef} tabIndex={-1} className="campaign-status"><div><b>已选择批量计划</b><span>{campaignStateLabel(campaign.state)} · 当前状态版本已读取 · {campaign.items.length} 项</span>{(campaign.state === 'unknown' || campaign.state === 'reconciling') && <small role="status">平台结果尚未确认，暂不允许暂停、恢复或重试；请刷新状态或进行人工核对。</small>}</div><div className="button-row compact">
      {availability.canResume && <button className="secondary" onClick={event => open('resume', event.currentTarget)}><PlayCircle size={16}/>恢复</button>}
      {availability.canPause && <button className="danger-action" onClick={event => open('pause', event.currentTarget)}><PauseCircle size={16}/>暂停</button>}
      {availability.canRetryFailed && <button className="secondary" onClick={event => open('retry_failed', event.currentTarget)} disabled={!failed.length}><RotateCcw size={16}/>重试失败项 ({failed.length})</button>}
      <button className="text-button" onClick={() => void load(campaign.id)} disabled={loading}>刷新状态版本</button>
    </div></div>}
