...
export function resolveDataConsistency(input: {
  ...
  const canonicalUnknown = input.productsLoaded && input.productCount > 0 && canonicalStatuses.length === 0
  ...
  return [
    ...
    { id: 'products', label: '商品', status: canonicalUnverified > 0 ? 'amber' : canonicalUnknown ? 'neutral' : canonicalKnown ? 'green' : 'amber', statusLabel: canonicalUnverified > 0 ? '待标准链核验' : canonicalUnknown ? '标准链待返回' : canonicalKnown ? '标准链已验证' : undefined, detail: !input.productsLoaded ? '商品列表尚未读取' : input.productCount === 0 ? '当前范围没有商品' : canonicalUnverified > 0 ? `${canonicalUnverified} 个商品的标准链尚未验证` : canonicalUnknown ? `${input.productCount} 个当前范围商品，服务端尚未返回标准链状态` : `${input.productCount} 个当前范围商品，标准链已验证`, nextStep: canonicalUnverified > 0 ? '打开商品关系并完成标准链核验' : canonicalUnknown ? '重新检查标准链状态' : '选择要处理的商品' },
