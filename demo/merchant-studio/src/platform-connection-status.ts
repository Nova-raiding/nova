...
export interface MerchantConnectionPresentation {
  status: '可读取' | '演示连接' | '需要重新授权' | '已撤销' | '仅有账号记录' | '人工登记（未授权）' | '平台暂未配置' | '连接状态待确认'
...
export function isRealReadableStore(connection?: { readable?: boolean; state?: string; dataMode?: string }): boolean {
  ...
  return state === 'connected' && dataMode !== 'fixture' && dataMode !== 'account_record_only' && dataMode !== 'manual_upload'
}
...
export function merchantConnectionPresentation(account: Pick<PlatformAccount, 'state' | 'readEnabled' | 'dataMode' | 'authorization'>): MerchantConnectionPresentation {
  ...
  if (state === 'manually_registered' || state === 'manual_operations' || account.dataMode === 'account_record_only' || account.dataMode === 'manual_upload') return { status: '人工登记（未授权）', tone: 'amber', sync: '不可读取', canSync: false, canReauthorize: false }
