...
export const MCP_METHOD_CONTRACTS: readonly McpMethodContract[] = [
  ...
  },
  {
    ...
    description: 'Create an in-memory batch campaign plan for up to 50 products in one brand and store; it does not generate or publish content.',
  ...
  },
  {
    ...
    description: 'Read an in-memory batch campaign plan and its product scope.',
    params: params({ campaign_id: { type: 'string' } }, ['campaign_id']),
  },
  {
    method: 'campaign.batch.generate',
    description: '为批量计划创建逐商品内容任务；每个任务仍需事实确认、规则审核和人工批准后才能发布。',
  ...
  { method: 'ops.commercial.rollout.upsert', description: 'Create or update a percentage or workspace-targeted offer rollout.', params: params({ offer_code: { type: 'string' }, workspace_id: { type: 'string' }, percentage: { type: 'string' }, enabled: { type: 'string', enum: ['true', 'false'] }, reason: { type: 'string' }, expected_revision: { type: 'string' } }, ['offer_code', 'percentage', 'reason']) },
  ...
  { method: 'billing.model-usage.resolve', description: 'Apply an optimistic-concurrency operations decision to one model usage settlement with an authenticated actor and audit reason.', params: params({ usage_id: { type: 'string' }, revision: { type: 'string' }, decision: { type: 'string', enum: ['retry', 'waive', 'manual_attention'] }, reason: { type: 'string' }, evidence_ref: { type: 'string' } }, ['usage_id', 'revision', 'decision', 'reason']) },
