...
export const MCP_METHOD_CONTRACTS: readonly McpMethodContract[] = [
  ...
  },
  {
    ...
    description: 'Record the authenticated customer acceptance of commercial.service-boundary.v1 (checksum 94cd78089cf960d4b556ed9990fcd653c03473bd143b94d0203ab978ea84d685). Included: system guidance, normal troubleshooting, brand configuration, generation guidance, rejection analysis, workflow optimization. Excluded: unlimited revisions, full production, full marketing strategy, daily operations, 24x7/non-business emergency, internal development. Outcomes are not guaranteed. Customer identity is derived from the authenticated session.',
    params: params({ policy_version: boundedString(128), policy_checksum: boundedString(128), acceptance_ref: boundedString(256), accepted_at: boundedString(64), idempotency_key: idempotencyKeyProperty }, ['policy_version', 'policy_checksum', 'acceptance_ref', 'accepted_at', 'idempotency_key']),
  },
  {
  ...
  },
  {
    ...
    description: 'Return invitations addressed to the current signed-in user. Read-only.',
    params: params({}),
  },
  {
    method: 'workspace.invitation.accept',
    description: 'Accept one invitation addressed to the current signed-in user and activate the workspace membership.',
    params: params({ expected_revision: positiveIntegerString, reason: { type: 'string' } }, ['expected_revision']),
  },
  {
  ...
  { method: 'ops.members.list', description: 'List workspace members and roles without exposing credentials.', params: params({ workspace_id: boundedString(256), offset: { type: 'string' }, limit: pageLimit100 }) },
  ...
  { method: 'ops.customer-delivery.list', description: 'List one bounded, server-filtered page of customer delivery records in the authorized workspace.', params: params({ target_workspace_id: boundedString(200, 1), query: boundedString(200), project_owner: boundedString(200), support_owner: boundedString(200), offset: nonNegativeIntegerString, limit: pageLimit100 }, ['target_workspace_id']) },
  ...
  { method: 'ops.customer-delivery.update', description: 'Update customer delivery profile fields with optimistic revision. Derived integration/acceptance summaries and training fields are not accepted; use checklist item or training endpoints. archivedAt archives or restores the record. Paid status requires scanned payment evidence.', params: params({ target_workspace_id: boundedString(200, 1), delivery_id: boundedString(256), expected_revision: positiveIntegerString, patch_json: { ...jsonObject('Profile JSON object accepting only companyName, contractNumber, paymentStatus, contractRef, projectOwner, supportOwner, paymentDate, paymentEvidenceRefs, plannedGoLiveAt, customerProfileStatus and archivedAt. contractRef is an uploaded contract asset reference or null, never an external URL. paymentEvidenceRefs is an array of uploaded payment asset references; server ownership, purpose and clean-scan checks remain required.'), maxLength: 16_384 } }, ['target_workspace_id', 'delivery_id', 'expected_revision', 'patch_json']) },
  { method: 'ops.customer-delivery.checklist.update', description: 'Exactly one of completed or items_json must be supplied. Scalar completed (true or false) is allowed only for customer_profile. System integration and functional acceptance use items_json or checklist-item.update. Operator confirmation is sufficient; evidence.asset_refs is optional audit context, and any supplied references must be bound to this delivery and purpose with a trusted clean scan.', params: params({ target_workspace_id: boundedString(200, 1), delivery_id: boundedString(256), checklist_key: { type: 'string', enum: ['customer_profile', 'system_integration', 'functional_acceptance'] }, completed: { ...booleanString, description: 'Scalar profile completion only; system_integration and functional_acceptance reject both true and false.' }, items_json: { ...jsonArray('System integration or functional acceptance items array. Each item has itemKey, completed and optional evidence. Optional evidence.asset_refs must contain scanned uploaded asset references.'), maxLength: 16_384 }, expected_revision: positiveIntegerString }, ['target_workspace_id', 'delivery_id', 'checklist_key', 'expected_revision'], ['completed', 'items_json'], [['completed', 'items_json']]) },
  ...
  { method: 'ops.customer-delivery.checklist-item.update', description: 'Update one persisted checklist item with optimistic revision. Operator confirmation is sufficient. evidence_json.asset_refs is optional audit context; each supplied reference must belong to this delivery and checklist purpose and have a trusted clean scan.', params: params({ target_workspace_id: boundedString(200, 1), delivery_id: boundedString(256), checklist_key: { type: 'string', enum: ['system_integration', 'functional_acceptance'] }, item_key: boundedString(256, 1), completed: booleanString, evidence_json: { ...jsonObject('Optional checklist audit evidence. When asset_refs is supplied, it must contain scanned uploaded asset references.'), maxLength: 16_384 }, expected_revision: positiveIntegerString }, ['target_workspace_id', 'delivery_id', 'checklist_key', 'item_key', 'completed', 'expected_revision']) },
  { method: 'ops.customer-delivery.training.complete', description: 'Independently update the manually confirmed customer training status with optimistic revision. evidence_refs_json remains a strict JSON array for backward compatibility, but may be empty because training proof is not required.', params: params({ target_workspace_id: boundedString(200, 1), delivery_id: boundedString(256), completed: booleanString, evidence_refs_json: { ...jsonArray('Optional legacy training asset references. Send an empty array when no proof is recorded.'), maxLength: 16_384 }, expected_revision: positiveIntegerString }, ['target_workspace_id', 'delivery_id', 'completed', 'evidence_refs_json', 'expected_revision']) },
  ...
  { method: 'ops.commercial.catalog-v2.mutate', description: 'Append a versioned commercial catalog lifecycle transition; facts are immutable and every mutation is audited.', params: params({ action: { type: 'string', enum: ['create', 'approve', 'publish', 'retire'] }, code: boundedString(128), kind: { type: 'string', enum: ['onboarding', 'monthly', 'point_pack', 'private_trial'] }, visibility: { type: 'string', enum: ['public', 'private'] }, required_capability: boundedString(128), price_fen: { type: 'string', pattern: '^(0|[1-9][0-9]*)$' }, price_mode: { type: 'string', enum: ['fixed', 'starts_at', 'custom'] }, duration_days: { type: 'string', pattern: '^[1-9][0-9]*$' }, payload_json: boundedString(33_000), benefits_json: boundedString(33_000), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['action', 'code', 'idempotency_key', 'reason', 'evidence_json']) },
  ...
  { method: 'ops.commercial.private-trial.invite.create', description: 'Create a private trial invite; the raw code is returned once and can be redeemed by the matching customer.', params: params({ target_workspace_id: boundedString(200), customer_ref: boundedString(256), expires_at: boundedString(64), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['target_workspace_id', 'customer_ref', 'expires_at', 'idempotency_key', 'reason', 'evidence_json']) },
  { method: 'ops.commercial.private-trial.invite.list', description: 'List private trial invites without returning raw invite codes.', params: params({ target_workspace_id: boundedString(200), limit: positiveIntegerString }, ['target_workspace_id']) },
  { method: 'ops.commercial.private-trial.invite.revoke', description: 'Revoke an unused private trial invite with immutable evidence.', params: params({ target_workspace_id: boundedString(200), invite_id: boundedString(256), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['target_workspace_id', 'invite_id', 'idempotency_key', 'reason', 'evidence_json']) },
  { method: 'ops.commercial.private-trial.eligibility.create', description: 'Create an auditable private 1999 CNY trial eligibility from a matching private invite; it does not create an order or grant access.', params: params({ target_workspace_id: boundedString(200), customer_ref: boundedString(256), invite_code: boundedString(256), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['target_workspace_id', 'customer_ref', 'invite_code', 'idempotency_key', 'reason', 'evidence_json']) },
  { method: 'ops.commercial.private-trial.order.create', description: 'Create the private 1999 CNY trial order from an approved, unexpired eligibility; the order remains pending until payment is verified.', params: params({ target_workspace_id: boundedString(200), eligibility_id: boundedString(256), idempotency_key: idempotencyKeyProperty, reason: reasonProperty }, ['target_workspace_id', 'eligibility_id', 'idempotency_key', 'reason']) },
  { method: 'ops.commercial.private-trial.trial-payment.verify', description: 'Verify a 1999 CNY private-trial payment and atomically grant its seven-day 1-brand, 1-store and 500-point entitlement.', params: params({ target_workspace_id: boundedString(200), order_id: boundedString(256), provider_event_id: boundedString(256), provider_order_id: boundedString(256), payment_subject_ref: boundedString(256), nonce: boundedString(256), payload_hash: boundedString(128), paid_at: boundedString(64), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['target_workspace_id', 'order_id', 'provider_event_id', 'provider_order_id', 'payment_subject_ref', 'nonce', 'payload_hash', 'paid_at', 'idempotency_key', 'reason', 'evidence_json']) },
  ...
  },
  {
    ...
    params: params({ channel: { type: 'string', enum: ['alipay'] }, amount_cny: { type: 'string' }, idempotency_key: { type: 'string' } }, ['channel', 'amount_cny']),
  },
  {
  ...
  },
  {
    ...
    description: 'Search catalog products. Store scope requires platform + account_id; workspace aggregation must be explicit with scope=workspace. The result also includes per-product next actions for store binding or fact confirmation. Set include_knowledge=true to attach bounded, approved workspace rules/assets plus persistent product/SKU knowledge documents that are ready, approved, and rights-cleared for the selected product context.',
    params: params({ scope: { type: 'string', enum: ['store', 'workspace'], description: '默认必须明确店铺；只有明确 scope=workspace 才允许全部店铺只读聚合。' }, query: { type: 'string' }, platform: platformProperty, account_id: { type: 'string' }, store_name: { type: 'string' }, brand_name: { type: 'string' }, sku_id: { type: 'string' }, remote_product_id: { type: 'string' }, listing_status: { type: 'string', enum: ['on_sale', 'off_sale', 'draft', 'unknown'] }, product_state: { type: 'string', enum: ['active', 'disabled'] }, sync_status: { type: 'string', enum: ['queued', 'running', 'succeeded', 'partial', 'failed'] }, date_from: { type: 'string' }, date_to: { type: 'string' }, include_knowledge: { ...booleanString, description: '返回当前工作区适用于商品平台/类目/店铺的、已批准且权益已确认的有界知识上下文。' }, limit: pageLimit100, offset: nonNegativeIntegerString }),
  },
  {
  ...
  { method: 'automation.scan', description: '执行店铺健康扫描并记录风险告警，返回同步、库存、规则、驳回和未知发布风险，并生成带官方入口与交互确认边界的结构化优化建议。', params: params({ platform: { type: 'string' }, account_id: { type: 'string' } }) },
...
function validCustomerDeliveryContractRef(value: unknown): boolean {
  ...
  if (!ref) return false
  // Match the API/persistence contract-reference format. External URLs cannot
  // establish upload ownership, delivery binding, or a trusted scanner verdict.
