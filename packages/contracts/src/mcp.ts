...
export const MCP_METHOD_CONTRACTS: readonly McpMethodContract[] = [
  ...
  { method: 'ops.commercial.catalog-v2.mutate', description: 'Append a versioned commercial catalog draft or retire an existing SKU; facts are immutable and every mutation is audited.', params: params({ action: { type: 'string', enum: ['create', 'retire'] }, code: boundedString(128), kind: { type: 'string', enum: ['onboarding', 'monthly', 'point_pack', 'private_trial'] }, visibility: { type: 'string', enum: ['public', 'private'] }, required_capability: boundedString(128), price_fen: { type: 'string', pattern: '^(0|[1-9][0-9]*)$' }, price_mode: { type: 'string', enum: ['fixed', 'starts_at', 'custom'] }, duration_days: { type: 'string', pattern: '^[1-9][0-9]*$' }, payload_json: boundedString(33_000), benefits_json: boundedString(33_000), idempotency_key: idempotencyKeyProperty, reason: reasonProperty, evidence_json: boundedString(33_000) }, ['action', 'code', 'idempotency_key', 'reason', 'evidence_json']) },
