...
const POLICY_GROUPS: readonly PolicyGroup[] = [
  ...
  read('customer.content.read', 'workspace', 'customer_content', ['brand-unit.list', 'brand-unit.listing.list', 'canonical.product.consistency', 'campaign.batch.list', 'campaign.batch.get', 'catalog.search', 'catalog.categories', 'rule.list', 'rule.sync.status', 'rule.history', 'asset.list', 'brand.get', 'catalog.sync.get', 'deliverable.list', 'task.history', 'feedback.list', 'creative.directions', 'publish.batch.get', 'publish.get', 'knowledge.rule.list', 'knowledge.asset.list', 'knowledge.learning.list', 'knowledge.competitor.list', 'multimodal.video.get']),
  read('customer.content.read', 'brand', 'customer_content', ['catalog.image.get', 'generation.get', 'task.timeline', 'content.versions', 'content.diff', 'creative.brief', 'creative.preview']),
