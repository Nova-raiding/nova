...
describe('authorization policy registry', () => {
  ...
  it('classifies ops console methods with explicit capability, scope, data and audit metadata', () => {
    ...
    expect(getMcpMethodPolicy('publish.manual.get')).toMatchObject({ capability: 'customer.content.read', scope: 'brand', dataClass: 'customer_content', effect: 'read' })
    expect(getMcpMethodPolicy('publish.manual.list')).toMatchObject({ capability: 'customer.content.read', scope: 'workspace', dataClass: 'customer_content', effect: 'read' })
    expect(getMcpMethodPolicy('ops.marketing.publish.manual-evidence.record')).toMatchObject({ capability: 'marketing.queue.update', scope: 'workspace', dataClass: 'customer_content', effect: 'write', audit: 'allow_and_deny', obligations: ['reason', 'revision', 'idempotency'] })
    ...
    expect(getMcpMethodPolicy('rule.sync.now')).toMatchObject({ capability: 'rule.update', scope: 'platform', effect: 'write' })
