...
export class OpenAICompatibleImageGenerator {
    options;
    fetchImpl;
    constructor(options) {
        this.options = options;
        if (!options.baseUrl.trim() || !options.apiKey.trim() || !options.model.trim())
            throw new Error('image provider URL, API key and model are required');
    ...
    async generate(input) {
        ...
        try {
            ...
            const payload = JSON.parse(await readBoundedResponseText(response, MAX_IMAGE_RELAY_RESPONSE_BYTES, 'image provider response'));
            await emitRelayUsage(this.options.usageSink, payload, response.headers, { modality: 'image', model: this.options.model, context: input.usageContext });
            if (!record(payload) || !Array.isArray(payload.data))
                throw new Error('image provider response does not contain data');
            const images = payload.data.flatMap(item => {
                if (!record(item))
                    return [];
                if (typeof item.url === 'string' && /^https:\/\//u.test(item.url))
                    return [item.url];
            ...
            if (images.length !== input.count)
                throw new Error('image provider returned an incomplete result');
            return images;
        }
        finally {
...
export function createImageGeneratorFromEnv(source = process.env, usageSink) {
    ...
    if (!relayUrl || !apiKey || !model)
        return undefined;
    if (!/^https:\/\//u.test(relayUrl) || inspectOutboundUrl(relayUrl, { environment: source.NODE_ENV, resolveDns: false }))
        return undefined;
    const responseFormat = source.IMAGE_RESPONSE_FORMAT === 'url' ? 'url' : 'b64_json';
    const outputFormat = ['png', 'jpeg', 'webp'].includes(source.IMAGE_OUTPUT_FORMAT ?? '') ? source.IMAGE_OUTPUT_FORMAT : undefined;
