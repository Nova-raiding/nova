    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        try { return validate(content) } catch (error) {
          ...
          const repairMessage = `上一个 JSON 未通过结构校验：${error.message.slice(0, REPAIR_DIAGNOSTIC_MAX_CHARS)}。只修复结构和缺失字段，不增加任何未确认商品事实；依据最初输入重新返回完整 JSON。模块 factSourceIds 或 claim.factSourceIds 为空时必须删除整个模块，不得编造 source ID。evidence.type 仅允许 real_image、parameter、test_report、comparison、usage_result、manual_review；evidence.status 仅允许 verified、missing、expired、conflict；claim.skuIds 非空时 referencedSkuIds 必须包含相同 SKU ID。不要复述上一份响应。`
