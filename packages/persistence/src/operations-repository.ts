...
type OperationAuditRow = Omit<OperationAudit, 'createdAt'> & { createdAt: Date | string }
function operationAuditFromRow(row: OperationAuditRow): OperationAudit {
  // pg returns timestamptz as Date; callers sort these values before JSON serialization.
...
export class PostgresOperationsRepository implements OperationsRepository {
  ...
  async append(input: Omit<OperationAudit, 'id' | 'createdAt'>) { validateOperationAuditInput(input); requireWorkspaceScope(input.workspaceId); return withWorkspaceTransaction(this.pool, input.workspaceId, async client => { const result = await client.query<OperationAuditRow>(`INSERT INTO workspace_operation_audit (id, workspace_id, actor_id, action, resource_type, resource_id, before_json, after_json, reason) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, workspace_id AS "workspaceId", actor_id AS "actorId", action, resource_type AS "resourceType", resource_id AS "resourceId", before_json AS "before", after_json AS "after", reason, created_at AS "createdAt"`, [randomUUID(), input.workspaceId, input.actorId, input.action, input.resourceType, input.resourceId, input.before, input.after, input.reason]); return operationAuditFromRow(result.rows[0]!) }) }
