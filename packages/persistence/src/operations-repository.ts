...
export class PostgresOperationsRepository implements OperationsRepository {
  ...
  async find(workspaceId: string, action: string, resourceType: string, resourceId: string) { requireWorkspaceScope(workspaceId); return withWorkspaceTransaction(this.pool, workspaceId, async client => { const result = await client.query<OperationAudit>(`SELECT id, workspace_id AS "workspaceId", actor_id AS "actorId", action, resource_type AS "resourceType", resource_id AS "resourceId", before_json AS "before", after_json AS "after", reason, created_at AS "createdAt" FROM workspace_operation_audit WHERE workspace_id=$1 AND action=$2 AND resource_type=$3 AND resource_id=$4 ORDER BY created_at DESC,id DESC LIMIT 1`, [workspaceId, action, resourceType, resourceId]); return result.rows[0] }) }
}
