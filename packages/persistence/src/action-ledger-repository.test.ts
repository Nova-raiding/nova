...
describe('MemoryActionLedgerRepository', () => {
  it('keeps settlement idempotent and records refund state', async () => {
    ...
    const replay = await repository.record({ workspaceId: 'ws_action', actionKey: 'model:one', actionKind: 'model_text', settlement: 'wallet', units: 1, amountFen: 1, actorId: 'other', description: '不应覆盖' })
...
describe('PostgresActionLedgerRepository', () => {
  it('records and maps provider authorization metadata and supports get', async () => {
    ...
    await expect(repository.record({ workspaceId: 'ws_action', actionKey: 'model:receipt', actionKind: 'model_text', settlement: 'wallet_overage', units: 1, amountFen: 1, actorId: 'merchant', description: '模型预扣', taskId: 'task_1', providerRequestId: 'authorization_1', reservedAmountFen: 1, multiplier: 2.5, settlementStatus: 'authorized' })).resolves.toMatchObject({ taskId: 'task_1', providerRequestId: 'authorization_1', reservedAmountFen: 1, multiplier: 2.5, settlementStatus: 'authorized' })
    const insert = client.calls.find(call => call.text.includes('INSERT INTO action_ledger'))
    expect(insert?.values?.slice(10)).toEqual(['task_1', 'authorization_1', 1, 2.5, 'authorized'])

    const getClient = new RecordingClient()
    getClient.enqueue(); getClient.enqueue(); getClient.enqueue([postgresRow()]); getClient.enqueue()
    await expect(new PostgresActionLedgerRepository(new RecordingPool(getClient)).get('ws_action', 'model:receipt')).resolves.toMatchObject({ taskId: 'task_1', reservedAmountFen: 1, multiplier: 2.5 })
  })
