...
export class MemoryCanonicalBackfillRunRepository implements CanonicalBackfillRunRepository {
  ...
  async update(input: UpdateCanonicalBackfillRun) { const workspaceId = requireWorkspaceScope(input.workspaceId); const run = this.rows.get(required(input.id, 'CANONICAL_BACKFILL_RUN_ID_REQUIRED')); if (!run || run.workspaceId !== workspaceId) throw new Error('CANONICAL_BACKFILL_RUN_NOT_FOUND'); if (run.revision !== input.expectedRevision) throw new CanonicalBackfillRunRevisionConflictError(); if (input.status !== undefined) { validateTransition(run.status, input.status); canRetryFailedRun(run, input.status) } const next: CanonicalBackfillRun = { ...run, ...(input.status === undefined ? {} : { status: input.status }), ...(input.lastResult === undefined ? {} : { lastResult: clone(input.lastResult) }), revision: run.revision + 1, updatedAt: new Date().toISOString() }; if (input.cursorProductId !== undefined) next.cursorProductId = input.cursorProductId; else if (shouldClearCursor(input)) delete next.cursorProductId; this.rows.set(run.id, next); return clone(next) }
}
export class PostgresCanonicalBackfillRunRepository implements CanonicalBackfillRunRepository {
  constructor(private readonly pool: SqlPool) {}
  async create(input: CreateCanonicalBackfillRun) { const value = validateCreate(input); return withWorkspaceTransaction(this.pool, value.workspaceId, async client => { const result = await client.query<Row>(`INSERT INTO canonical_backfill_runs (id,workspace_id,status,dry_run,batch_limit,created_by,reason) VALUES ($1,$2,'planned',$3,$4,$5,$6) RETURNING ${projection}`, [`backfill_${randomUUID()}`, value.workspaceId, value.dryRun, value.batchLimit ?? null, value.createdBy, value.reason]); return map(result.rows[0]!) }) }
