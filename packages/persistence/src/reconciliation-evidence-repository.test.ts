...
describe('reconciliation evidence repository', () => {
  ...
  it('writes and reads evidence through a workspace-scoped Postgres transaction', async () => {
    ...
    expect(client.calls).toContain('SELECT id,workspace_id,job_id,execution_attempt,provider_request_id,query_attempt,idempotency_key,provider_state,provider_status,response_digest,artifact_digest,usage_ledger_id,action_ledger_id,usage,cost,observed_at,next_attempt_at,error_code,error_message,created_at FROM reconciliation_evidence WHERE workspace_id=$1 AND idempotency_key=$2')
    expect(client.calls.some(query => query.includes('FOR UPDATE'))).toBe(false)
    expect(client.calls.some(query => query.includes('ON CONFLICT (workspace_id,idempotency_key) DO NOTHING'))).toBe(true)
  })
})
