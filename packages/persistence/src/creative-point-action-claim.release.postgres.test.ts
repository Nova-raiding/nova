...
describe('migration 251 enqueue ownership PostgreSQL', () => {
  postgresIt('binds a no-knowledge, no-brand first charged enqueue atomically and fences replay', async () => {
    ...
    try {
      ...
      const secondWorkspaceId = 'ws_enqueue_claim_other'
      await db.query("INSERT INTO workspaces(id,status) VALUES($1,'active')", [secondWorkspaceId])
      const secondWorkspaceAction = 'model:generation:other-workspace'
      await repo.claim({ workspaceId: secondWorkspaceId, actionKey: secondWorkspaceAction, intentSha256, leaseMs: 60_000 })
      const firstWorkspaceRows = await withWorkspaceTransaction(app, workspaceId, client => client.query<{ action_key: string }>(
        'SELECT action_key FROM creative_point_action_claims ORDER BY action_key',
      ))
      const secondWorkspaceRows = await withWorkspaceTransaction(app, secondWorkspaceId, client => client.query<{ action_key: string }>(
        'SELECT action_key FROM creative_point_action_claims ORDER BY action_key',
      ))
      expect(firstWorkspaceRows.rows).toEqual([{ action_key: actionKey }])
      expect(secondWorkspaceRows.rows).toEqual([{ action_key: secondWorkspaceAction }])
      ...
      // Simulate a successful database commit whose response never reaches the
      // caller. The API error path tries releaseUnbound(); the durable claim
      // must win that race and keep its exact job/event binding for recovery.
      await expect((async () => {
        ...
        throw new Error('simulated committed enqueue response loss')
      })()).rejects.toThrow('simulated committed enqueue response loss')
      await expect(repo.releaseUnbound(owner)).rejects.toMatchObject({ code: 'CREATIVE_ACTION_STALE_OWNER' })
      expect(await repo.get(claimInput)).toMatchObject({ phase: 'bound', reservationId: 'reservation-one', jobId: 'job-one', eventId: 'event-one' })
      expect((await db.query("SELECT id FROM outbox_events WHERE workspace_id=$1 AND aggregate_id='job-one' AND event_type='generation.requested'", [workspaceId])).rows).toEqual([{ id: 'event-one' }])
