...
describe('migration 250 runtime role replay', () => {
  postgresIt('keeps only the tenant role able to execute knowledge claims after post-migration bootstrap', async () => {
    ...
    try {
      ...
      const privileges = await database.query<{ app_claim: boolean; app_settle: boolean; ops_claim: boolean; ops_settle: boolean }>(`
      ...
      `, [claimSignature, settleSignature])
      expect(privileges.rows[0]).toEqual({ app_claim: true, app_settle: true, ops_claim: false, ops_settle: false })
