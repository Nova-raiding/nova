...
export interface WorkspaceBootstrapInput {
  ...
  displayName: string
  actorId: string
  /** Creation is reserved for explicitly opted-in local fixtures. */
  allowCreate?: boolean
}
...
export class WorkspaceBootstrapError extends Error {
  constructor(readonly code: 'WORKSPACE_BOOTSTRAP_IDENTITY_MISMATCH' | 'WORKSPACE_BOOTSTRAP_BINDING_INACTIVE' | 'WORKSPACE_ADMIN_ASSIGNMENT_REQUIRED', message = code) {
    super(message)
...
function normalized(input: WorkspaceBootstrapInput): WorkspaceBootstrapInput {
  ...
  if (!issuer || !externalSubject || (allowCreate && !candidateWorkspaceId) || !displayName || !actorId) throw new WorkspaceBootstrapError('WORKSPACE_BOOTSTRAP_IDENTITY_MISMATCH')
  return { ...input, issuer, externalSubject, ...(candidateWorkspaceId ? { candidateWorkspaceId } : {}), displayName, actorId, allowCreate, ...(input.identityId?.trim() ? { identityId: input.identityId.trim() } : {}) }
}
...
export class MemoryWorkspaceBootstrapRepository implements WorkspaceBootstrapRepository {
  ...
  private async bootstrapLocked(key: string, input: WorkspaceBootstrapInput): Promise<WorkspaceBootstrapResult> {
    ...
    if (!input.allowCreate) throw new WorkspaceBootstrapError('WORKSPACE_ADMIN_ASSIGNMENT_REQUIRED')
    const candidateWorkspaceId = input.candidateWorkspaceId!
    this.activateWorkspace(candidateWorkspaceId)
    const member = await this.members.upsert({ workspaceId: candidateWorkspaceId, externalSubject: input.externalSubject, displayName: input.displayName, role: 'workspace_owner', status: 'active', invitedBy: input.actorId })
    if (input.identityId) await this.members.bindIdentity({ workspaceId: candidateWorkspaceId, externalSubject: input.externalSubject, identityId: input.identityId })
    const result = { workspaceId: candidateWorkspaceId, displayName: input.displayName, created: true }
    this.bindings.set(key, { workspaceId: result.workspaceId, displayName: result.displayName })
    await this.operations.append({ workspaceId: result.workspaceId, actorId: input.actorId, action: 'workspace.bootstrap', resourceType: 'workspace', resourceId: result.workspaceId, before: {}, after: { workspaceId: result.workspaceId, displayName: result.displayName, owner: input.externalSubject, memberId: member.id, status: 'active' }, reason: '本地 fixture 创建工作区' })
    return result
  }
}
...
export class PostgresWorkspaceBootstrapRepository implements WorkspaceBootstrapRepository {
  ...
  async bootstrap(raw: WorkspaceBootstrapInput): Promise<WorkspaceBootstrapResult> {
    ...
    try {
      ...
      await client.query(`SELECT set_config('app.workspace_id', $1, true)`, [candidateWorkspaceId])
      await client.query(`INSERT INTO workspaces (id, status) VALUES ($1, 'active')`, [candidateWorkspaceId])
      const memberId = randomUUID()
      await client.query(`INSERT INTO workspace_members (id, workspace_id, external_subject, display_name, role, status, invited_by, identity_id) VALUES ($1,$2,$3,$4,'workspace_owner','active',$5,$6)`, [memberId, candidateWorkspaceId, input.externalSubject, input.displayName, input.actorId, input.identityId ?? null])
      await client.query(`INSERT INTO workspace_identity_bindings (issuer, external_subject, identity_id, workspace_id, display_name) VALUES ($1,$2,$3,$4,$5)`, [input.issuer, input.externalSubject, input.identityId ?? null, candidateWorkspaceId, input.displayName])
      await client.query(`INSERT INTO workspace_operation_audit (id, workspace_id, actor_id, action, resource_type, resource_id, before_json, after_json, reason) VALUES ($1,$2,$3,'workspace.bootstrap','workspace',$2,'{}'::jsonb,$4,'本地 fixture 创建工作区')`, [randomUUID(), candidateWorkspaceId, input.actorId, { workspaceId: candidateWorkspaceId, displayName: input.displayName, owner: input.externalSubject, memberId, status: 'active' }])
      await client.query('COMMIT')
      committed = true
      return { workspaceId: candidateWorkspaceId, displayName: input.displayName, created: true }
    } catch (error) {
