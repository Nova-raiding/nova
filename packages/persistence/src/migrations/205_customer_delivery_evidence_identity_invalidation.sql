-- Forward-only repair for migration 204. A clean asset is still different
-- evidence when its tenant, identity, source revision, receipt, object or MIME
-- identity changes. Invalidate every delivery referencing the old evidence,
-- including drafts whose effective_at is already null.
CREATE OR REPLACE FUNCTION public.invalidate_customer_delivery_effectiveness_for_asset()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog
AS $customer_delivery_evidence_identity_invalidation$
DECLARE
  delivery record;
...
BEGIN
  ...
  RETURN NULL;
END;
$customer_delivery_evidence_identity_invalidation$;
