...
export interface BrandUnitRepository {
  ...
  updateCampaignTasks(input: { workspaceId: string; id: string; taskIds: string[]; state: CampaignBatchRow['state']; generationIdempotencyKey?: string }): Promise<CampaignBatchRow>
...
export class MemoryBrandUnitRepository implements BrandUnitRepository {
  ...
  async updateCampaignTasks(input: { workspaceId: string; id: string; taskIds: string[]; state: CampaignBatchRow['state']; generationIdempotencyKey?: string }) { const row = this.campaigns.get(`${input.workspaceId}:${input.id}`); if (!row) throw new Error('CAMPAIGN_BATCH_NOT_FOUND'); if (productionCampaignStates.has(input.state)) requireCanonicalListingForProduction(row.items ?? []); const items = row.items?.map((item, index) => ({ ...item, ...(input.taskIds[index] ? { taskId: input.taskIds[index] } : {}), state: input.taskIds[index] ? 'generating' as const : item.state })); const updated = { ...row, ...(items ? { items } : {}), taskIds: [...input.taskIds], ...(input.generationIdempotencyKey ? { generationIdempotencyKey: input.generationIdempotencyKey } : {}), state: input.state, revision: (row.revision ?? 1) + 1, updatedAt: now() }; this.campaigns.set(`${input.workspaceId}:${input.id}`, updated); return updated }
...
export class PostgresBrandUnitRepository implements BrandUnitRepository {
  ...
  async createCampaign(input: Omit<CampaignBatchRow, 'createdAt' | 'updatedAt'>) {
    ...
    return withWorkspaceTransaction(this.pool, input.workspaceId, async client => {
      ...
      if (existing.rows[0]) {
        ...
        return { campaign: { id: row.id, workspaceId: row.workspaceId, brandId: row.data.brandId, platform: row.data.platform, accountId: row.data.accountId, productIds: row.data.productIds, ...(row.data.targets ? { targets: row.data.targets } : {}), ...(row.data.taskIds ? { taskIds: row.data.taskIds } : {}), ...(row.data.generationIdempotencyKey ? { generationIdempotencyKey: row.data.generationIdempotencyKey } : {}), idempotencyKey, state: row.state, revision: row.revision, manifestHash: row.manifestHash, createdAt: row.createdAt, updatedAt: row.updatedAt }, replayed: true }
      }
  ...
  async getCampaign(input: { workspaceId: string; id: string }) {
    ...
    return withWorkspaceTransaction(this.pool, input.workspaceId, async client => {
      ...
      const taskIds = items.rows.map(item => item.taskId).filter((id): id is string => Boolean(id))
      return { id: row.id, workspaceId: row.workspaceId, brandId: row.data.brandId, platform: row.data.platform, accountId: row.data.accountId, productIds: row.data.productIds, ...(row.data.targets ? { targets: row.data.targets } : {}), ...(taskIds.length ? { taskIds } : row.data.taskIds ? { taskIds: row.data.taskIds } : {}), ...(row.data.generationIdempotencyKey ? { generationIdempotencyKey: row.data.generationIdempotencyKey } : {}), items: items.rows, state: row.state, revision: row.revision, manifestHash: row.manifestHash, createdAt: row.createdAt, updatedAt: row.updatedAt }
    })
  }
  async updateCampaignTasks(input: { workspaceId: string; id: string; taskIds: string[]; state: CampaignBatchRow['state']; generationIdempotencyKey?: string }) {
    requireWorkspaceScope(input.workspaceId)
    return withWorkspaceTransaction(this.pool, input.workspaceId, async client => {
      ...
      const result = await client.query(`UPDATE batch_campaigns SET state=$3, data=jsonb_set(jsonb_set(data, '{taskIds}', $4::jsonb, true), '{generationIdempotencyKey}', to_jsonb($5::text), true), revision=revision+1, updated_at=now() WHERE workspace_id=$1 AND id=$2`, [input.workspaceId, input.id, input.state, JSON.stringify(input.taskIds), input.generationIdempotencyKey ?? `campaign-generate:${input.id}`])
      ...
      return { id: row.id, workspaceId: row.workspaceId, brandId: row.data.brandId, platform: row.data.platform, accountId: row.data.accountId, productIds: row.data.productIds, ...(row.data.targets ? { targets: row.data.targets } : {}), ...(row.data.taskIds ? { taskIds: row.data.taskIds } : {}), ...(row.data.generationIdempotencyKey ? { generationIdempotencyKey: row.data.generationIdempotencyKey } : {}), state: row.state, revision: row.revision, manifestHash: row.manifestHash, createdAt: row.createdAt, updatedAt: row.updatedAt }
    })
  }
  ...
  async transitionCampaignLifecycle(input: CampaignLifecycleTransitionInput) {
    requireWorkspaceScope(input.workspaceId)
    return withWorkspaceTransaction(this.pool, input.workspaceId, async client => {
      type LifecycleData = { brandId: string; platform: BrandUnitPlatform; accountId: string; productIds: string[]; targets?: CampaignTargetRow[]; taskIds?: string[]; generationIdempotencyKey?: string; _campaignLifecycle?: { pause?: { state: CampaignBatchState; items: CampaignItemRow[] }; operations?: Record<string, { fingerprint: string; campaign: CampaignBatchRow }> } }
    ...
    const campaign: CampaignBatchRow = { id: row.id, workspaceId: row.workspaceId, brandId: row.data.brandId, platform: row.data.platform, accountId: row.data.accountId, productIds: row.data.productIds, ...(row.data.targets ? { targets: row.data.targets } : {}), ...(row.data.taskIds ? { taskIds: row.data.taskIds } : {}), ...(row.data.generationIdempotencyKey ? { generationIdempotencyKey: row.data.generationIdempotencyKey } : {}), items, state, manifestHash: row.manifestHash, revision, createdAt: row.createdAt, updatedAt: timestamp }
