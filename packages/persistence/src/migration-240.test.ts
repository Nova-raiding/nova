...
describe('migration 242 creative point allocation reservation index repair', () => {
  it('restores the legacy-missing reservation lookup index without blocking writes', async () => {
    const migrations = await loadMigrations()
    const migration = migrations.find(item => item.version === 242)
    expect(migration).toMatchObject({
      version: 242,
      name: 'repair_creative_point_allocation_reservation_index',
      transactional: false,
    })
    expect(migrations.findIndex(item => item.version === 242)).toBe(migrations.findIndex(item => item.version === 241) + 1)

    const sql = await readFile(new URL('./migrations/242_repair_creative_point_allocation_reservation_index.sql', import.meta.url), 'utf8')
