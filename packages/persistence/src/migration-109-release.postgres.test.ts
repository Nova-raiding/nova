...
describe('migration 109 PostgreSQL asset scan redrive acceptance', () => {
  postgresIt('atomically redrives once, preserves old evidence, audits, isolates tenants, and rejects mutation', async () => {
    ...
    try {
      ...
      const input = { workspaceId: 'ws_redrive_a', assetId, deadLetterOutboxEventId: old.id, expectedAssetRevision: 1, recoveryKey: 'recovery-109-once', actorId: 'security-operator', reason: 'scanner recovered after incident', scanMaxAttempts: 12, authorizationSnapshot }
      ...
      expect(left.event).toMatchObject({ eventType: 'asset.scan_redrive_requested', sequence: 2, payload: { source_revision: 2, mime_type: 'image/png', recovery_from_outbox_event_id: old.id, recovery_key: input.recoveryKey, authorization_snapshot: authorizationSnapshot } })
      ...
      await expect(repository.redrive({ ...input, workspaceId: 'ws_redrive_b', authorizationSnapshot: { ...authorizationSnapshot, workspace_id: 'ws_redrive_b', context_id: 'workspace:ws_redrive_b' } })).rejects.toMatchObject({ code: 'ASSET_SCAN_REDRIVE_ASSET_NOT_FOUND' })
