...
export type ImageGenerationExecutionState = 'available' | 'leased' | 'provider_reserved' | 'provider_dispatching' | 'provider_started' | 'outcome_unknown' | 'completed' | 'failed'
...
export type ProviderReservedImageGenerationExecution = ImageGenerationExecution & { state: 'provider_reserved'; ownerToken: string; leaseExpiresAt: string; providerOperationKey: string }
...
export interface ImageGenerationExecutionRepository {
  ...
  reserveProviderOperation(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }): Promise<ProviderReservedImageGenerationExecution>
  beginProviderDispatch(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }): Promise<ProviderDispatchingImageGenerationExecution>
...
export class MemoryImageGenerationExecutionRepository implements ImageGenerationExecutionRepository {
  ...
  async reserveProviderOperation(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }) {
    const current = this.owned(input, 'leased'); const at = new Date(instant(input.now)).toISOString()
    const providerOperationKey = current.providerOperationKey ?? deterministicProviderOperationKey(current.workspaceId, current.jobId, current.eventId)
    const row = { ...current, state: 'provider_reserved' as const, providerOperationKey, updatedAt: at } as ProviderReservedImageGenerationExecution
    this.rows.set(keyOf(current.workspaceId, current.jobId), row)
    return row
  }

  async beginProviderDispatch(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }) {
    const current = this.owned(input, 'provider_reserved'); const at = new Date(instant(input.now)).toISOString()
    if (!current.providerOperationKey) throw new ImageGenerationExecutionError('IMAGE_GENERATION_EXECUTION_LEASE_LOST', current)
    const row = { ...current, state: 'provider_dispatching' as const, updatedAt: at } as ProviderDispatchingImageGenerationExecution
    this.rows.set(keyOf(current.workspaceId, current.jobId), row)
    return row
  }

  async markProviderStarted(input: { workspaceId: string; jobId: string; ownerToken: string; providerRequestId: string; now?: string }) {
    const current = this.owned(input, 'provider_dispatching'); const providerRequestId = normalizedId(input.providerRequestId, 'PROVIDER_REQUEST_ID'); const at = new Date(instant(input.now)).toISOString()
  ...
  private owned(input: { workspaceId: string; jobId: string; ownerToken: string }, state: 'leased' | 'provider_reserved' | 'provider_dispatching' | 'provider_started') {
  ...
  private terminal(input: { workspaceId: string; jobId: string; ownerToken: string; errorCode?: string; errorMessage?: string; now?: string }, state: 'outcome_unknown' | 'completed' | 'failed', allowedStates: Array<'provider_started' | 'provider_dispatching'> = ['provider_started']) {
    const current = allowedStates.map(candidate => { try { return this.owned(input, candidate) } catch { return undefined } }).find(Boolean) as ImageGenerationExecution | undefined
    if (!current) throw new ImageGenerationExecutionError('IMAGE_GENERATION_EXECUTION_LEASE_LOST', this.rows.get(keyOf(workspace(input.workspaceId), normalizedId(input.jobId, 'JOB_ID'))))
    const at = new Date(instant(input.now)).toISOString(); const error = state === 'completed' ? {} : { errorCode: normalizedId(input.errorCode ?? '', 'ERROR_CODE'), errorMessage: normalizedId(input.errorMessage ?? '', 'ERROR_MESSAGE') }
...
type ExecutionRow = { workspace_id: string; job_id: string; event_id: string; state: ImageGenerationExecutionState; attempt: number; owner_token: string | null; lease_expires_at: string | Date | null; provider_started_at: string | Date | null; provider_operation_key: string | null; provider_request_id: string | null; error_code: string | null; error_message: string | null; updated_at: string | Date }
const executionProjection = 'workspace_id,job_id,event_id,state,attempt,owner_token,lease_expires_at,provider_started_at,provider_operation_key,provider_request_id,error_code,error_message,updated_at'
const asIso = (value: string | Date) => value instanceof Date ? value.toISOString() : String(value)
const mapExecution = (row: ExecutionRow): ImageGenerationExecution => ({ workspaceId: row.workspace_id, jobId: row.job_id, eventId: row.event_id, state: row.state, attempt: row.attempt, ...(row.owner_token ? { ownerToken: row.owner_token } : {}), ...(row.lease_expires_at ? { leaseExpiresAt: asIso(row.lease_expires_at) } : {}), ...(row.provider_started_at ? { providerStartedAt: asIso(row.provider_started_at) } : {}), ...(row.provider_operation_key ? { providerOperationKey: row.provider_operation_key } : {}), ...(row.provider_request_id ? { providerRequestId: row.provider_request_id } : {}), ...(row.error_code ? { errorCode: row.error_code } : {}), ...(row.error_message ? { errorMessage: row.error_message } : {}), updatedAt: asIso(row.updated_at) })
...
export class PostgresImageGenerationExecutionRepository implements ImageGenerationExecutionRepository {
  ...
  async claim(input: { workspaceId: string; jobId: string; eventId: string; leaseMs: number; now?: string }) {
    ...
    const outcome = await withWorkspaceTransaction(this.pool, workspaceId, async client => {
      const result = await client.query<ExecutionRow>(`INSERT INTO image_generation_executions (workspace_id,job_id,event_id,state,attempt,owner_token,lease_expires_at,updated_at)
        ...
        ON CONFLICT (workspace_id,job_id) DO UPDATE SET state='leased',attempt=CASE WHEN image_generation_executions.provider_operation_key IS NULL THEN image_generation_executions.attempt+1 ELSE image_generation_executions.attempt END,event_id=EXCLUDED.event_id,owner_token=EXCLUDED.owner_token,lease_expires_at=EXCLUDED.lease_expires_at,provider_started_at=NULL,provider_request_id=NULL,error_code=NULL,error_message=NULL,updated_at=EXCLUDED.updated_at
        WHERE image_generation_executions.event_id=$3 AND image_generation_executions.provider_operation_key IS NULL AND (image_generation_executions.state='available' OR (image_generation_executions.state='leased' AND image_generation_executions.lease_expires_at <= $5))
  ...
  async reserveProviderOperation(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }) {
    const workspaceId = workspace(input.workspaceId); const jobId = normalizedId(input.jobId, 'JOB_ID'); const ownerToken = normalizedId(input.ownerToken, 'OWNER_TOKEN'); const now = new Date(instant(input.now)).toISOString()
    return withWorkspaceTransaction(this.pool, workspaceId, async client => {
      const operationKey = deterministicProviderOperationKey(workspaceId, jobId, (await this.getWithin(client, workspaceId, jobId))?.eventId ?? '')
      const result = await client.query<ExecutionRow>(`UPDATE image_generation_executions SET state='provider_reserved',provider_operation_key=COALESCE(provider_operation_key,$4),updated_at=$5 WHERE workspace_id=$1 AND job_id=$2 AND state='leased' AND owner_token=$3 RETURNING ${executionProjection}`, [workspaceId, jobId, ownerToken, operationKey, now])
      ...
      return mapExecution(result.rows[0]) as ProviderReservedImageGenerationExecution
    })
  }
  ...
  async beginProviderDispatch(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string }) {
    const workspaceId = workspace(input.workspaceId); const jobId = normalizedId(input.jobId, 'JOB_ID'); const ownerToken = normalizedId(input.ownerToken, 'OWNER_TOKEN'); const now = new Date(instant(input.now)).toISOString()
    return withWorkspaceTransaction(this.pool, workspaceId, async client => {
      const result = await client.query<ExecutionRow>(`UPDATE image_generation_executions SET state='provider_dispatching',updated_at=$4 WHERE workspace_id=$1 AND job_id=$2 AND state='provider_reserved' AND owner_token=$3 AND provider_operation_key IS NOT NULL RETURNING ${executionProjection}`, [workspaceId, jobId, ownerToken, now])
      if (!result.rows[0]) throw new ImageGenerationExecutionError('IMAGE_GENERATION_EXECUTION_LEASE_LOST', await this.getWithin(client, workspaceId, jobId))
      return mapExecution(result.rows[0]) as ProviderDispatchingImageGenerationExecution
    })
  }

  async markProviderStarted(input: { workspaceId: string; jobId: string; ownerToken: string; providerRequestId: string; now?: string }) { return this.transition(input, 'provider_dispatching', 'provider_started', input.providerRequestId) as Promise<ProviderStartedImageGenerationExecution> }
  ...
  private async transition(input: { workspaceId: string; jobId: string; ownerToken: string; now?: string; providerRequestId?: string; errorCode?: string; errorMessage?: string }, from: 'provider_dispatching' | 'provider_started' | Array<'provider_dispatching' | 'provider_started'>, to: ImageGenerationExecutionState, providerRequestId?: string, errorCode?: string, errorMessage?: string) {
...
const deterministicProviderOperationKey = (workspaceId: string, jobId: string, eventId: string) => `image_provider_operation_${createHash('sha256').update([workspaceId, jobId, eventId].join('\0'), 'utf8').digest('hex')}`
...
function stateError(execution?: ImageGenerationExecution): never {
  ...
  if (execution?.state === 'provider_started' || execution?.state === 'outcome_unknown' || execution?.state === 'provider_reserved' || execution?.state === 'provider_dispatching' || (execution?.state === 'leased' && execution.providerOperationKey)) throw new ImageGenerationExecutionError('IMAGE_GENERATION_PROVIDER_OUTCOME_UNKNOWN', execution)
