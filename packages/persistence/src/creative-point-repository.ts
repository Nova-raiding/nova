...
export interface CreativePointReserveMutation extends CreativePointMutation<CreativePointReservation> { replayed: boolean }
...
export interface CreativePointRepository {
  ...
  reserve(input: ReserveCreativePointsInput): Promise<CreativePointReserveMutation>
...
export type CreativePointErrorCode = 'CREATIVE_POINT_INPUT_INVALID' | 'CREATIVE_POINT_IDEMPOTENCY_CONFLICT' | 'CREATIVE_POINT_BALANCE_UNKNOWN' | 'CREATIVE_POINT_INSUFFICIENT' | 'CREATIVE_POINT_RESERVATION_NOT_FOUND' | 'CREATIVE_POINT_RESERVATION_FINALIZED' | 'CREATIVE_POINT_RESERVATION_TERMINAL' | 'CREATIVE_POINT_ALLOCATION_FAILED'
...
/**
 * A reserve idempotency key is only replayable while the hold it produced is
 * still consumable. Returning a released/settled reservation would let a
 * retried action run against a hold that no longer exists (the settlement path
 * refuses to charge a non-active reservation), so the replay fails loudly
 * instead of silently handing back a corpse or minting a second hold.
 */
function requireConsumableReplay(reservation: CreativePointReservation): CreativePointReservation {
  if (reservation.status !== 'active') throw new CreativePointRepositoryError('CREATIVE_POINT_RESERVATION_TERMINAL', `creative point reservation ${reservation.id} is already ${reservation.status}; reserve must not replay a finalized hold`)
  return reservation
}
...
export class MemoryCreativePointRepository implements CreativePointRepository {
  ...
  async reserve(input: ReserveCreativePointsInput): Promise<CreativePointReserveMutation> {
    ...
    // The stored replay snapshot keeps the status the hold had when it was
    // created, so the live record is the authority on whether it can still be
    // consumed.
    if (replay) return { ...replay, value: requireConsumableReplay(this.reservations.get(replay.value.id) ?? replay.value), replayed: true }
    if ([...this.reservations.values()].some(item => item.workspaceId === workspaceId && item.actionKey === input.actionKey)) throw new CreativePointRepositoryError('CREATIVE_POINT_IDEMPOTENCY_CONFLICT', 'action key was already reserved with another idempotency key')
