...
const requireAuthorizationInstant = (value: string) => {
  requireText(value, 1, 64)
  if (!Number.isFinite(Date.parse(value))) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
  return value
}
...
const authorizationRevisionFromSql = (value: string | number): number => {
  if (typeof value === 'string' && !/^\d+$/u.test(value)) throw new AuthorizationRepositoryError('AUTHORIZATION_REVISION_CONFLICT')
  const revision = Number(value)
  requireExpectedRevision(revision)
  return revision
}
const resourceScopeContainsResourceId = (scope: Record<string, unknown>, workspaceId: string, resourceId: string): boolean => {
  ...
  try { validateResourceScope(scope, workspaceId) } catch { return false }
  const [kind, ids] = Object.entries(scope).find(([key]) => key.endsWith('_ids'))!
  // The trusted worker entry point binds the real event/aggregate to this
  // workspace. Keep that aggregate ID in the reservation; a workspace grant
  // identifies the tenant boundary, not an aggregate whose ID is the workspace.
  return kind === 'workspace_ids' || (ids as string[]).includes(resourceId)
}
const validateResourceScope = (scope: Record<string, unknown>, workspaceId: string) => {
  if (!scope || typeof scope !== 'object' || Array.isArray(scope)) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
  const idEntries = Object.entries(scope).filter(([key]) => key.endsWith('_ids'))
  // A grant must describe one exact resource kind.  Accepting multiple *_ids
  // collections would turn a single resourceId into an ambiguous union scope.
  if (idEntries.length !== 1) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
  if (idEntries[0]![0] === 'workspace_ids') {
    const ids = idEntries[0]![1]
    if (!Array.isArray(ids) || ids.length !== 1 || ids[0] !== workspaceId || Object.hasOwn(scope, 'type') || Object.hasOwn(scope, 'ids')) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
  }
  for (const [key, value] of Object.entries(scope)) {
    requireText(key, 1, 255)
    if (key.endsWith('_ids')) {
      if (!Array.isArray(value) || value.length === 0 || value.length > 100 || value.some(id => !isSafeAuthorizationIdentifier(id) || id === '*')) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
      continue
    }
    if (typeof value !== 'string') throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID')
    requireText(value, 1, 255)
  }
}
const normalizeGrant = (input: IssueAuthorizationGrantInput, issuedAt: string) => {
  ...
  validateResourceScope(input.resourceScope, input.workspaceId)
...
export class MemoryAuthorizationRepository implements AuthorizationRepository {
  ...
  async getGrant(id: string, subject: string) { requireText(id, 1, 255); requireText(subject, 1, 255); const row = this.grants.get(id); return row?.subjectIdentityId === subject ? clone(row) : undefined }
  async listActivePlatformRoles(subject: string, at = this.clock().toISOString()) { requireText(subject, 1, 255); requireText(at, 1, 64); const time = Date.parse(at); if (!Number.isFinite(time)) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID'); return [...this.roles.values()].filter(row => row.subjectIdentityId === subject && !row.revokedAt && Date.parse(row.validFrom) <= time && (!row.expiresAt || Date.parse(row.expiresAt) > time)).map(clone) }
  ...
  async listActiveGrants(subject: string, workspace: string, at = this.clock().toISOString()) { requireText(subject, 1, 255); requireText(workspace, 1, 255); requireText(at, 1, 64); const time = Date.parse(at); if (!Number.isFinite(time)) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID'); return [...this.grants.values()].filter(row => row.subjectIdentityId === subject && row.workspaceId === workspace && !row.revokedAt && Date.parse(row.issuedAt) <= time && Date.parse(row.expiresAt) > time && row.useCount < row.maxUses).map(clone) }
  ...
  async consumeGrant(input: ConsumeAuthorizationGrantInput) {
    ...
    requireAuthorizationInstant(at)
  ...
  async reserveExecution(input: ReserveAuthorizationExecutionInput) {
    ...
    if (input.grantId !== undefined) {
      ...
      // queue admission.
      if (!grant || grant.subjectIdentityId !== input.subjectIdentityId || grant.workspaceId !== input.workspaceId || grant.scopeHash !== input.scopeHash || !resourceScopeContainsResourceId(grant.resourceScope, input.workspaceId, input.resourceId) || !grant.capabilities.includes(input.capability) || grant.revokedAt || grant.revision !== input.expectedGrantRevision || grant.authorizationRevision !== input.expectedAuthorizationRevision || grant.useCount < 1) return undefined
      const at = requireAuthorizationInstant(input.at ?? this.clock().toISOString())
...
const roleFromRow = (row: RoleRow): PlatformRoleAssignment => {
  ...
  return { ...rest, authorizationRevision: authorizationRevisionFromSql(row.authorizationRevision), validFrom: iso(row.validFrom), ...(optionalIso(expiresAt) ? { expiresAt: optionalIso(expiresAt) } : {}), ...(optionalIso(revokedAt) ? { revokedAt: optionalIso(revokedAt), revokedBy: revokedBy!, revocationReason: revocationReason! } : {}), createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) }
}
const grantFromRow = (row: GrantRow): AuthorizationGrant => {
  const { revokedAt, revokedBy, revocationReason, ...rest } = row
  return { ...rest, authorizationRevision: authorizationRevisionFromSql(row.authorizationRevision), approvedAt: iso(row.approvedAt), issuedAt: iso(row.issuedAt), expiresAt: iso(row.expiresAt), ...(optionalIso(revokedAt) ? { revokedAt: optionalIso(revokedAt), revokedBy: revokedBy!, revocationReason: revocationReason! } : {}), createdAt: iso(row.createdAt), updatedAt: iso(row.updatedAt) }
}
const authorizationExecutionReservationFromRow = (row: AuthorizationExecutionReservationRow): AuthorizationExecutionReservation => ({
  ...
  scopeHash: row.scopeHash, authorizationRevision: authorizationRevisionFromSql(row.authorizationRevision),
...
export class PostgresAuthorizationRepository implements AuthorizationRepository {
  ...
  private async bump(client: SqlClient, subject: string, expected: number, actor: string, reason: string) { requireExpectedRevision(expected); await client.query(`INSERT INTO authorization_revisions (subject_identity_id, revision, updated_by, update_reason) VALUES ($1,0,$2,$3) ON CONFLICT (subject_identity_id) DO NOTHING`, [subject, actor, reason]); const result = await client.query<{ revision: string | number }>(`UPDATE authorization_revisions SET revision=revision+1, updated_by=$3, update_reason=$4, updated_at=now() WHERE subject_identity_id=$1 AND revision=$2 RETURNING revision`, [subject, expected, actor, reason]); if (!result.rows[0]) throw new AuthorizationRepositoryError('AUTHORIZATION_REVISION_CONFLICT'); return authorizationRevisionFromSql(result.rows[0].revision) }
  async getAuthorizationRevision(subject: string) { return this.transaction(async client => authorizationRevisionFromSql((await client.query<{ revision: string | number }>('SELECT revision FROM authorization_revisions WHERE subject_identity_id=$1', [subject])).rows[0]?.revision ?? 0), true) }
  async getGrant(id: string, subject: string) { requireText(id, 1, 255); requireText(subject, 1, 255); return this.transaction(async client => { const row = (await client.query<GrantRow>(`SELECT ${grantProjection} FROM ops_access_grants WHERE id=$1 AND subject_identity_id=$2`, [id, subject])).rows[0]; return row ? grantFromRow(row) : undefined }, true) }
  async listActivePlatformRoles(subject: string, at = this.clock().toISOString()) { requireText(subject, 1, 255); requireText(at, 1, 64); if (!Number.isFinite(Date.parse(at))) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID'); return this.transaction(async client => (await client.query<RoleRow>(`SELECT ${roleProjection} FROM platform_role_assignments WHERE subject_identity_id=$1 AND revoked_at IS NULL AND valid_from <= $2 AND (expires_at IS NULL OR expires_at > $2) ORDER BY role,id`, [subject, at])).rows.map(roleFromRow), true) }
  ...
  async listActiveGrants(subject: string, workspace: string, at = this.clock().toISOString()) { requireText(subject, 1, 255); requireText(workspace, 1, 255); requireText(at, 1, 64); if (!Number.isFinite(Date.parse(at))) throw new AuthorizationRepositoryError('AUTHORIZATION_GRANT_INVALID'); return this.transaction(async client => (await client.query<GrantRow>(`SELECT ${grantProjection} FROM ops_access_grants WHERE subject_identity_id=$1 AND workspace_id=$2 AND revoked_at IS NULL AND issued_at <= $3 AND expires_at > $3 AND use_count < max_uses ORDER BY expires_at,id`, [subject, workspace, at])).rows.map(grantFromRow), true) }
  ...
  async reserveExecution(input: ReserveAuthorizationExecutionInput): Promise<AuthorizationExecutionReservation | undefined> {
    ...
    return this.transaction(async client => {
      ...
      const revisionRow = (await client.query<{ revision: string | number }>('SELECT revision FROM authorization_revisions WHERE subject_identity_id=$1 FOR UPDATE', [input.subjectIdentityId])).rows[0]
      const authorizationRevision = authorizationRevisionFromSql(revisionRow?.revision ?? 0)
      ...
      if (input.grantId !== undefined && (!grant || grant.workspaceId !== input.workspaceId || !resourceScopeContainsResourceId(grant.resourceScope, input.workspaceId, input.resourceId) || !grant.capabilities.includes(input.capability) || grant.scopeHash !== input.scopeHash || grant.revokedAt || Number(grant.revision) !== input.expectedGrantRevision || authorizationRevisionFromSql(grant.authorizationRevision) !== input.expectedAuthorizationRevision || Number(grant.useCount) < 1 || Date.parse(String(grant.issuedAt)) > Date.parse(at) || Date.parse(String(grant.expiresAt)) <= Date.parse(at))) return undefined
