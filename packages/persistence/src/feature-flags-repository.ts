...
type FeatureFlagCursor = [updatedAt: string, id: string, scope: string]
const listScope = (input: { environment?: string; query?: string }) => intentHash({ environment: input.environment ?? null, query: input.query?.trim().toLocaleLowerCase() ?? null })
const cursorOf = (row: StoredFeatureFlag, scope: string) => Buffer.from(JSON.stringify([row.updatedAt, row.id, scope])).toString('base64url')
const decodeCursor = (cursor?: string): FeatureFlagCursor | undefined => {
  if (!cursor) return undefined
  try {
    const value = JSON.parse(Buffer.from(cursor, 'base64url').toString('utf8'))
    if (!Array.isArray(value) || value.length !== 3 || value.some(item => typeof item !== 'string') || !Number.isFinite(Date.parse(value[0])) || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value[1]) || !/^[0-9a-f]{64}$/.test(value[2])) throw new Error()
    return value as FeatureFlagCursor
...
const isAfterCursor = (row: StoredFeatureFlag, cursor: FeatureFlagCursor) => row.updatedAt < cursor[0] || (row.updatedAt === cursor[0] && row.id < cursor[1])

const normalizeTargets = (targets: StoredFlagTarget[]) => {
  const seen = new Set<string>()
  return targets.map(target => {
    const normalized = target.type === 'percentage' ? { ...target, value: String(Number(target.value)) } : target
    const key = `${normalized.type}:${normalized.value}`
    if (seen.has(key)) throw new FeatureFlagRepositoryError('FEATURE_FLAG_TARGET_DUPLICATE')
    seen.add(key)
...
export class MemoryFeatureFlagsRepository implements FeatureFlagsRepository {
  ...
  async list(input: { environment?: string; query?: string; cursor?: string; limit?: number } = {}) {
    ...
    if (cursor && cursor[2] !== scope) throw new FeatureFlagRepositoryError('FEATURE_FLAG_CURSOR_SCOPE_MISMATCH')
    ...
    return { items: clone(items), ...(rows.length > limit ? { nextCursor: cursorOf(items[items.length - 1]!, scope) } : {}) }
  }
  ...
  async save(input: SaveFeatureFlagInput) {
    ...
    const hash = intentHash({ ...input, targets, idempotencyKey: undefined })
