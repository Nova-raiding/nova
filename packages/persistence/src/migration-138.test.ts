...
describe('migration 138 interactive confirmation ticket reservations', () => {
  it('adds an additive, tenant-scoped reservation lease with least-privilege updates', async () => {
    const sql = await readFile(new URL('./migrations/138_interactive_confirmation_ticket_reservations.sql', import.meta.url), 'utf8')
    ...
    expect((await loadMigrations()).at(-1)).toMatchObject({ version: 138, name: 'interactive_confirmation_ticket_reservations' })
  })
})
