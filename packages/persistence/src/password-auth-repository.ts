...
export class MemoryPasswordAuthRepository implements PasswordAuthRepository {
  ...
  async createMerchantAccount(input: { login: string; password: string; enterpriseName: string; contactName: string; workspaceIds: string[]; actorId: string; reason: string; bootstrapWorkspace?: boolean }) {
    ...
    if ((!input.workspaceIds.length && input.bootstrapWorkspace !== true) || (input.workspaceIds.length && input.bootstrapWorkspace === true) || !input.actorId.trim() || input.reason.trim().length < 4) throw Object.assign(new Error('ACCOUNT_PROVISIONING_INVALID'), { code: 'AUTH_ACCOUNT_PROVISIONING_INVALID' })
    if (this.accounts.has(login)) throw Object.assign(new Error('LOGIN_ALREADY_EXISTS'), { code: 'AUTH_LOGIN_ALREADY_EXISTS' })
    let passwordHash: string
    try { passwordHash = await hashPassword(input.password) } catch (error) { if ((error as Error).message === 'PASSWORD_POLICY_INVALID') throw Object.assign(new Error('PASSWORD_POLICY_INVALID'), { code: 'AUTH_PASSWORD_POLICY_INVALID' }); throw error }
    const now = new Date().toISOString()
  ...
  async bindBootstrappedWorkspace(input: { login: string; identityId: string; workspaceId: string }) {
    const account = this.accounts.get(normalizeLogin(input.login))
    if (!account || account.identityId !== input.identityId || account.accountType !== 'merchant' || account.status !== 'active' || (account.workspaceIds.length > 0 && (account.workspaceIds.length !== 1 || account.workspaceIds[0] !== input.workspaceId))) throw Object.assign(new Error('BOOTSTRAP_ACCOUNT_CHANGED'), { code: 'AUTH_BOOTSTRAP_ACCOUNT_CHANGED' })
    if (account.workspaceIds.length === 0) {
      account.workspaceIds = [input.workspaceId]
      account.revision += 1
  ...
  async refresh(token: string) {
    ...
    const previous = this.sessions.get(tokenHash)
    const account = previous ? [...this.accounts.values()].find(item => item.id === previous.account.id) : undefined
    if (!previous || previous.status !== 'active' || !account || account.status !== 'active' || account.authEpoch !== previous.authEpoch || Date.parse(previous.expiresAt) <= Date.now()) {
      if (previous) previous.status = 'revoked'
      throw Object.assign(new Error('AUTH_SESSION_INVALID'), { code: 'AUTH_SESSION_INVALID' })
    }
    // Claim the old session synchronously before any await so only one
    // concurrent refresh can mint a replacement.
    previous.status = 'revoked'
    auditMemory(this.events, 'auth.logout', account.id, { session_id: previous.sessionId, reason: 'session_refresh' })
    const raw = newOpaqueToken(); const now = Date.now(); const session: SessionRecord = { tokenHash: tokenDigest(raw), sessionId: randomUUID(), account: accountPublic(account), issuedAt: new Date(now).toISOString(), expiresAt: new Date(now + SESSION_MS).toISOString(), authEpoch: account.authEpoch, status: 'active' }
  ...
  async issueMcpAuthorizationCode(input: McpOAuthContext & { account: PasswordAccount; redirectUri: string; codeChallenge: string; workspaceId?: string }) {
    ...
    const workspaceId = input.workspaceId ?? (workspaceIds.length === 1 ? workspaceIds[0] : undefined)
    if (!workspaceId || !workspaceIds.includes(workspaceId)) throw mcpOAuthError('MCP_OAUTH_WORKSPACE_AMBIGUOUS')
    const code = newOpaqueToken()
    const expiresAt = Date.now() + MCP_CODE_MS
    this.mcpCodes.set(tokenDigest(code), { ...input, accountId: account.id, identityId: account.identityId, workspaceId, codeHash: tokenDigest(code), accountAuthEpoch: account.authEpoch, identityAuthEpoch: account.authEpoch, expiresAt, used: false })
    return { code, expiresAt: new Date(expiresAt).toISOString(), workspaceId }
  }
  ...
  async exchangeMcpAuthorizationCode(input: McpOAuthContext & { redirectUri: string; code: string; codeVerifier: string }) {
    ...
    if (!account || account.status !== 'active' || account.authEpoch !== record.accountAuthEpoch || !account.workspaceIds.includes(record.workspaceId)) throw mcpOAuthError('MCP_OAUTH_INVALID_GRANT')
    record.used = true
...
export class PostgresPasswordAuthRepository implements PasswordAuthRepository {
  ...
  async createMerchantAccount(input: { login: string; password: string; enterpriseName: string; contactName: string; workspaceIds: string[]; actorId: string; reason: string; bootstrapWorkspace?: boolean }) {
    ...
    if ((!workspaceIds.length && input.bootstrapWorkspace !== true) || (workspaceIds.length && input.bootstrapWorkspace === true) || !input.actorId.trim() || input.reason.trim().length < 4) throw Object.assign(new Error('ACCOUNT_PROVISIONING_INVALID'), { code: 'AUTH_ACCOUNT_PROVISIONING_INVALID' })
  ...
  async bindBootstrappedWorkspace(input: { login: string; identityId: string; workspaceId: string }) {
    const login = assertLogin(input.login)
    if (!input.identityId.trim() || !input.workspaceId.trim()) throw Object.assign(new Error('BOOTSTRAP_ACCOUNT_CHANGED'), { code: 'AUTH_BOOTSTRAP_ACCOUNT_CHANGED' })
    return this.withClient(async client => {
      // Serialize same-account bind/retry after the workspace bootstrap's own
      // identity lock. A competing request must observe the committed binding.
      ...
      if (current.workspaceIds.length === 0) {
        ...
        const updated = await client.query(`UPDATE platform_password_accounts SET workspace_ids=ARRAY[$3]::text[],revision=revision+1,updated_at=now() WHERE login_identifier=$1 AND identity_id=$2 AND account_type='merchant' AND status='active' AND cardinality(workspace_ids)=0 RETURNING id`, [login, input.identityId, input.workspaceId])
        if (updated.rowCount !== 1) throw Object.assign(new Error('BOOTSTRAP_ACCOUNT_CHANGED'), { code: 'AUTH_BOOTSTRAP_ACCOUNT_CHANGED' })
        await this.syncEnterpriseName(client, [input.workspaceId], current.enterpriseName ?? '')
  ...
  async login(input: { login: string; password: string; ip?: string; userAgent?: string }) { const login = assertLogin(input.login); const outcome = await this.withClient(async client => { const account = await this.find(client, login); const now = Date.now(); if (account?.lockedUntil && Date.parse(iso(account.lockedUntil)) > now) return { failure: Object.assign(new Error('AUTH_ACCOUNT_LOCKED'), { code: 'AUTH_ACCOUNT_LOCKED' }) } as const; const valid = account ? await verifyPassword(account.passwordHash, input.password) : false; if (account && valid && account.status !== 'active') { await client.query(`INSERT INTO platform_identity_events (id,identity_id,event_type,actor_id,reason,evidence_json) VALUES ($1,$2,'auth.login_blocked',$3,'password authentication blocked for inactive account',$4)`, [randomUUID(), account.identityId, account.login, { status: account.status }]); return { failure: Object.assign(new Error('AUTH_ACCOUNT_NOT_ACTIVE'), { code: 'AUTH_ACCOUNT_NOT_ACTIVE' }) } as const } if (!account || !valid) { if (account) { const incremented = await client.query<{ failed_attempts: number | string }>(`UPDATE platform_password_accounts SET failed_attempts=failed_attempts+1, locked_until=CASE WHEN failed_attempts+1>=5 THEN now()+interval '15 minutes' ELSE locked_until END, updated_at=now() WHERE id=$1 RETURNING failed_attempts`, [account.id]); const attempts = Number(incremented.rows[0]?.failed_attempts ?? account.failedAttempts + 1); await client.query(`INSERT INTO platform_identity_events (id,identity_id,event_type,actor_id,reason,evidence_json) VALUES ($1,$2,$3,$4,'password authentication failed',$5)`, [randomUUID(), account.identityId, attempts >= 5 ? 'auth.locked' : 'auth.login_failed', account.login, { attempts }]) } return { failure: loginError() } as const } await client.query(`UPDATE platform_password_accounts SET failed_attempts=0, locked_until=NULL, updated_at=now() WHERE id=$1`, [account.id]); const token = newOpaqueToken(); const issuedAt = new Date(now).toISOString(); const expiresAt = new Date(now + SESSION_MS).toISOString(); const sessionId = randomUUID(); await client.query(`INSERT INTO platform_password_sessions (id,account_id,token_hash,auth_epoch,issued_at,expires_at,ip_hash,user_agent_hash) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [sessionId, account.id, tokenDigest(token), account.authEpoch, issuedAt, expiresAt, input.ip ? tokenDigest(input.ip) : null, input.userAgent ? tokenDigest(input.userAgent) : null]); // platform_identity_events.session_id references OIDC/API sessions only; password sessions are kept in evidence_json.
    ...
    // The rejection is raised after the transaction commits. It used to be
    // raised inside it, so `withClient` rolled the bookkeeping back: the
    // failed-attempt counter never advanced, `locked_until` was never stamped
    // and no `auth.login_failed`/`auth.locked` event survived, which made
    // `AUTH_ACCOUNT_LOCKED` unreachable on PostgreSQL. The memory adapter
    // mutates its records in place, so only the in-memory tests saw a lockout.
  ...
  async refresh(token: string) {
    return this.withClient(async client => {
      const result = await client.query<any>(`SELECT a.*,i.auth_epoch AS "identityAuthEpoch",i.access_status AS "identityStatus",s.id AS "sessionId",s.status AS "sessionStatus",s.auth_epoch AS "sessionAuthEpoch",s.expires_at AS "sessionExpiresAt" FROM platform_password_sessions s JOIN platform_password_accounts a ON a.id=s.account_id JOIN platform_identities i ON i.id=a.identity_id WHERE s.token_hash=$1 FOR UPDATE OF s`, [tokenDigest(token)])
      const row = result.rows[0]
      const valid = row && row.sessionStatus === 'active' && row.status === 'active' && row.identityStatus === 'active' && Number(row.auth_epoch) === Number(row.sessionAuthEpoch) && Number(row.identityAuthEpoch) === Number(row.sessionAuthEpoch) && Date.parse(iso(row.sessionExpiresAt)) > Date.now()
      if (!valid) throw Object.assign(new Error('AUTH_SESSION_INVALID'), { code: 'AUTH_SESSION_INVALID' })
      const claimed = await client.query(`UPDATE platform_password_sessions SET status='revoked',revoked_at=now(),revoke_reason='session_refresh' WHERE id=$1 AND status='active'`, [row.sessionId])
      if (claimed.rowCount !== 1) throw Object.assign(new Error('AUTH_SESSION_INVALID'), { code: 'AUTH_SESSION_INVALID' })
      const account = await this.find(client, row.login_identifier)
      if (!account) throw Object.assign(new Error('AUTH_SESSION_INVALID'), { code: 'AUTH_SESSION_INVALID' })
      const raw = newOpaqueToken(); const issuedAt = new Date().toISOString(); const expiresAt = new Date(Date.now() + SESSION_MS).toISOString(); const sessionId = randomUUID()
      await client.query(`INSERT INTO platform_password_sessions (id,account_id,token_hash,auth_epoch,issued_at,expires_at) VALUES ($1,$2,$3,$4,$5,$6)`, [sessionId, account.id, tokenDigest(raw), account.authEpoch, issuedAt, expiresAt])
      await client.query(`INSERT INTO platform_identity_events (id,identity_id,event_type,actor_id,reason,evidence_json) VALUES ($1,$2,'auth.refresh',$3,'session refreshed',$4)`, [randomUUID(), account.identityId, account.login, { previous_session_id: row.sessionId, session_id: sessionId }])
      return { token: raw, principal: { account: this.public(account), sessionId, issuedAt, expiresAt } }
    })
  }
  ...
  async issueMcpAuthorizationCode(input: McpOAuthContext & { account: PasswordAccount; redirectUri: string; codeChallenge: string; workspaceId?: string }) {
    return this.withClient(async client => {
      ...
      const workspaceIds: string[] = Array.isArray(account.workspaceIds) ? account.workspaceIds : []
      const workspaceId = input.workspaceId ?? (workspaceIds.length === 1 ? workspaceIds[0] : undefined)
      if (workspaceId && !workspaceIds.includes(workspaceId)) throw mcpOAuthError('MCP_OAUTH_WORKSPACE_AMBIGUOUS')
