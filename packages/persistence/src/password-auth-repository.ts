...
export interface PasswordAuthRepository {
  ...
  activateMerchantAccount(input: { login: string; workspaceIds: string[] }): Promise<PasswordAccount>
}
...
export class MemoryPasswordAuthRepository implements PasswordAuthRepository {
  ...
  async activateMerchantAccount(input: { login: string; workspaceIds: string[] }) {
    const login = assertLogin(input.login)
...
export class PostgresPasswordAuthRepository implements PasswordAuthRepository {
  ...
  async activateMerchantAccount(input: { login: string; workspaceIds: string[] }) { const login = assertLogin(input.login); return this.withClient(async client => { const result = await client.query<any>(`UPDATE platform_password_accounts SET status='active', workspace_ids=$2, revision=revision+1, updated_at=now() WHERE login_identifier=$1 AND account_type='merchant' RETURNING id,login_identifier AS login,account_type AS "accountType",enterprise_name AS "enterpriseName",contact_name AS "contactName",status,roles,workspace_ids AS "workspaceIds",failed_attempts AS "failedAttempts",locked_until AS "lockedUntil",revision,created_at AS "createdAt",updated_at AS "updatedAt"`, [login, [...new Set(input.workspaceIds.map(value => value.trim()).filter(Boolean))]]); const account = result.rows[0] as PasswordAccount | undefined; if (!account) throw Object.assign(new Error('AUTH_ACCOUNT_NOT_FOUND'), { code: 'AUTH_ACCOUNT_NOT_FOUND' }); return account }) }
