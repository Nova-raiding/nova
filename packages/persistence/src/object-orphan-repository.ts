...
function isSafeObjectKey(objectKey: unknown, workspaceId: string): objectKey is string {
  if (typeof objectKey !== 'string' || !objectKey.trim() || objectKey.startsWith('/') || objectKey.includes('\\') || /[\u0000-\u001f\u007f\r\n]/u.test(objectKey)) return false
  const parts = objectKey.split('/')
  return parts.length >= 3 && (parts[0] === 'quarantine' || parts[0] === 'clean') && parts[1] === workspaceId && parts.every(part => part.length > 0 && part !== '.' && part !== '..')
}
function validateEnqueue(input: { workspaceId: string; objectKey: string; reason: string }) {
  const workspaceId = requireWorkspaceScope(input.workspaceId)
  if (!isSafeObjectKey(input.objectKey, workspaceId) || !input.reason.trim()) throw new Error('ORPHAN_INPUT_INVALID')
}
...
export class MemoryObjectOrphanRepository implements ObjectOrphanRepository {
  ...
  async enqueue(input: { workspaceId: string; objectKey: string; reason: string; lastError?: string }) {
    ...
    const row: ObjectStorageOrphan = current ? { ...current, reason: input.reason, ...(input.lastError ? { lastError: input.lastError } : {}), attempts: current.attempts + 1, state: 'pending', nextAttemptAt: timestamp, leaseToken: undefined, leaseUntil: undefined, updatedAt: timestamp } : { id: `orphan_${randomUUID()}`, ...input, state: 'pending', attempts: 1, nextAttemptAt: timestamp, createdAt: timestamp, updatedAt: timestamp }
...
const orphanProjection = `orphan.id, orphan.workspace_id AS "workspaceId", orphan.object_key AS "objectKey", orphan.reason, orphan.state, orphan.attempts, orphan.last_error AS "lastError", orphan.next_attempt_at AS "nextAttemptAt", orphan.created_at AS "createdAt", orphan.updated_at AS "updatedAt", orphan.lease_token AS "leaseToken", orphan.lease_until AS "leaseUntil"`
export class PostgresObjectOrphanRepository implements ObjectOrphanRepository {
  constructor(private readonly pool: SqlPool) {}
  async enqueue(input: { workspaceId: string; objectKey: string; reason: string; lastError?: string }) { validateEnqueue(input); return withWorkspaceTransaction(this.pool, input.workspaceId, async client => { const result = await client.query<ObjectStorageOrphan>(`INSERT INTO object_storage_orphans (id, workspace_id, object_key, reason, attempts, last_error) VALUES ($1,$2,$3,$4,1,$5) ON CONFLICT (workspace_id, object_key) DO UPDATE SET reason=EXCLUDED.reason, attempts=object_storage_orphans.attempts+1, last_error=EXCLUDED.last_error, state='pending', next_attempt_at=now(), lease_token=NULL, lease_until=NULL, updated_at=now() RETURNING ${projection}`, [randomUUID(), input.workspaceId, input.objectKey, input.reason, input.lastError ?? null]); return result.rows[0]! }) }
  ...
  async claimPending(workspaceId: string, options: ObjectOrphanClaimOptions = {}) { requireWorkspaceScope(workspaceId); const { limit, leaseMs } = optionsOf(options); const token = `orphan_lease_${randomUUID()}`; return withWorkspaceTransaction(this.pool, workspaceId, async client => (await client.query<ObjectStorageOrphan>(`WITH candidates AS (SELECT id FROM object_storage_orphans WHERE workspace_id=$1 AND state='pending' AND next_attempt_at<=COALESCE($2::timestamptz,now()) AND (lease_until IS NULL OR lease_until<=COALESCE($2::timestamptz,now())) ORDER BY next_attempt_at, created_at FOR UPDATE SKIP LOCKED LIMIT $3) UPDATE object_storage_orphans AS orphan SET lease_token=$4, lease_until=COALESCE($2::timestamptz,now()) + ($5::text || ' milliseconds')::interval, updated_at=COALESCE($2::timestamptz,now()) FROM candidates WHERE orphan.workspace_id=$1 AND orphan.id=candidates.id RETURNING ${orphanProjection}`, [workspaceId, options.now ?? null, limit, token, leaseMs])).rows) }
