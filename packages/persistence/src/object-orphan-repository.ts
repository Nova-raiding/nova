...
export class MemoryObjectOrphanRepository implements ObjectOrphanRepository {
  ...
  async listPending(workspaceId: string, limit = DEFAULT_LIMIT) { const { limit: safeLimit } = optionsOf({ limit }); const now = Date.now(); return [...this.rows.values()].filter(row => row.workspaceId === workspaceId && row.state === 'pending' && Date.parse(row.nextAttemptAt) <= now).slice(0, safeLimit).map(row => ({ ...row })) }
...
export class PostgresObjectOrphanRepository implements ObjectOrphanRepository {
  ...
  async markCleaned(input: { workspaceId: string; id: string; leaseToken?: string }) { requireWorkspaceScope(input.workspaceId); await withWorkspaceTransaction(this.pool, input.workspaceId, async client => { await client.query(`UPDATE object_storage_orphans SET state='cleaned', lease_token=NULL, lease_until=NULL, updated_at=now() WHERE workspace_id=$1 AND id=$2 AND state='pending' AND (($3::text IS NULL AND lease_token IS NULL) OR (lease_token=$3 AND lease_until>now()))`, [input.workspaceId, input.id, input.leaseToken ?? null]) }) }
  async markRetry(input: { workspaceId: string; id: string; error: string; nextAttemptAt: string; manualAttention?: boolean; leaseToken?: string }) { requireWorkspaceScope(input.workspaceId); await withWorkspaceTransaction(this.pool, input.workspaceId, async client => { await client.query(`UPDATE object_storage_orphans SET state=$3, attempts=attempts+1, last_error=$4, next_attempt_at=$5, lease_token=NULL, lease_until=NULL, updated_at=now() WHERE workspace_id=$1 AND id=$2 AND state='pending' AND (($6::text IS NULL AND lease_token IS NULL) OR (lease_token=$6 AND lease_until>now()))`, [input.workspaceId, input.id, input.manualAttention ? 'manual_attention' : 'pending', input.error, input.nextAttemptAt, input.leaseToken ?? null]) }) }
}
