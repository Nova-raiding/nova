...
describe('MigrationRunner', () => {
  ...
  it('loads the ordered production migration set', async () => {
    ...
    expect(byVersion.get(189)).toMatchObject({ name: 'enterprise_display_projection' })
    expect(byVersion.get(189)?.sql).toContain('enterprises_workspace_scope')
    ...
    expect(byVersion.get(200)).toMatchObject({ name: 'customer_delivery_control_plane_acl' })
    expect(byVersion.get(201)).toMatchObject({ name: 'customer_delivery_required_evidence' })
    expect(byVersion.get(202)).toMatchObject({ name: 'customer_delivery_evidence_transition' })
    expect(byVersion.get(203)).toMatchObject({ name: 'workspace_identity_binding_member_fk' })
    expect(byVersion.get(204)).toMatchObject({ name: 'customer_delivery_atomic_evidence' })
    expect(byVersion.get(205)).toMatchObject({ name: 'customer_delivery_evidence_identity_invalidation' })
    expect(byVersion.get(206)).toMatchObject({ name: 'customer_delivery_evidence_invalidation_lock_order' })
    expect(byVersion.get(207)).toMatchObject({ name: 'customer_delivery_evidence_receipt_identity' })
    expect(byVersion.get(208)).toMatchObject({ name: 'alert_webhook_receipts' })
    expect(byVersion.get(209)).toMatchObject({ name: 'isolate_alert_webhook_receiver_role' })
    expect(byVersion.get(219)).toMatchObject({ name: 'public_platform_rules' })
    expect(byVersion.get(219)?.sql).toContain('CREATE TABLE IF NOT EXISTS public_platform_rule_versions')
    ...
    expect(byVersion.get(221)).toMatchObject({ name: 'commercial_refund_amount_bound' })
    ...
    expect(byVersion.get(236)).toMatchObject({ name: 'repair_ops_read_acl' })
    expect(byVersion.get(237)).toMatchObject({ name: 'repair_control_plane_identity_acl' })
    ...
    expect(byVersion.get(239)?.sql).not.toContain('REVOKE ALL ON mcp_oauth_authorization_codes FROM PUBLIC, merchant_app')
    expect(byVersion.get(239)?.sql).not.toContain('REVOKE ALL ON mcp_oauth_tokens FROM PUBLIC, merchant_app')
    expect(byVersion.get(240)).toMatchObject({ name: 'revoke_mcp_oauth_app_acl' })
    expect(byVersion.get(240)?.sql).toContain('REVOKE ALL ON mcp_oauth_authorization_codes FROM PUBLIC, merchant_app')
    expect(byVersion.get(240)?.sql).toContain('REVOKE ALL ON mcp_oauth_tokens FROM PUBLIC, merchant_app')
    expect(byVersion.get(241)).toMatchObject({ name: 'repair_ops_directory_finance_read_acl' })
    expect(byVersion.get(242)).toMatchObject({ name: 'repair_creative_point_allocation_reservation_index', transactional: false })
    ...
    expect(byVersion.get(137)).toMatchObject({ name: 'task_canonical_listing_identity' })
    ...
    expect(byVersion.get(141)).toMatchObject({ name: 'interactive_confirmation_ticket_acl_guard' })
    expect(byVersion.get(146)).toMatchObject({ name: 'commercial_catalog_v2' })
    expect(byVersion.get(146)?.sql).toContain("'50g'")
    expect(byVersion.get(146)?.sql).toContain("'pending_business_approval', false")
    expect(byVersion.get(150)).toMatchObject({ name: 'repair_legacy_creative_point_allocations' })
    expect(byVersion.get(150)?.sql).toContain('RENAME COLUMN points TO points_delta')
    ...
    expect(byVersion.get(160)).toMatchObject({ name: 'commercial_point_adjustment_approval_acl' })
    expect(byVersion.get(160)?.sql).toContain('REVOKE INSERT, UPDATE, DELETE, TRUNCATE')
    ...
    expect(byVersion.get(217)?.sql).toContain('model_cost_budget_reservations_modality_check')
    expect(byVersion.get(217)?.sql).toContain("'embedding'")
