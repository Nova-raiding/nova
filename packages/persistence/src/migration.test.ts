...
describe('MigrationRunner', () => {
  ...
  it('loads the ordered production migration set', async () => {
    ...
    expect(migrations.map(migration => migration.version)).toEqual(Array.from({ length: 46 }, (_, index) => index + 1))
    expect(migrations[1]?.sql).toContain('FORCE ROW LEVEL SECURITY')
    expect(migrations.at(-2)).toMatchObject({ version: 45, name: 'platform_identity_lifecycle' })
    expect(migrations.at(-2)?.sql).toContain("current_setting('app.platform_scope', true) = 'platform_ops'")
    expect(migrations.at(-1)).toMatchObject({ version: 46, name: 'model_usage_settlement' })
    expect(migrations.at(-1)?.sql).toContain('model_usage_ledger_workspace_receipt_key')
