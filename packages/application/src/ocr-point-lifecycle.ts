...
export function quoteOcrPointHold(taskCapCny: unknown) {
  if (typeof taskCapCny !== 'number' || !Number.isFinite(taskCapCny) || taskCapCny <= 0) {
    throw new TypeError('OCR_TASK_COST_CAP_INVALID')
  }
  return { points: ocrCreativePointsFromCost(taskCapCny), policyVersion: OCR_COST_POINT_POLICY_VERSION }
}

export type OcrPointDecision =
  | { action: 'settle'; actualPoints: number; policyVersion: typeof OCR_COST_POINT_POLICY_VERSION }
...
export function decideOcrPointFinalization(input: {
  ...
  try { actualPoints = ocrCreativePointsFromCost(input.actualCostCny) }
  ...
  return { action: 'settle', actualPoints, policyVersion: OCR_COST_POINT_POLICY_VERSION }
}
