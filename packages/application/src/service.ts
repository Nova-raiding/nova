...
export function assetReadiness(asset: Pick<AssetMetadata, 'scanStatus' | 'parseStatus' | 'rightsStatus' | 'rightsScope' | 'factsConfirmedBy' | 'factsConfirmedAt'>): AssetReadiness {
  const reasons: string[] = []
  if (asset.scanStatus === 'blocked') reasons.push('安全扫描阻断')
  else if (asset.scanStatus !== 'clean') reasons.push('等待安全扫描')
  ...
  const blocked = asset.scanStatus === 'blocked' || asset.parseStatus === 'failed' || asset.rightsStatus === 'rejected' || asset.rightsScope === 'unusable'
...
type CompetitorReferenceSnapshot = NonNullable<KnowledgeGenerationContext['competitorReferences']>[number]

function parseCompetitorReference(value: unknown): CompetitorReferenceSnapshot | undefined {
  ...
  try {
    ...
    return { competitorAnalysisId: parsed.competitorAnalysisId.trim(), structuralObservations, expressionObservations, differentiationAngles, safeExpressionGuidance, compliance: { originalTextCopied: false, competitorBrandReused: false } }
  } catch { throw new DomainError('TASK_COMPETITOR_REFERENCE_INVALID', 'competitor_reference_json 必须是经审核的差异化竞品参考对象', 400) }
}
...
function zipStored(files: Record<string, string>): Uint8Array {
  ...
  for (const [name, value] of Object.entries(files)) {
    const nameBytes = Buffer.from(name, 'utf8'); const data = Buffer.from(value, 'utf8'); const header = Buffer.alloc(30 + nameBytes.length)
...
export class MerchantService {
  ...
  private captureTaskInputSnapshot(task: Task, snapshotId = `task:${task.id}:v${task.version}`) {
    ...
    const preferredAssetIds = [...this.assets.values()].filter(asset => asset.preference?.verdict === 'excellent' && assetUsableForTask(asset)).map(asset => asset.id)
    ...
    const competitorReference = parseCompetitorReference(task.answers.competitor_reference_json)
    const knowledgeContext = this.options.knowledgeContextProvider?.({ workspaceId: task.workspaceId, platform: task.platform, ...(product.category ? { category: product.category } : {}), ...(brand?.name ? { brand: brand.name } : {}), ...(product.storeName ? { store: product.storeName } : {}), ...(competitorReference ? { competitorReference } : {}), asOf: rulesCheckedAt })
    ...
    const snapshot: TaskInputSnapshot = deepFreeze(structuredClone({
      ...
      ...(brand ? { brand } : {}),
      assets,
      ...(knowledgeContext ? { knowledgeContext: { ...knowledgeContext, ...(competitorReference ? { competitorReferences: [competitorReference] } : {}) } } : {}),
    }))
  ...
  private taskSnapshot(task: Task) {
    ...
    return existing ?? this.captureTaskInputSnapshot(task)
  }
  ...
  private deriveTaskQuestions(task: Pick<Task, 'requestText' | 'answers' | 'deferredQuestionIds' | 'accountId' | 'platform'>, product: Product) {
    ...
    if (!product.factsConfirmed && answers.confirm_facts !== true) {
      add({ id: 'confirm_facts', kind: 'blocking', prompt: '请确认商品事实、SKU、价格和库存是否准确。', why: '正式生成和发布必须基于已确认事实。', ifSkipped: '任务保持草稿，不能选择方向或生成正式内容。' }, 100)
    }
    ...
    if (product.skus?.length) {
      if (product.skus.length > 1) {
        ...
        if (!validSku) {
          add({ id: 'sku_id', kind: 'blocking', prompt: '当前商品是多 SKU，请选择需要处理的 SKU。', why: '不同 SKU 的价格、库存和属性会影响内容与发布字段映射。', ifSkipped: '不能进入方案确认与发布，因为会导致字段归属不明确。' }, 95)
        }
      }
    }
    ...
    if (task.accountId && account) {
      if (account.tokenState === 'revoked') {
        add({ id: 'authorization', kind: 'blocking', prompt: '平台授权已撤销，请重新连接店铺授权。', why: '未授权时不能确认发布差异并写回平台。', ifSkipped: '不能进入方案确认，不影响本地内容草稿生成。' }, 96)
      } else if (account.tokenState === 'refresh_required') {
        add({ id: 'authorization', kind: 'blocking', prompt: '平台授权会话需续期后才能继续。', why: '旧授权会让发布回写和回执比对不可信。', ifSkipped: '先续期再继续选择方向与生成，避免审核不一致。' }, 90)
      }
    }
    ...
    if (blockingRuleFindings.length) {
      ...
      add({ id: 'rule_conflict', kind: 'blocking', prompt: `当前平台规则需要运营处理后才能继续：${ruleNames}`, why: '规则生效时间或优先级冲突会改变内容审核结论，不能由商家在 Codex 中覆盖。', ifSkipped: '任务保持草稿；请让平台运营更新、启用或停用对应规则后重新确认。' }, 99)
    }
    ...
    if (/促销|活动|优惠|折扣|满减|券|秒杀/u.test(pricePolicy) && !answers.activity_valid_until) {
      add({ id: 'activity_valid_until', kind: 'blocking', prompt: '这次价格或优惠活动的有效期到什么时候？', why: '过期优惠会造成价格错误和平台审核风险。', ifSkipped: '不能确认制作方案，也不会生成带活动价格的内容。' }, 88)
    }
    ...
    if (hasAnswer('blocking')) return [...blocking, ...recommended, ...visibleOptional].slice(0, maxPerRound).map(item => item.question)
  ...
  cloneTask(workspaceId: string, taskId: string, requestText?: string, target?: { productId?: string; platform?: Platform; accountId?: string; region?: string }) {
    ...
    return this.createTask({ workspaceId, productId: targetProductId, platform: targetPlatform, ...(accountId ? { accountId } : {}), ...(target?.region?.trim() ? { region: target.region.trim() } : source.region ? { region: source.region } : {}), requestText: requestText?.trim() || `从任务 ${source.id} 创建的${targetPlatform === source.platform ? '' : `${targetPlatform} `}副本` })
  }
  listCreativeDirections(workspaceId: string, taskId: string): CreativeDirection[] {
    ...
    const product = this.products.get(task.productId)
    if (!product || product.workspaceId !== workspaceId) throw new DomainError('PRODUCT_NOT_FOUND', '商品不存在或不属于当前工作区', 404)
    if (task.directions?.length) return task.directions.map(direction => ({ ...direction }))
    const subject = product.title
    const evidence = (sellingPoints: string[]) => sellingPoints.map(text => {
      const fact = product.sellingPoints?.find(point => point.text === text)
  ...
  updateCreativeDirections(input: { workspaceId: string; taskId: string; action: 'regenerate' | 'merge' | 'modify'; directionIds?: string[]; directionId?: string; changes?: Partial<Pick<CreativeDirection, 'name' | 'coreIdea' | 'structure' | 'visualDirection' | 'sellingPoints' | 'fitReason' | 'copyDirection' | 'risk'>>; feedback?: string; expectedVersion?: number }) {
    ...
    if (input.action === 'regenerate') {
      const suffix = input.feedback?.trim() ? `（结合反馈：${input.feedback.trim().slice(0, 40)}）` : ''
      directions = current.slice(0, 3).map((direction, index) => ({ ...direction, id: `${String.fromCharCode(65 + index)}-v${revision}`, name: `${direction.name}${suffix}` }))
      ...
      newDirection = { ...original, ...changes, id: `${original.id}-v${revision}` }
      directions = current.map(direction => direction.id === original.id ? newDirection! : direction)
    }
  ...
  listSyncJobs(workspaceId: string) { return [...this.syncJobs.values()].filter(job => job.workspaceId === workspaceId).sort((a, b) => b.createdAt.localeCompare(a.createdAt)) }
  ...
  hydrateSnapshot(input: { entityType: 'product' | 'task' | 'content_version' | 'publish_job' | 'platform_account' | 'generation_job' | 'image_generation_job' | 'brand_profile' | 'asset' | 'feedback' | 'sync_job'; entity: unknown }) {
    ...
    if (input.entityType === 'asset') {
      ...
      this.assets.set(entity.id, { ...asset, parseStatus: asset.parseStatus ?? 'pending', contentTrust: asset.contentTrust ?? untrustedAssetContent(), references: asset.references?.length ? asset.references : [{ name: asset.name, mimeType: asset.mimeType, firstSeenAt: asset.createdAt }] })
    }
  ...
  registerAsset(input: { workspaceId: string; name: string; mimeType: string; sizeBytes: number; sha256: string; storageKey: string; rightsStatus?: AssetMetadata['rightsStatus']; rightsScope?: AssetMetadata['rightsScope']; applicablePlatforms?: Platform[]; applicableRegions?: string[]; usageScopes?: string[]; validFrom?: string; validTo?: string; aiModificationAllowed?: boolean }): AssetRegistrationResult {
  ...
  updateAssetParse(input: { workspaceId: string; assetId: string; state: AssetMetadata['parseStatus']; facts?: Record<string, unknown>; error?: string; errorContext?: AssetMetadata['parseErrorContext']; source?: 'parser' | 'model_ocr' | 'manual'; confirmedBy?: string }) {
    ...
    if (input.source === 'manual' && asset.scanStatus !== 'clean') throw new DomainError('ASSET_FACTS_SCAN_REQUIRED', '素材完成安全扫描后才能人工确认事实', 409)
  ...
  updateAssetRights(input: { workspaceId: string; assetId: string; rightsStatus: AssetMetadata['rightsStatus']; rightsScope?: AssetMetadata['rightsScope']; applicablePlatforms?: Platform[]; applicableRegions?: string[]; usageScopes?: string[]; validFrom?: string; validTo?: string; aiModificationAllowed?: boolean }) {
    ...
    if (asset.scanStatus !== 'clean' && input.rightsStatus === 'approved') throw new DomainError('ASSET_RIGHTS_SCAN_REQUIRED', '素材完成安全扫描后才能确认权利', 409)
  ...
  enqueueImageGeneration(input: { workspaceId: string; productId: string; taskId?: string; contentVersionId?: string; skuIds?: string[]; sourceAssetIds?: string[]; imageMode?: 'create' | 'optimize'; direction?: string; count?: number; idempotencyKey: string }) {
    const product = this.products.get(input.productId)
    if (!product || product.workspaceId !== input.workspaceId) throw new DomainError('PRODUCT_NOT_FOUND', '商品不存在或不属于当前工作区', 404)
    ...
    const direction = input.direction?.trim() || '商品主体清晰、简洁电商白底主图'
    const sourceProductVersion = product.version ?? 1
    const intentHash = createHash('sha256').update(JSON.stringify({ workspaceId: input.workspaceId, productId: product.id, taskId: task?.id ?? null, contentVersionId: input.contentVersionId ?? null, skuIds, sourceAssetIds: sourceAssetIds ?? [], imageMode, sourceProductVersion, direction, count })).digest('hex')
    ...
    const job: ImageGenerationJob = { id: id('imggen'), workspaceId: input.workspaceId, productId: input.productId, state: 'queued', idempotencyKey: input.idempotencyKey, direction, imageMode, count, skuIds, ...(sourceAssetIds?.length ? { sourceAssetIds } : {}), ...(task ? { taskId: task.id } : {}), ...(input.contentVersionId ? { contentVersionId: input.contentVersionId } : {}), sourceProductVersion, intentHash, artifactRole: 'candidate', archiveState: 'pending', createdAt: now(), updatedAt: now(), revision: 1 }
  ...
  async completeImageGeneration(input: { workspaceId: string; jobId: string }) {
    ...
    if (!product || product.workspaceId !== input.workspaceId) throw new DomainError('PRODUCT_NOT_FOUND', '商品不存在或不属于当前工作区', 404)
    job.state = 'running'; job.revision += 1; job.updatedAt = now()
    if (!this.options.imageGenerator && process.env.NODE_ENV === 'production') {
      job.state = 'failed'; job.errorCode = 'IMAGE_GENERATION_NOT_CONFIGURED'; job.errorMessage = '生产环境未配置商品主图生成服务'; job.revision += 1; job.updatedAt = now()
      throw new DomainError('IMAGE_GENERATION_NOT_CONFIGURED', '生产环境未配置商品主图生成服务', 503)
    }
    let images: string[]
    try {
      images = this.options.imageGenerator
        ? await this.options.imageGenerator.generate({ productTitle: product.title, ...(product.category ? { category: product.category } : {}), direction: job.direction, mode: job.imageMode, count: job.count, ...(job.sourceAssetIds?.length ? { sourceAssetRefs: job.sourceAssetIds } : {}), ...(product.images?.length ? { sourceImageUrls: product.images.slice(0, 10) } : {}), usageContext: { workspaceId: input.workspaceId, actionId: `image:${job.idempotencyKey}` } })
        : Array.from({ length: job.count }, (_, index) => generatedMainImage(product, job.id, index, job.direction))
    } catch (error) {
      job.state = 'failed'; job.errorCode = 'IMAGE_GENERATION_FAILED'; job.errorMessage = error instanceof Error ? error.message.slice(0, 500) : '图片生成服务失败'; job.revision += 1; job.updatedAt = now()
      throw error
    }
    job.images = images; job.state = 'succeeded'; job.revision += 1; job.updatedAt = now()
    return { job, images: [...images], product }
  }
  archiveImageGenerationOutputs(workspaceId: string, jobId: string, outputs: VisualGenerationOutput[], archiveState: ImageGenerationJob['archiveState']) {
    ...
    job.outputs = outputs.map(output => ({ ...output }))
  ...
  selectVisuals(input: { workspaceId: string; contentVersionId: string; visualRefs: string[]; expectedRevision: number; idempotencyKey: string; selectedBy: string; reason: string }) {
    ...
    const items = input.visualRefs.map((visualRef, index) => {
      ...
      return { visualRef, role: index === 0 ? 'main' as const : 'secondary' as const, ...(job.skuIds?.length ? { skuIds: [...job.skuIds] } : {}), ordinal: output.ordinal, sha256: output.sha256, mimeType: output.mimeType, sizeBytes: output.sizeBytes, sourceProductVersion: job.sourceProductVersion, reviewStatus: 'passed' as const }
    })
    const selectionHash = hash(items.map(item => ({ visualRef: item.visualRef, role: item.role, ...(item.skuIds?.length ? { skuIds: item.skuIds } : {}), ordinal: item.ordinal, sha256: item.sha256, mimeType: item.mimeType, sizeBytes: item.sizeBytes, sourceProductVersion: item.sourceProductVersion })))
  ...
  assignMarketingQueueItem(input: { workspaceId: string; itemType: 'generation' | 'publish'; itemId: string; operatorId: string; expectedRevision?: number }) {
    const operatorId = input.operatorId.normalize('NFKC').trim()
    if (!operatorId || operatorId.length > 128 || /[\u0000-\u001f\u007f\p{Cf}]/u.test(operatorId)) throw new DomainError('QUEUE_OPERATOR_INVALID', '队列负责人标识无效', 400)
  ...
  listPublishJobs(workspaceId: string) { return [...this.publishJobs.values()].filter(job => job.workspaceId === workspaceId) }
  ...
  reviewContent(workspaceId: string, contentVersionId: string, rules?: { availableRuleVersionIds: string[]; forbiddenTerms: string[]; ruleHits?: RuleHit[] }): ReviewFinding[] {
    ...
    }), ...brandVisualFindings, ...(product?.images ? reviewProductImages(product.images) : [])]
  ...
  reviewContentReport(workspaceId: string, contentVersionId: string, rules?: { availableRuleVersionIds: string[]; forbiddenTerms: string[]; ruleHits?: RuleHit[] }) {
    ...
    return buildReviewReport(findings, {
  ...
  exportContent(workspaceId: string, contentVersionId: string, format: ContentExportFormat = 'bundle'): ContentExport {
    ...
    const files = {
      ...
      'brief.json': brief,
      'content.md': markdown,
      'review-findings.json': reviewFile,
      'source-map.json': sourceMap,
      ...(publishReceipt ? { 'publish-receipt.json': publishReceipt } : {}),
    }
    if (format === 'markdown') return { fileName: `content-v${version.version}.md`, contentType: 'text/markdown; charset=utf-8', body: markdown }
    if (format === 'manifest') return { fileName: `manifest-v${version.version}.json`, contentType: 'application/json; charset=utf-8', body: JSON.stringify(manifest, null, 2) }
    ...
    return { fileName: `content-v${version.version}-bundle.zip`, contentType: 'application/zip', body: '', binaryBody: zipStored(Object.fromEntries(Object.entries(files).map(([name, value]) => [name, typeof value === 'string' ? value : JSON.stringify(value, null, 2)]))) }
  }

  createTask(input: { workspaceId: string; productId: string; platform: Platform; accountId?: string; region?: string; requestText?: string; brandId?: string; canonicalProductId?: string; listingId?: string; campaignId?: string; campaignItemId?: string; taskId?: string }) {
    const product = this.products.get(input.productId)
    if (!product || product.workspaceId !== input.workspaceId) throw new DomainError('PRODUCT_NOT_FOUND', '商品不存在或不属于当前工作区', 404)
  ...
  createTaskGroup(input: { workspaceId: string; entries: Array<{ productId: string; platform: Platform; accountId?: string; region?: string; skuId?: string }>; requestText?: string; idempotencyKey?: string }) {
    ...
    const rawCanonicalEntries = input.entries.map(entry => ({ platform: entry.platform, productId: entry.productId, accountId: entry.accountId ?? null, region: entry.region ?? null, skuId: entry.skuId ?? null })).sort((left, right) => `${left.platform}:${left.accountId ?? ''}:${left.productId}:${left.skuId ?? ''}`.localeCompare(`${right.platform}:${right.accountId ?? ''}:${right.productId}:${right.skuId ?? ''}`))
  ...
  understandTaskRequest(workspaceId: string, requestText: string): TaskUnderstanding {
    ...
    const extracted = this.extractTaskFields(text, productCandidates.length === 1 ? this.products.get(productCandidates[0]!.id) : undefined)
    ...
    if (productCandidates.length > 1) questions.push({ id: 'product_id', kind: 'blocking', prompt: '检测到多个候选商品，请选择稳定商品 ID。', why: '同名商品不能自动绑定。', ifSkipped: '任务保持待确认。' })
    ...
    const canCreate = platformCandidates.length > 0 && childTasks.every(child => child.bindingState === 'ready') && (!splitBySku || Boolean(uniqueProduct?.factsConfirmed))
    const mode = !platformCandidates.length ? 'needs_clarification' as const : platformCandidates.length > 1 ? 'split_by_platform' as const : splitBySku ? 'split_by_sku' as const : 'single_task' as const
    const reason = mode === 'split_by_platform' ? '每个平台创建独立子任务并分别保存字段映射、规则版本、内容版本和发布回执' : mode === 'split_by_sku' ? '每个 SKU 创建独立交付包并分别保存价格、库存、图片、规则、内容版本和发布回执' : mode === 'single_task' ? '单平台任务使用唯一商品绑定' : '需要先明确目标平台'
    return { requestText: text, platformCandidates, productCandidates, extracted, questions, executionPlan: { mode, canCreate, reason, splitBySku, childTasks } }
  }

  createTaskFromRequest(input: { workspaceId: string; requestText: string; idempotencyKey?: string }): TaskRequestCreation {
    ...
    const entries = understanding.executionPlan.childTasks.map(child => {
      ...
      return { productId: product.id, platform: child.platform, ...(product.accountId ? { accountId: product.accountId } : {}) }
    })
    if (understanding.executionPlan.splitBySku) {
      ...
      const group = this.createTaskGroup({ workspaceId: input.workspaceId, entries: product.skus.map(sku => ({ productId: product.id, platform: entries[0]!.platform, ...(product.accountId ? { accountId: product.accountId } : {}), skuId: sku.id })), requestText: understanding.requestText, ...(input.idempotencyKey ? { idempotencyKey: input.idempotencyKey } : {}) })
  ...
  answerTask(workspaceId: string, taskId: string, answers: Record<string, string | number | boolean | string[]>, expectedVersion?: number) {
    ...
    this.assertExpectedTaskVersion(task, expectedVersion)
    const allowed = new Set(['brand_id', 'store_id', 'product_id', 'sku_id', 'placement', 'goal', 'audience', 'scene', 'selling_points', 'price_policy', 'activity_valid_until', 'promotion_json', 'output_count', 'constraints', 'asset_ids', 'competitor_reference_json', 'confirm_facts', 'defer_questions'])
    for (const key of Object.keys(answers)) if (!allowed.has(key)) throw new DomainError('TASK_ANSWER_INVALID', `不支持的任务字段: ${key}`, 400)
    ...
    if (answers.competitor_reference_json !== undefined) parseCompetitorReference(answers.competitor_reference_json)
  ...
  preparePublish(taskId: string) {
    ...
    const confirmationHash = hash({ taskId, contentVersionId: version.id, remoteSnapshotHash, payloadHash, selectionHash })
    task.pendingPublish = { contentVersionId: version.id, payloadSnapshot: clone(payloadSnapshot), payloadHash, remoteSnapshotHash, confirmationHash, selectionHash, selectedVisuals: clone(selectedVisuals), preparedAt: now() }
    task.state = 'publish_prepared'
    task.version += 1
    return { task, version, remoteSnapshotHash, confirmationHash, payloadHash, selectionHash, storeContext: { platform: task.platform, accountId: task.accountId ?? null, authorizationRevision: account ? account.authRevision ?? account.revision : null, alias: account?.storeAlias ?? null }, operation, changes: Object.keys(fields), protectedFields: operation === 'update' ? ['price', 'inventory', 'sku', 'images', 'attributes', 'listing_status'] : ['images', 'listing_status'], visualPreview: { imageMode: payloadSnapshot.imageMode, count: selectedVisuals.length, items: selectedVisuals.map(item => ({ visualRef: item.visualRef, role: item.role, ordinal: item.ordinal, mimeType: item.mimeType, sizeBytes: item.sizeBytes, reviewStatus: item.reviewStatus, firstIsMainImage: item.role === 'main' })), executionReady: selectedVisuals.length === 0, ...(selectedVisuals.length ? { blocker: 'IMAGE_PUBLISH_ADAPTER_UNAVAILABLE' } : {}) } }
  }

  confirmPublish(input: { workspaceId: string; taskId: string; batchId?: string; contentVersionId: string; confirmationHash: string; remoteSnapshotHash: string; idempotencyKey: string; accountId?: string; mediaAdapterReady?: boolean }) {
    ...
    const expectedConfirmationHash = hash({ taskId: task.id, contentVersionId: input.contentVersionId, remoteSnapshotHash: input.remoteSnapshotHash, payloadHash: pending.payloadHash, selectionHash: currentSelectionHash })
    const currentPayload = this.buildPublishPayloadSnapshot(version, product, currentSelection.length > 0)
    if (expectedConfirmationHash !== input.confirmationHash || hash(currentSelection) !== hash(pending.selectedVisuals) || hash(currentPayload) !== pending.payloadHash) throw new DomainError('STALE_PUBLISH_CONFIRMATION', '确认摘要与当前内容、选图或远端快照不匹配，请重新准备发布', 409)
    ...
    const account = accountId && this.platformAccounts.has(accountId)
      ...
      : undefined
    this.assertActiveJobCapacity(input.workspaceId)
    const job: PublishJob = { id: id('pub'), workspaceId: input.workspaceId, ...(input.batchId ? { batchId: input.batchId } : {}), taskId: task.id, contentVersionId: input.contentVersionId, platform: task.platform, ...(accountId ? { accountId } : {}), ...(account ? { accountRevision: account.authRevision ?? account.revision } : {}), idempotencyKey: input.idempotencyKey, state: 'queued', confirmationHash: input.confirmationHash, remoteSnapshotHash: input.remoteSnapshotHash, payloadSnapshot: clone(pending.payloadSnapshot), payloadHash: pending.payloadHash, ...(pending.selectionHash ? { selectionHash: pending.selectionHash } : {}), selectedVisuals: clone(pending.selectedVisuals), createdAt: now(), revision: 1 }
    this.publishJobs.set(job.id, job)
    this.idempotency.set(`${input.workspaceId}:${input.idempotencyKey}`, job.id)
  ...
  private validateVisualSelection(task: Task, version: ContentVersion, product: Product): SelectedVisualSnapshot[] {
    ...
    const items = selection.items.map((snapshot, index) => {
      ...
      const current = { visualRef: output.visualRef, role: index === 0 ? 'main' as const : 'secondary' as const, ...(job.skuIds?.length ? { skuIds: [...job.skuIds] } : {}), ordinal: output.ordinal, sha256: output.sha256, mimeType: output.mimeType, sizeBytes: output.sizeBytes, sourceProductVersion: job.sourceProductVersion, reviewStatus: 'passed' as const }
      const normalizedSnapshot = { ...snapshot, role: snapshot.role ?? current.role }
      const currentHash = hash(current)
      const legacyCurrentHash = hash({ ...current, skuIds: undefined })
      const legacySnapshotHash = hash({ ...normalizedSnapshot, skuIds: undefined })
    ...
    const currentHash = hash(items.map(item => ({ visualRef: item.visualRef, role: item.role, ...(item.skuIds?.length ? { skuIds: item.skuIds } : {}), ordinal: item.ordinal, sha256: item.sha256, mimeType: item.mimeType, sizeBytes: item.sizeBytes, sourceProductVersion: item.sourceProductVersion })))
