...
const sku = { code: 'basic', kind: 'monthly' as const, version_id: 'basic-v2', lifecycle: 'approved' as const, executable: true as const, effective_at: '2026-09-01T00:00:00Z', blockers: [] as string[], server_snapshot_ref: 'snapshot:basic-v2' }
...
describe('CommercialPurchaseService', () => {
  it('passes only a server snapshot reference to order creation', async () => {
    ...
    expect(createFromServerSnapshot).toHaveBeenCalledWith({ workspace_id: 'ws-1', actor_id: 'actor-1', purchase_kind: 'purchase', server_snapshot_ref: 'snapshot:basic-v2', idempotency_key: 'order-1', reason: 'subscribe' })
  })
