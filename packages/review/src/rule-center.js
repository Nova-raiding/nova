...
export class RuleCenter {
    ...
    audits = [];
    sequence = 0;
    constructor(clock = () => new Date().toISOString(), seeds = []) {
        this.clock = clock;
    ...
    versionId(packId, version) { return `${packId}@${version}`; }
    checksum(seed) {
        return createHash('sha256').update(JSON.stringify(seed)).digest('hex');
    }
    validate(seed) {
        if (!seed.packId.trim() || !seed.name.trim() || !seed.version.trim() || !seed.source.reference.trim())
            throw new Error('RULE_VERSION_INVALID');
        if (!seed.source.checkedAt)
            throw new Error('RULE_SOURCE_CHECK_REQUIRED');
        for (const term of seed.checks?.forbiddenTerms ?? [])
            if (!term.trim())
                throw new Error('RULE_CHECK_INVALID');
    }
    seed(seed) {
        this.validate(seed);
        ...
        if (siblings.some(item => item.version === seed.version))
            throw new Error('RULE_VERSION_DUPLICATE');
        if (version.status === 'active' && siblings.some(item => item.status === 'active'))
            throw new Error('RULE_ACTIVE_VERSION_CONFLICT');
        ...
        if (version.status === 'active')
            this.audits.push({ id: this.nextAuditId(), rulePackId: seed.packId, ruleVersionId: version.id, version: version.version, action: 'activated', actorId: version.createdBy, at });
    }
    latest(packId, version) {
    ...
    list(options = {}) {
        ...
        for (const items of this.versions.values()) {
            for (const item of items)
                if (options.includeInactive || item.status === 'active')
                    result.push(this.projection(item));
        }
        ...
        for (const items of this.versions.values())
            for (const item of items)
                if (item.status === 'active') {
                    for (const term of item.checks.forbiddenTerms ?? [])
                        terms.add(term);
                    for (const field of item.checks.requiredFields ?? [])
                        required.add(field);
                }
     ...
     */
    evaluate(context = {}, at = this.clock()) {
        const now = Date.parse(at);
        if (Number.isNaN(now))
            throw new Error('RULE_EVALUATION_TIME_INVALID');
        ...
        for (const scope of scopeOrder) {
            for (const items of this.versions.values()) {
                for (const item of items.filter(candidate => (candidate.status === 'active' || candidate.status === 'expired') && candidate.scope === scope)) {
                    ...
                    if (scope !== 'global' && (!expected || (target && target !== expected) || !target))
                        continue;
                    ...
                    if ((item.effectiveFrom && Number.isNaN(from)) || (item.effectiveTo && Number.isNaN(to)))
                        continue;
                    if (now < from) {
        ...
        for (let higherIndex = 0; higherIndex < applicable.length; higherIndex += 1) {
            ...
            for (let lowerIndex = higherIndex + 1; lowerIndex < applicable.length; lowerIndex += 1) {
                ...
                if ((higher.action ?? 'block') === 'block' && lower.action === 'allow')
                    findings.push({ code: 'RULE_PRIORITY_CONFLICT', severity: 'error', action: 'block', field: 'rules', ruleVersionId: lower.id, message: `低优先级规则 ${lower.version} 不能覆盖 ${higher.scope} 范围的硬阻断规则 ${higher.version}` });
            }
        }
        const terms = new Set();
        const required = new Set();
    ...
    publish(input) {
        ...
        if (siblings.some(item => item.version === input.version))
            throw new Error('RULE_VERSION_DUPLICATE');
        const at = this.clock();
        const version = {
        ...
        siblings.push(version);
        this.versions.set(input.packId, siblings);
    ...
    setStatus(input) {
        if (!input.reason.trim())
            throw new Error('RULE_STATUS_REASON_REQUIRED');
...
export const defaultRuleCenterSeeds = [
...
//# sourceMappingURL=rule-center.js.map