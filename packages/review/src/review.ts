...
export interface ReviewFinding {
  code: 'MISSING_SOURCE' | 'MISSING_RULE_VERSION' | 'PRICE_NOT_ALLOWED' | 'SKU_MISMATCH' | 'SKU_IMAGE_MAPPING_INVALID' | 'FORBIDDEN_TERM' | 'BRAND_FORBIDDEN_TERM' | 'BRAND_VISUAL_ASSET_NOT_READY' | 'BRAND_FONT_LICENSE_NOT_APPROVED' | 'RULE_EXPIRED' | 'RULE_NOT_YET_EFFECTIVE' | 'RULE_PRIORITY_CONFLICT' | 'RULE_SOURCE_INVALID' | 'PROMOTION_EXPIRED' | 'PROMOTION_SCOPE_INVALID' | 'PROMOTION_SKU_UNREFERENCED' | 'MAIN_IMAGE_REQUIRED' | 'IMAGE_URL_INVALID' | 'DUPLICATE_IMAGE' | 'IMAGE_FORMAT_UNSUPPORTED' | 'IMAGE_TOO_SMALL' | 'PRODUCT_FACTS_UNCONFIRMED' | 'SELLING_POINT_PROOF_MISSING' | 'VISUAL_BRIEF_MISSING' | 'VISUAL_BRIEF_INCOMPLETE' | 'TECHNICAL_SCHEMA_INVALID' | 'TECHNICAL_EXPORT_MANIFEST_MISSING' | 'PLATFORM_PREFLIGHT_PENDING'
...
export function reviewDeterministic(input: DeterministicReviewInput): ReviewFinding[] {
  ...
  for (const [index, promotion] of (input.promotions ?? []).entries()) {
    ...
    const validFrom = promotion.validFrom ? Date.parse(promotion.validFrom) : undefined
    const validTo = promotion.validTo ? Date.parse(promotion.validTo) : undefined
    if (promotion.validFrom && Number.isNaN(validFrom)) findings.push(finding({ code: 'PROMOTION_EXPIRED', severity: 'error', priority: 'P0', field: `${field}.validFrom`, message: '促销生效时间格式无效，不能用于当前内容', repairSuggestion: '提供合法的促销生效时间并重新确认价格快照', kind: 'fact', sourceIds: promotion.sourceId ? [promotion.sourceId] : [] }))
    else if (validFrom !== undefined && validFrom > Date.now()) findings.push(finding({ code: 'PROMOTION_EXPIRED', severity: 'error', priority: 'P0', field: `${field}.validFrom`, message: '促销尚未到生效时间，不能用于当前内容', repairSuggestion: '等待活动生效或切换到当前有效的价格快照', kind: 'fact', sourceIds: promotion.sourceId ? [promotion.sourceId] : [] }))
    if (promotion.validTo && Number.isNaN(validTo)) findings.push(finding({ code: 'PROMOTION_EXPIRED', severity: 'error', priority: 'P0', field: `${field}.validTo`, message: '促销结束时间格式无效，不能用于当前内容', repairSuggestion: '提供合法的促销结束时间并重新确认价格快照', kind: 'fact', sourceIds: promotion.sourceId ? [promotion.sourceId] : [] }))
    else if (validTo !== undefined && validTo <= Date.now()) findings.push(finding({ code: 'PROMOTION_EXPIRED', severity: 'error', priority: 'P0', field: `${field}.validTo`, message: '促销已过期，不能继续批准或导出当前内容', repairSuggestion: '更新促销有效期并重新确认价格，或移除促销表达后创建新版本', kind: 'fact', sourceIds: promotion.sourceId ? [promotion.sourceId] : [] }))
...
const categoryDefinitions: Array<{ id: ReviewCategoryId; name: string; codes: ReviewFinding['code'][] }> = [
  ...
  { id: 'copy_price_compliance', name: '文案、价格与合规', codes: ['FORBIDDEN_TERM', 'MISSING_RULE_VERSION', 'RULE_EXPIRED', 'RULE_NOT_YET_EFFECTIVE', 'RULE_PRIORITY_CONFLICT', 'RULE_SOURCE_INVALID', 'PROMOTION_EXPIRED', 'PROMOTION_SCOPE_INVALID', 'PROMOTION_SKU_UNREFERENCED'] },
