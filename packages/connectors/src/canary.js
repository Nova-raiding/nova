function check(capability, passed, simulated, detail) {
    return { capability, passed, simulated, ...(detail ? { detail } : {}) };
}
function evidence(input, capability, state, simulated = false) {
    return {
 ...
 */
export async function runPlatformCanary(input) {
    const checks = [];
    const evidenceItems = [];
    const add = (capability, passed, simulated, detail) => {
    ...
    try {
        ...
        add('authorize', false, false, error instanceof Error ? error.message : String(error));
    }
    let full;
    try {
    ...
    try {
        ...
        add('incremental_sync', false, false, error instanceof Error ? error.message : String(error));
    }
    let remoteId;
    if (!input.allowWrite) {
        ...
        try {
            ...
            add('update', false, false, error instanceof Error ? error.message : String(error));
        }
    }
    if (!input.allowRevoke)
        add('revoke', false, false, 'revoke canary disabled; set explicit allowRevoke for a disposable test account');
    else {
        try {
            ...
            add('revoke', false, false, error instanceof Error ? error.message : String(error));
        }
    }
    if (!input.mediaFile || !input.connector.uploadMedia) {
        ...
        try {
            const media = { visualRef: `canary-${input.connector.platform}`, role: 'main', mimeType: input.mediaFile.mimeType, sha256: input.mediaFile.sha256, bytes: input.mediaFile.bytes, idempotencyKey: `platform-canary-media-${input.connector.platform}-${Date.now()}` };
            ...
            add('media_upload', false, false, error instanceof Error ? error.message : String(error));
        }
    }
