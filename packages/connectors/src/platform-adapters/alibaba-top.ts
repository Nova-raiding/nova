...
export function createAlibabaTopSigner(options: AlibabaTopSignerOptions): RequestSigner {
  ...
  return {
    ...
    sign(request) {
      ...
      if (request.credential?.accessToken) params.session = request.credential.accessToken
      delete params.sign
...
function formatTopTimestamp(value: Date): string {
...
function signTop(method: AlibabaTopSignerOptions['signMethod'], secret: string, canonical: string): string {
  if (method === 'md5') return createHash('md5').update(`${secret}${canonical}${secret}`, 'utf8').digest('hex').toUpperCase()
...
function numberValue(value: unknown): number { const valueNumber = typeof value === 'number' ? value : Number(value); return Number.isFinite(valueNumber) ? valueNumber : 0 }
