 ...
 */
export const OFFICIAL_PLATFORM_HOSTS = {
...
function matchesAllowlist(host, allowlist) {
    ...
    return allowlist.some(entry => {
        ...
        if (candidate.startsWith('*.'))
            return value.endsWith(candidate.slice(1)) && value !== candidate.slice(2);
        return value === candidate;
    });
}
export function isSecureEnvironment(environment = process.env.NODE_ENV) {
    return environment === 'staging' || environment === 'production';
}
export function inspectOutboundUrl(raw, policy = {}) {
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (isSecureEnvironment(environment) && url.protocol !== 'https:')
    ...
    if (!host || blockedHostnames.has(host) || host.endsWith('.local') || host.endsWith('.internal'))
        return 'PRIVATE_ADDRESS_BLOCKED';
    if (!policy.allowPrivateHosts && isBlockedIp(host))
        return 'PRIVATE_ADDRESS_BLOCKED';
    if (policy.allowedHosts && !matchesAllowlist(host, policy.allowedHosts))
        return 'HOST_NOT_ALLOWLISTED';
    return undefined;
}
export async function assertOutboundUrl(raw, policy = {}) {
    const reason = inspectOutboundUrl(raw, policy);
    if (reason)
        throw new Error(`unsafe outbound URL: ${reason}`);
    if (policy.resolveDns === false)
        return;
    const environment = policy.environment ?? process.env.NODE_ENV;
    ...
    if (!isSecureEnvironment(environment))
        return;
...
export function officialHostsFor(platform) {
    return OFFICIAL_PLATFORM_HOSTS[platform];
}
//# sourceMappingURL=outbound-security.js.map