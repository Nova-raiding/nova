...
function event(overrides: Record<string, unknown> = {}): DurableOutboxEvent {
  return {
    ...
    payload: { commercial_access_snapshot: {
      ...
      access_revision: 'access_7', balance_state: 'positive', subscription_snapshot_id: 'subscription_snapshot_3', subscription_snapshot_checksum: 'a'.repeat(64),
...
function live(snapshot: WorkerCommercialAccessSnapshot) {
  return {
    ...
    accessRevision: snapshot.accessRevision, balanceState: snapshot.balanceState, subscriptionSnapshotId: snapshot.subscriptionSnapshotId,
    subscriptionSnapshotChecksum: snapshot.subscriptionSnapshotChecksum, rateVersion: snapshot.rateVersion, quotedPoints: snapshot.quotedPoints,
...
describe('worker commercial execution gate', () => {
  ...
  it('supports point-required zero-charge work without inventing a reservation', async () => {
    const noCharge = event({ access_mode: 'POINT_REQUIRED_NO_CHARGE', quoted_points: 0, reservation_id: undefined })
