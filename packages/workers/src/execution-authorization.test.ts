...
import { createExecutionAuthorizationGuard, createUnavailableExecutionAuthorizationGuard } from './execution-authorization.js'
  ...
  return {
    ...
    payload: {
      authorization_snapshot: {
        schema_version: 1, decision_id: 'decision_enqueue', actor_id: 'merchant_1', workspace_id: 'ws_a', context_id: 'workspace:ws_a', context_version: 'ctx_7', policy_version: 'policy_3', grant_revision: 'grant_11', scope_hash: 'a'.repeat(64), capability: 'publish.execute', resource_id: 'publish_1', authorized: true, decided_at: '2026-08-31T09:59:00.000Z',
        ...overrides,
      },
    },
  }
...
describe('worker execution-time authorization', () => {
  it('binds the enqueue snapshot to fresh authoritative execution evidence', async () => {
    const recheck = vi.fn(async ({ snapshot }: { snapshot: { actorId: string; scopeHash: string } }) => ({
      recheckId: 'decision_execute', actorId: snapshot.actorId, workspaceId: 'ws_a', contextId: 'workspace:ws_a', contextVersion: 'ctx_8', policyVersion: 'policy_4', grantRevision: 'grant_12', scopeHash: snapshot.scopeHash, capability: 'publish.execute' as const, resourceId: 'publish_1', authorized: true, checkedAt: '2026-08-31T09:59:59.000Z',
    }))
  ...
  it('rejects revoke, stale evidence and resource substitution', async () => {
    const base = { recheckId: 'decision_execute', actorId: 'merchant_1', workspaceId: 'ws_a', contextId: 'workspace:ws_a', contextVersion: 'ctx_8', policyVersion: 'policy_4', grantRevision: 'grant_12', scopeHash: 'a'.repeat(64), capability: 'publish.execute' as const, resourceId: 'publish_1', authorized: true, checkedAt: '2026-08-31T09:59:59.000Z' }
  ...
  it('supports a redriven asset scan capability without weakening its event binding', async () => {
    ...
    const guard = createExecutionAuthorizationGuard(async ({ snapshot }) => ({
      recheckId: 'decision_scan_execute', actorId: snapshot.actorId, workspaceId: 'ws_a', contextId: 'workspace:ws_a', contextVersion: 'ctx_8', policyVersion: 'policy_4', grantRevision: 'grant_12', scopeHash: snapshot.scopeHash, capability: 'asset.scan.execute', resourceId: 'asset_1', authorized: true, checkedAt: '2026-08-31T09:59:59.000Z',
    }), { now: () => now })
