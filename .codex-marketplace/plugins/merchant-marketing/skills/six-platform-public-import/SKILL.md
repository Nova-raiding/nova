...
3. 解析与标准化：优先使用 `scripts/extract-product.mjs` 提取 JSON-LD、OpenGraph 和页面标题；保留原始字段的来源定位，金额统一为人民币元并保留两位小数；图片只保存公开 URL/哈希，不把图片字节或竞品原文直接写入知识库。
...
- 解析脚本只处理已经取得的 HTML，不负责网络访问；调用方必须先完成公开 URL 安全校验，并将抓取失败与字段缺失原样保留。
