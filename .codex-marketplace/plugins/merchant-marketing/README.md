...
包外签名安装器还会在修改插件文件前，通过 Credential Manager 中的 P-256 安装身份为“ZIP SHA-256、插件版本、发布者指纹、上一已提交包摘要和递增序号”生成签名候选。只有安装及可选登录全部成功后才原子提交新绑定；失败时不覆盖上一条已验签绑定，上一版本目录和配置备份继续作为升级回退证据。绑定缺失、签名链损坏、安装身份变化或并发覆盖都会失败关闭。当前缺少 Windows 实机和正式证书，因此这只是代码级门禁，不能作为生产安装通过证据。
...
插件内置两层视频策划技能：`ecommerce-video-marketing` 负责商品痛点、卖点、受众、平台调性和营销脚本；`storyboard-prompt-assistant` 负责逐镜头时长、景别、运镜、正/负提示词和连续性。它们只生成可审阅的脚本与分镜，真实成片仍必须经过商品事实、商家确认、平台模型中转、创意点、归档和发布前审核；脚本或分镜不等于已生成或已发布视频。
...
当前登录、工作区/RLS、角色、交付、创意点、模型中转与成本证据均保留。`billing.status` 展示服务端余额与恢复状态；旧人民币收款和 provider 成本账本不能解锁业务功能。内容制作不修改店铺授权或自动解除账号停用。
