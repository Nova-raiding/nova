...
describe('Codex stdio MCP bridge', () => {
  it('presents first-use progress as a native welcome and a brand-confirmation conversation', async () => {
    let completed = 0
  ...
  it('requires current-session confirmation for protected writes even when the legacy write env is enabled', async () => {
    ...
    try {
      ...
      // Confirmation is an API-backed action as well as opening the local session.
  ...
  it('does not let DEPLOY_ENV production bypass the interactive write gate', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, NODE_ENV: 'development', DEPLOY_ENV: 'production', MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
  ...
  it('returns standardized action cards in structuredContent with a concise user-facing next step', async () => {
    ...
    try {
      ...
      const response = await nextLine(child.stdout)
      // Billing status is projected to a merchant-safe capability snapshot;
      // internal action cards and next actions are not exposed by the bridge.
      expect(response.result.structuredContent).toMatchObject({ availability: 'unknown' })
      expect(response.result.structuredContent.action_cards).toBeUndefined()
      expect(response.result.content[0].text).toContain('创意点状态：待确认')
  ...
  it('does not present fixture-only stores as a usable merchant workspace', async () => {
    ...
    try {
      ...
      expect(response.result.content[0].text).toContain('官方授权页面')
      expect(response.result.structuredContent.question).toContain('连接真实店铺')
  ...
  it('never emits an ops-only tool as a merchant store-capacity action', async () => {
    ...
    try {
      ...
      expect(structured).toMatchObject({ availability: 'unknown' })
      expect(structured.action_cards).toBeUndefined()
      expect(structured.store_capacity).toBeUndefined()
    } finally {
      child.kill()
      await close(server)
    }
  ...
  it('exposes standard discovery and forwards the scoped API envelope', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_ASSET_RESOURCE_DOMAINS: 'https://assets.example.test', MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize' })}\n`)
      expect((await nextLine(child.stdout)).result).toMatchObject({ capabilities: { tools: {} }, serverInfo: { name: 'merchant-marketing', version: '0.1.0+codex.20260916100500' } })
      ...
      expect(rechargeUi.result.contents[0].text).toContain('充值')
      expect(rechargeUi.result.contents[0].text).toContain('call("billing.status")')
      expect(rechargeUi.result.contents[0].text).not.toContain('call("billing.export"')
      ...
      for (const [name, [resourceUri, invoking, invoked]] of Object.entries(taskComponents).filter(([name]) => listed.result.tools.some((tool: { name: string }) => tool.name === name))) {
      ...
      for (const name of ['campaign.batch.pause', 'campaign.batch.resume', 'campaign.batch.retry_failed'].filter(name => listed.result.tools.some((item: { name: string }) => item.name === name))) {
  ...
  it('binds the minimal image chooser to every archived clean catalog candidate result', async () => {
    ...
    try {
      ...
      expect(multiple.result.structuredContent).toEqual({
        ...
        selection_request: { job_id: 'job_secret', expected_revision: '7', candidates: [{ ordinal: 1, visual_ref: 'visual_secret_1', selectable: false, subject_label: '商品主体', availability_label: '待人工审核' }, { ordinal: 2, visual_ref: 'visual_secret_2', selectable: false, subject_label: '商品主体', availability_label: '待人工审核' }] },
      ...
      expect(single.result.structuredContent).toEqual({
        ...
        question: '要使用这张作为主图吗？',
        expected_input: { kind: 'main_image_selection', accepts: ['component_selection', 'natural_language'], selection_count: 1 },
        selection_request: { job_id: 'job_secret', expected_revision: '7', candidates: [{ ordinal: 1, visual_ref: 'visual_secret_1', selectable: false, subject_label: '商品主体', availability_label: '待人工审核' }] },
      ...
      expect(unarchived.result).toHaveProperty('structuredContent')
      ...
      for (const [id, state, summary] of [['job_queued', 'queued', '图片任务已排队'], ['job_failed', 'failed', '本次主图生成未完成'], ['job_unknown', 'unknown', '图片结果尚未确认']] as const) {
        ...
        expect(response.result.structuredContent.completed_summary).toContain(state === 'unknown' ? '自动核对中' : summary)
        if (state === 'queued') {
  ...
  it('restores the preferred image after refresh and accepts a new choice only with the new revision', async () => {
    ...
    try {
      ...
      expect(initial.result.structuredContent.selection_request.candidates[0]).toMatchObject({ subject_label: '白色运动鞋', availability_label: '待人工审核', selectable: false })
  ...
  it('uploads an attached local file without putting base64 in the model tool arguments', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_ASSET_SCAN_POLL_TIMEOUT_MS: '500', MERCHANT_ASSET_SCAN_POLL_INTERVAL_MS: '25' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/list' })}\n`)
  ...
  it.each([
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_ASSET_SCAN_POLL_TIMEOUT_MS: '100', MERCHANT_ASSET_SCAN_POLL_INTERVAL_MS: '25' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
  ...
  it('keeps an unavailable official OAuth connection merchant-safe and resumable', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_MCP_RETRY_ATTEMPTS: '1' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
  ...
  it('keeps the uploaded asset pending when status polling is temporarily unavailable', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_MCP_RETRY_ATTEMPTS: '1', MERCHANT_ASSET_SCAN_POLL_TIMEOUT_MS: '100', MERCHANT_ASSET_SCAN_POLL_INTERVAL_MS: '25' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'asset.upload', arguments: { name: 'product.png', mime_type: 'image/png', content_base64: Buffer.from('image').toString('base64') } } })}\n`)
  ...
  it('materializes content exports as private MCP file resources without returning body or base64 to the model', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_ARTIFACT_DIR: directory },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
  ...
  it('retries transient local API readiness failures during Codex startup', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_MCP_RETRY_DELAY_MS: '50' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'workspace.health', arguments: {} } })}\n`)
  ...
  it('does not retry a non-idempotent write after an ambiguous gateway failure', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_MCP_RETRY_ATTEMPTS: '5', MERCHANT_MCP_RETRY_DELAY_MS: '50' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'content.approve', arguments: { content_version_id: 'version_1', expected_version: '1' } } })}\n`)
  ...
  it('preserves actionable gateway error codes and field-level details for Codex', async () => {
    ...
    const address = await listen(server)
    const child = spawn(process.execPath, ['apps/plugin/mcp/bridge.mjs'], { cwd: process.cwd(), env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true' }, stdio: ['pipe', 'pipe', 'pipe'] })
    try {
  ...
  it('preserves redacted authorization decision evidence for ChatGPT', async () => {
    ...
    const address = await listen(server)
    const child = spawn(process.execPath, [BRIDGE_PATH], { cwd: process.cwd(), env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: `http://127.0.0.1:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_WRITE_ENABLED: 'true' }, stdio: ['pipe', 'pipe', 'pipe'] })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'creative.brief', arguments: { product_id: 'product_1', asset_type: 'banner' } } })}\n`)
      const response = await nextLine(child.stdout)
  ...
  it.each([
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, MERCHANT_MCP_BASE_URL: endpoint, MERCHANT_WORKSPACE_ID: 'ws_test' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'workspace.health', arguments: {} } })}\n`)
  ...
  it('fails closed before contacting a remote staging endpoint without HTTPS, token, or strict auth', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, NODE_ENV: 'staging', MERCHANT_MCP_BASE_URL: `http://example.test:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_TOKEN: '${MERCHANT_MCP_TOKEN}', MERCHANT_STRICT_AUTH: '${MERCHANT_STRICT_AUTH}', MERCHANT_ALLOW_FIXTURE_FALLBACK: '${MERCHANT_ALLOW_FIXTURE_FALLBACK}' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'workspace.health', arguments: {} } })}\n`)
  ...
  it('blocks production generation results that omit relay evidence', async () => {
    ...
    const child = spawn(process.execPath, [BRIDGE_PATH], {
      cwd: process.cwd(),
      env: { ...TEST_PROCESS_ENV, NODE_ENV: 'staging', MERCHANT_MCP_BASE_URL: `https://example.test:${address.port}`, MERCHANT_WORKSPACE_ID: 'ws_test', MERCHANT_MCP_TOKEN: 'test-token', MERCHANT_STRICT_AUTH: 'true', MERCHANT_MCP_WRITE_ENABLED: 'true', MERCHANT_ALLOW_FIXTURE_FALLBACK: '${MERCHANT_ALLOW_FIXTURE_FALLBACK}' },
      stdio: ['pipe', 'pipe', 'pipe'],
    })
    try {
      child.stdin.write(`${JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'workspace.interactive.confirm', arguments: { confirmation: 'I_CONFIRM_INTERACTIVE_WRITES' } } })}\n`)
      expect((await nextLine(child.stdout)).result).toMatchObject({ isError: true, structuredContent: { code: 'API_UNAVAILABLE' } })
    } finally {
      child.kill()
      await close(server)
    }
