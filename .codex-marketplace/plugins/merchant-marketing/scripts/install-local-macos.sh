...
[ "$(uname -s 2>/dev/null || true)" = "Darwin" ] || { echo '本地插件安装器仅支持 macOS 桌面宿主；当前不会伪造安装成功。' >&2; exit 1; }
command -v launchctl >/dev/null 2>&1 || { echo '未找到 launchctl；请在 macOS 用户会话中运行。' >&2; exit 1; }
base_url=; workspace=
while [ "$#" -gt 0 ]; do
  case "$1" in
...
case "$base_url" in http://127.0.0.1:*|http://localhost:*|https://*) ;; *) echo 'base URL 必须是 HTTPS，或本机 loopback HTTP；不会发送不安全的远程请求。' >&2; exit 2 ;; esac
case "$base_url" in *\?*|*\#*|*/mcp|*/mcp/) echo 'base URL 不得包含查询参数、片段或 /mcp 路径。' >&2; exit 2 ;; esac
case "$workspace" in ws_[A-Za-z0-9_-]*|workspace_[A-Za-z0-9_-]*|demo-workspace) ;; *) echo 'workspace 必须是服务端分配的工作区标识。' >&2; exit 2 ;; esac
token=
if [ -t 0 ]; then
  printf '请输入 Store Nova 本地连接 token（不会回显）：' >&2
  old_stty=$(stty -g 2>/dev/null || true); stty -echo 2>/dev/null || true; IFS= read -r token
  [ -n "$old_stty" ] && stty "$old_stty" 2>/dev/null || true; printf '\n' >&2
else IFS= read -r token || true; fi
