...
`doc/` 是冻结的历史归档：这些文档写于早期阶段，其中的版本号、工具数量、迁移号和链接可能已经过期，仅作历史背景保留，不再更新。交付文档树是 `docs/`（安装手册、产品总文档、使用指南、Runbook 与发布解阻清单均在其中），所有新提交的文档都以 `docs/` 为目标。业务代码、测试和真实运行证据仍是完成度判断的权威来源。

本目录（`doc/`）中列出的“当前权威入口”仅指历史归档范围内的入口，不代表当前交付基线。
