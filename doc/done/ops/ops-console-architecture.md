...
当前项目已经有运营后台，不是从零开始：`apps/ops-console` 是独立的 React + Ant Design 管理端，通过 `/mcp` 调用 API。后台覆盖商业化和平台控制面，并已接入知识规则、资产权益、学习建议、竞品参考、多模态候选和平台驳回修正版的安全运营队列；真实 provider、平台写入和生产证据仍受独立上线门禁控制。
...
| 访问 | 本地演示使用 Bearer；生产使用 OIDC httpOnly 会话 + 网关 HMAC principal，并携带 `x-workspace-id` | `apps/ops-console/src/App.tsx`、`apps/api/src/server.ts`、`doc/ops-console-runbook.md` |
...
`platform_ops` 是平台控制面角色，不是客户工作区成员。平台运营默认只访问跨工作区聚合、账务、用户治理、平台/商业配置和审计摘要；不得调用 `ops.marketing.queue` 或其分配、视觉审查、生成重试、发布确认、修正版动作来读取或修改客户商品、素材、内容和发布任务详情。上述营销队列能力只对当前工作区的有效角色开放（`workspace_owner`、`merchant_admin`、`operator`），或对被明确授予该工作区 support 权限的支持人员开放。服务端拒绝优先，前端隐藏不构成授权。

## 3.4 竞品与产品模式核验

本轮只采用官方公开资料作为模式参考，不把竞品功能当作本项目已实现证据：

- Shopify Analytics 将核心指标可视化与可下钻明细表放在同一报告，并提供组织级跨店分析视图；因此本后台总览采用“状态卡片 → 有界列表/明细 → 运营动作”的层级，跨工作区查询必须是显式授权的聚合视图。[Shopify analytics](https://help.shopify.com/en/manual/reports-and-analytics/shopify-reports)、[Organization analytics](https://help.shopify.com/en/manual/organization-settings/analytics)
- Stripe Dashboard 将余额、支付、退款、争议和导出视为同一财务运营面；因此本项目把订单、钱包流水、退款预留/完成、模型成本和对账放在 Finance 域，并要求每个动作有幂等键、审计理由和 provider 回执。[Stripe Payments](https://stripe.com/gb/payments/features)
- OpenAI 的官方用量资料强调 Usage 与 Costs 的口径区分，以及按用户、产品和模型观察使用与支出、按工作区/组/个人设置控制；因此本项目保留 `mine` 默认范围、显式 `workspace` 范围、provider 成本与商家收费双列、日预算和人工对账状态，不把本地 token 记录直接当作供应商账单。[Reviewing API usage and costs](https://help.openai.com/en/articles/10478918)、[OpenAI usage analytics and spend controls](https://openai.com/index/chatgpt-enterprise-spend-controls/)

由此形成三条本项目约束：运营页面必须同时展示事实和下一步动作；跨店/跨租户聚合必须与个人明细分离；财务和模型用量必须保留供应商原始证据与本地结算状态，任何缺失都显示为待核对而不是成功。

## 4. 权限矩阵

| 能力 | workspace_owner | merchant_admin | operator | support | finance | platform_ops |
|---|---:|---:|---:|---:|---:|---:|
| 查看本工作区知识/任务 | ✓ | ✓ | ✓ | 按明确工作区授权 | - | - |
| 审核品牌/客户资产 | ✓ | ✓ | ✓ | - | - | - |
| 确认/驳回学习建议 | ✓ | ✓ | 按授权 | - | - | - |
| 发布平台规则 | - | - | - | - | - | ✓/规则管理员 |
| 修改套餐/平台配置 | ✓ | ✓ | - | - | - | ✓ |
...
| 文档/代码 | 作用 |
|---|---|
| `doc/PRD-merchant-marketing-codex-final.md` | FR-17 产品需求和验收基线 |
| `apps/ops-console/src/App.tsx` | 当前运营台 UI 和 MCP 调用 |
| `design-system/merchant-ops-console/MASTER.md` | UI/UX Pro Max 生成并持久化的运营台设计系统：高密度数据布局、可访问性、键盘焦点和低动效约束 |
...
| `doc/ops-console-runbook.md` | 部署、凭证和发布操作手册 |
| `doc/diagrams/ops-console-architecture.html` | 可离线打开的架构图 |
