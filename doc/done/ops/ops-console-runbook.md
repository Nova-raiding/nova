...
当前侧栏和深链接共有 11 个一级域：总览 `overview`、用户与租户 `users`、成员与权限 `members`、任务与内容 `tasks`、商家与店铺 `stores`、规则治理 `rules`、模型服务 `models`、存储与对账 `storage`、账务与退款 `finance`、审计中心 `audit`、知识治理 `knowledge`。功能开关页面已下线，旧地址应返回 404。生产验收必须逐域覆盖角色可见性、直接 URL、刷新、前进/后退、query 保留和错误恢复；历史浏览器结果不能替代当前域验收。
