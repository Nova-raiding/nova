...
- [x] `queued/processing/archiving/unknown/succeeded/failed` 均有独立文案和允许动作。（本地状态动作契约与桌面矩阵：`b9d00ed`、`fea660d`、`980f985`；真实 Provider 生命周期仍未验收。）
- [x] unknown 不提供直接重试；retryable 失败使用同一幂等请求。（应用/API 状态动作与重试幂等契约：`b9d00ed`、`fea660d`。）
- [x] 候选卡同时呈现归档、扫描、真实性、人工审核状态。（本地候选 gate 字段、状态文案和证据门禁测试：`182d7cf`、`fea660d`；不代表真实 Provider/对象存储证据。）
- [x] 未过门禁的候选不可选择；用户能看到阻断原因。（候选禁用、`aria-describedby` 和阻断原因测试：`182d7cf`。）
- [x] 候选使用稳定 ID key，图片有比例占位和单图失败恢复。（`182d7cf`、`819f705`、`980f985`；非首屏懒加载的完整性能证据仍未闭合。）
- [x] 候选非首屏懒加载在完整桌面性能矩阵中验证。（本地 `image-generation-desktop-responsive.spec.js` 已覆盖 1280、1440、1920 宽度，并断言首张候选优先加载、其余候选延迟加载；真实网络条件、CDN 命中率和生产性能仍不在本地证据范围内。）
- [x] 首次加载、后台刷新、错误和终态均有可访问反馈，且不抢焦点。（桌面矩阵与错误恢复/键盘测试：`980f985`、`182d7cf`；真实读屏和生产网络故障仍未闭合。）
- [ ] 桌面浏览器真实 API 验收通过；API/模型中转缺配置时显示明确阻断。（本地新增 API 503 桌面错误恢复测试：任务读取错误可见、使用 `alert` 播报、焦点移至错误摘要并提供“刷新任务状态”；真实 API/模型中转凭据仍未验收。）
