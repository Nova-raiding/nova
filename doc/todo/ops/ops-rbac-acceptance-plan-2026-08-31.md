...
- **本地已完成一个可重放切片：** `brand.extract` 与 `POST /v1/brand-profile/extract` 通过真实 Bearer 身份共享同一授权决策；覆盖 review-only allow、显式 deny、跨租户 fail-closed、双 transport request/trace/decision audit 及素材证据不泄露。见 `apps/api/src/brand-extract-http-mcp-parity.acceptance.test.ts`。该切片不代表 P1-GATE-004 的全量生产 OIDC/RLS 验收。
...
- [ ] **P1-BE-010：RLS 最终防线。** 使用真实 production-like app/ops DB roles 执行正负 SQL probe；应用 allow + RLS scope mismatch 仍拒绝，`merchant_ops + platform_scope` 只能读批准的聚合/脱敏视图，不能读取客户正文表。 本地增量已补齐 durable grant 的数据库精确 workspace scope trigger 与跨租户负向 probe（migration 152、`authorization-grant-scope-integrity.postgres.test.ts`）；完整 production-like role/RLS 矩阵仍未闭合。
...
- [x] **临时授权禁止 scope promotion（本地 P0 修复）。** `9dd9f9b`；brand resource atom 只能从非 `temporary_grant` 的 workspace capability 派生，防止 workspace 临时授权被重标记为 `resource_grant` 后绕过 max-use 消费、撤销/过期与 revision 执行前复核。单测证明纯 temporary grant 不可派生，正常 workspace membership 仍可作为能力来源；定向 2/2、contracts/server 78/78 通过。精确 brand 临时授权仍由原始 atom 匹配与消费链处理。P1-BE-003 保持 TODO。
- [x] **ChatGPT bridge 授权 decision evidence 透传（本地 P1 修复）。** `d2e6de0`；插件 bridge 的安全 details 白名单新增 `decision_id`、`capability`、`reason_code`、`required_scope`、`workbench`、`explicit_deny`、`obligations_missing` 与 `policy_version`，使 ChatGPT 插件可关联 API 拒绝与服务端审计。测试同时证明任意 authorization 值和内部路径不会进入 structuredContent；定向 2/2、bridge 全套 65/65 通过。用户端仍仅为 ChatGPT 插件真实链路。P1-BE-003 保持 TODO。
- [x] **Task/content 多资源一致性 fail-closed（本地 P1 修复）。** `2a3f5e0`（resolver）+ `e8f6513`（并发提交中包含回归断言）；当请求同时携带 `task_id` 与 `content_version_id` 时，服务端加载两者并要求 content version 归属同一 task，任一资源缺失、跨 workspace 或 task 不一致均保持 unresolved，由统一 evaluator 以 `AUTHZ_SCOPE_MISMATCH` 拒绝。图片生成回归证明 owner 也不能用可见 task 搭配隐藏品牌 content version，拒绝前后 image job 集合不变；品牌场景 1/1、contracts/server 78/78 通过。并发暂存归属已用 blame 核验且未改写共享历史；task/product 一致性与 canonical-only brand 解析仍待封口，P1-BE-003 保持 TODO。
- [x] **Task/product 多资源一致性 fail-closed（本地 P1 修复）。** `af5eb55`；当 exact-brand 请求同时携带 `task_id`/`content_version_id` 所解析的 task 与 `product_id` 时，服务端要求 task 的 product relation 与请求商品完全一致；商品缺失、跨 workspace 或关系不一致保持 unresolved，并在 handler 与图片任务入队前以 `AUTHZ_SCOPE_MISMATCH` 拒绝。owner 跨商品组合回归断言 image job 集合不变；品牌场景 1/1、contracts/server 127/127 通过。canonical-only brand 解析仍待封口，P1-BE-003 保持 TODO。
- [x] **Product brand scope 仅信任 canonical 关系（本地 P1 修复）。** `3402a50`；exact-brand resolver 不再用 legacy `product.brandId` 作为 canonical 关系缺失时的授权回退，只在当前 workspace 的 canonical product source relation 唯一解析到一个品牌时派生 brand scope；无映射与多品牌歧义统一 fail-closed。回归故意给无 canonical 映射商品写入 legacy `brandId`，证明 workspace owner 仍在创意 handler、钱包和模型调用前以 `AUTHZ_SCOPE_MISMATCH` 拒绝；品牌场景 1/1、contracts/server 127/127 通过。P1-BE-003 保持 TODO。
- [x] **发布任务详情 exact brand viewer scope（本地切片）。** `e24d457`（并行 owner 提交中包含 `publish_job_id → publish job → task → brand` resolver 与 HTTP workspace 绑定）+ `153886c`（policy、HTTP 映射与回归）；`publish.get` 从 workspace 迁移为 brand read，MCP 和 `GET /v1/publish-jobs/{jobId}` 的隐藏品牌任务均在状态刷新和回执投影前以 `AUTHZ_SCOPE_MISMATCH` 拒绝。无单任务 ID 的 `GET /v1/publish-jobs` 明确映射到 workspace 级 `publish.batch.get`，继续由列表实现按品牌授权过滤，避免 exact resolver 把合法集合查询全部拒绝。品牌场景 1/1、contracts/server/coverage 138/138 通过；并发归属已用 blame 核验且未改写共享历史。发布写操作的 publisher 最低角色仍待迁移，P1-BE-003 保持 TODO。
- [x] **内容审批 exact brand publisher scope（本地切片）。** `c4b7a28`；`content.approve` 从 workspace 迁移为 brand write，统一 brand atom 派生对 `customer.publish.execute` 明确要求 publisher，普通内容 write 仍要求 editor、read 仍要求 viewer。单元契约锁定三类最低角色；运行回归证明仅有 editor 的品牌成员在规则评估、审批状态和持久化前以 `AUTHZ_SCOPE_MISMATCH` 拒绝，拒绝前后 content version 完整快照一致。品牌场景 1/1、contracts/server/coverage 139/139 通过。publish.prepare/confirm 及批量发布的多品牌 all-of 授权仍待迁移，P1-BE-003 保持 TODO。
- [x] **发布准备 exact brand publisher scope 与 HTTP parity（本地切片）。** `ec79491`（HTTP 命名路径资源绑定及 path/body 冲突 fail-closed）+ `a19fcd8`（并行 owner 提交中的 brand policy、publisher 正反例与 MCP/HTTP 回归）+ `d114f8b`（MCP/HTTP 前置 canonical/店铺/规则门禁、task snapshot 与 `publish.prepared` event 原子持久化及失败内存回滚）；editor 在任何 preview 状态写入前以 `AUTHZ_SCOPE_MISMATCH` 拒绝，publisher 对唯一 canonical/listing 商品通过 MCP 与 HTTP 均可进入 `publish_prepared`。拒绝路径 task 与 publish job 快照不变；品牌场景 1/1、contracts/server/coverage 140/140、全项目 typecheck 通过。并发归属已用 blame 核验且未改写共享历史；真实 PostgreSQL 故障恢复与 ChatGPT 宿主证据仍属于上线门禁，`publish.confirm` 及批量 all-of 仍待迁移，P1-BE-003 保持 TODO。
- [x] **MCP 发布确认 durable task 状态一致性（本地 P1 修复）。** `7c57f13`；`publish.confirm` 使用 deferred memory commit 时，持久事务不再保存旧的 `publish_prepared` task，而与 HTTP 路径一致保存 `publishing/version+1` task、queued publish job 和 `publish.requested` outbox，事务成功后才提交内存索引。MCP 完整 catalog→publish 流程 1/1 通过。真实 PostgreSQL 提交后进程崩溃、重启恢复及内存 commit 竞态仍属于上线门禁，P1-BE-003 保持 TODO。
- [x] **发布确认 exact brand publisher 与 worker 撤权复核（本地 P0/P1 修复）。** `d63214b`；`publish.confirm` 从 workspace 迁移为 brand scope并保留 confirmation/idempotency obligations。task-derived brand 只接受 `task.productId → 唯一 canonical relation → brand` 且必须与 task brand 一致；生产 worker snapshot 接受 exact brand publish decision并以 `brand:<id>` 绑定 context，membership 来源在首次平台写入前重新要求当前 publisher 权限，workspace owner 保持全品牌语义，其他 worker capability 不扩权。MCP/HTTP editor 均在确认票、钱包、容量槽与 job/outbox 前以 `AUTHZ_SCOPE_MISMATCH` 拒绝且 publish job 集合不变；品牌场景 1/1、contracts/server/coverage 142/142、MCP 完整发布流程 1/1、全项目 typecheck 通过。HTTP 一次性票据 parity、模型成本证据冻结、真实 PostgreSQL/ChatGPT/平台回读仍是上线阻断，P1-BE-003 保持 TODO。
- [x] **HTTP 发布确认一次性票据 parity（本地安全优先切片）。** `0e90a1f` + `38aaf29`；`POST /v1/publish-jobs` 对首次幂等请求在钱包、容量槽、job/outbox 写入前消费与 MCP 相同的 actor/session/intent 绑定票据，已存在的同键 job 重放不重复消费。OpenAPI 同步暴露票据字段，并准确标注当前 64 位 bearer 语义。API server E2E 全套 60/60、OpenAPI contract 1/1、全项目 typecheck 通过。当前实现仍是消费后失败必须重新确认的 fail-closed 语义；`reserve → durable commit → finalize/release`、bearer 服务端摘要存储及 HTTP/MCP 完整并发重放矩阵仍待独立迁移。当前运行环境数据库停留 v132、API 镜像要求至少 v135、源码迁移到 v137，`/readyz` 因 schema 落后返回 503；真实 ChatGPT 宿主到 MCP、模型中转成本证据及平台回读均未完成，因此 P1-BE-003 与整体发布门禁继续保持 TODO / PRODUCTION NO-GO。
