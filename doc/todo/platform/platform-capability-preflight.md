...
```sh
npm run evidence:validate -- --file doc/todo/platform/platform-capability-evidence.example.json
...
六个平台可以使用统一门禁。它会逐个平台运行真实 canary、合并候选 evidence，再调用固定路径 `/usr/local/libexec/merchant/attest-capability-evidence` 由受保护控制面补齐 release/image-set/manifest/Git/nonce 绑定与 Ed25519 签名；可选执行真实 PostgreSQL Worker 重启验收。应用容器和仓库脚本不持有私钥；社交平台只有在补齐各自官方适配器证据后才能通过该门禁：
...
RELEASE_IMAGE_SET_DIGEST="$RELEASE_IMAGE_SET_DIGEST" \
PLATFORM_CANARY_BASE_EVIDENCE=/secure/evidence/platform-capability-evidence.base.json \
...
runner 默认要求 `jd,taobao,tmall,pinduoduo,xiaohongshu,douyin` 六个平台全部通过；任何一个平台、支付配置、证据、外部 attester 或 Worker 验收失败，都不会写出带正式签名的 production-ready 证据。写入/撤销 canary 仍需额外设置原有的双重确认变量。

`PLATFORM_CANARY_BASE_EVIDENCE` 必须是同一 release 的六平台证据矩阵；单平台 `test:platform-canary` 只生成候选 evidence，不能代替正式签名。最终部署路径会对 attester 输出再次执行 `--require-canary --require-signed-production`，核对同一 release、image-set、manifest、Git、nonce、固定公钥和 key ID。
