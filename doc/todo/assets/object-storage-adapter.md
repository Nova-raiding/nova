...
### 2026-08-31 上传入口 MIME 规范化增量

- REST 二进制上传入口现在先将 `Content-Type` 规范化为小写再进入对象存储和素材快照流程，兼容大小写不同但语义合法的 MIME，避免重传被错误判为 `OBJECT_METADATA_INVALID`。
- 对象存储文件名也统一做 Unicode 规范化和小写 canonicalization；展示层仍保留商家原始文件名，避免 macOS 大小写不敏感文件系统与大小写敏感云对象 key 之间产生 metadata key 冲突。
- 相关存储/API/application/契约/plugin 回归 298/298 通过，TypeScript build 和 `git diff --check` 通过；真实云 bucket、KMS、生命周期、恢复与 RLS 证据仍未完成，本文件继续保持 TODO。
