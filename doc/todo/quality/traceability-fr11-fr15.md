...
核对日期：2026-08-24  
核对范围：`doc/PRD-merchant-marketing-codex-final.md` v1.4 Final、仓库源码、测试、部署模板和现有交付文档  
...
PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:796-814`。
...
| 查看、字段/段落 diff、候选版本、设为当前、恢复旧版本 | 已完成（代码/本地验收） | 版本 HTTP 验收、`content.diff`、恢复创建新版本；`doc/todo/quality/implementation-status.md:31-33,148` | 无任意分支合并 UI 属于 PRD 明确非 P0，不是缺口 |
...
PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:816-844`。

| PRD 要求 | 状态 | 已核对证据 | 缺口/外部依赖 |
|---|---|---|---|
| Markdown、JSON、ZIP 导出 | 已完成（代码/测试） | `packages/application/src/service.ts` 导出路径；应用测试覆盖 manifest/ZIP | 需在最终构建产物中固定版本和 checksum |
| README、content、manifest、review-findings、source-map | 已完成 | `doc/todo/quality/implementation-status.md:32,216`；导出实现已包含这些文件 | 需检查生产对象存储下载链路和权限 |
...
交付包生成能力基本具备；视频 provider 完成结果已绑定 durable archive contract，provider URL/job 不会单独被视为可恢复资产。但“真实平台成功回执包”、云对象版本恢复和生产视频 provider 证据仍是外部依赖；当前本地 fixture 发布只能得到 `submitted`/`simulated`，不能满足 PRD 的最终 published 验收。

## 4. FR-13 历史、搜索与反馈

PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:846-860`。

| PRD 要求 | 状态 | 已核对证据 | 缺口/外部依赖 |
|---|---|---|---|
| 按品牌、店铺、商品、SKU、平台、状态、日期搜索 | 已完成（代码/测试） | `doc/todo/quality/implementation-status.md:64` 列出历史检索覆盖；API/Repository 有筛选索引 | 需用真实大数据量工作区验证查询延迟 |
...
PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:862-876`。
...
| 试点排障 Runbook 和导出工具 | 部分完成 | `doc/todo/release/production-ops-runbook.md`、导出 API | 未完成真实值班演练、回滚演练和权限验证 |
...
PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:878-929`。
...
依据：`doc/todo/platform/platform-capability-preflight.md:5-16` 明确 fixture 只证明 contract，不证明官方 API；`doc/todo/platform/platform-capability-preflight.md:74-91` 明确 `production_canary` 必须有 application/store、scope、API 版本、evidence ref、验证人和真实配置。
...
证据：`packages/application/src/version-vector.test.ts:5-31`、`doc/todo/quality/implementation-status.md:31-34`。
...
PRD 原文位置：`doc/PRD-merchant-marketing-codex-final.md:1030-1086、1200-1213、1363-1372、1472-1490`。
...
直接证据：`doc/todo/infra/cloud-resources-and-deployment.md:224-245` 明确真实云模式、`cloudGate=true`、零 mock 和全阈值要求；`doc/todo/infra/capacity-evidence.example.json` 是示例，且 `cloud_gate=false`、`platform_mock_ratio=1`、`model_mock_ratio=1`。
...
3. 按 `doc/todo/platform/platform-capability-preflight.md` 逐 profile 跑 authorize/read/full_sync/incremental/create/update/query_status/revoke，并保存非敏感原始 evidence。
...
本报告没有修改业务代码，也没有把本地测试、fixture、loopback 或 Compose 结果升级为生产完成。现有交付文档本身也明确指出真实平台 canary、真实云容量、托管基础设施和运维实操仍需外部完成：`doc/todo/release/release-handoff-0.1.0.md:15-34`、`doc/todo/platform/platform-capability-preflight.md:16,72,91`、`doc/todo/infra/cloud-resources-and-deployment.md:224-245`。
