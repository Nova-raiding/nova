...
如果附件自动解析失败，素材不会被删除或覆盖。先按提示重试读取；若格式不支持或重试耗尽，可调用人工事实确认，填写事实和原因后继续。解析失败只表示自动读取没有完成，不代表上传失败，也不代表 OCR 已成功。

素材列表在配额仓储可用时还会显示已用、预留、上限和可用空间；接近上限会提示先清理历史版本，超额时上传和生成会被阻断。该状态只反映当前工作区配额，不代表对象存储或云服务已完成生产配置。
...
- `IMAGE_MODEL`：图片模型名称。
- `IMAGE_GENERATION_PATH`、`IMAGE_EDIT_PATH`：可选的中转站相对路径，默认均为 `/images/generations`；只允许安全的相对路径。
