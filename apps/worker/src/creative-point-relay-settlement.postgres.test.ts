...
describe('creative point relay settlement PostgreSQL E2', () => {
  postgresIt('keeps a provider reservation active when API-owned settlement proof is absent', async () => {
    ...
    try {
      ...
      const appRoleSql = await readFile(new URL('../../../infra/local/ensure-app-role.sql', import.meta.url), 'utf8')
      await database.query(appRoleSql)
