...
export function CanonicalBackfillConflictSection({ enabled = false, canUpdate = false, brands = [], onScan }: { enabled?: boolean; canUpdate?: boolean; brands?: BrandNavigationItem[]; onScan?: () => Promise<void> | void }) {
  ...
  return <Card title="Canonical 回填人工冲突队列" extra={<Space><Button icon={<ScanOutlined />} loading={scanState === "running"} onClick={() => void scan()}>扫描并刷新队列</Button><Button icon={<ReloadOutlined />} loading={loading} onClick={() => void load()}>刷新队列</Button></Space>}>
    ...
    {!canUpdate && <Alert type="info" showIcon message="当前为只读权限" description="你可以查看冲突及其状态，但缺少 canonical.backfill.update，不能认领或处理冲突。" style={{ marginBottom: 12 }} />}
    {scanState === "running" && <Alert type="info" showIcon role="status" message="正在扫描一致性" description="正在读取当前工作区的 canonical 关系并刷新冲突队列；扫描不会修改商品关系。" style={{ marginBottom: 12 }} />}
    {scanState === "success" && <Alert type="success" showIcon role="status" message="一致性扫描已完成" description="队列已按最新服务端结果刷新；空结果不代表全量回填已完成。" style={{ marginBottom: 12 }} />}
    {scanState === "error" && <Alert type="error" showIcon role="alert" message="一致性扫描失败" description={scanError || "服务端未返回可验证结果，当前不应据此判断没有冲突。"} action={<Button size="small" onClick={() => void scan()}>重试扫描</Button>} style={{ marginBottom: 12 }} />}
    {error && <Alert role="alert" type="error" showIcon message="冲突队列操作失败" description={error} action={<Button size="small" onClick={() => void load()}>重试</Button>} />}
    ...
    <Modal title="处理 canonical 回填冲突" open={Boolean(selected)} okText="确认并提交处理" cancelText="取消" okButtonProps={{ disabled: !canSubmitConflictResolution({ canUpdate, brandId, evidenceConfirmed, note }), loading }} onCancel={closeResolution} onOk={() => void resolve()}>
      {selected && <Space direction="vertical" style={{ width: "100%" }} size={12}>
        <Alert type="warning" message="这是人工确认后的受限商品事实修复" description="仅 MISSING_BRAND 会在同一事务中写入明确品牌并重新校验；revision 或商品版本过期时，服务端会拒绝提交。" />
        ...
        {!canUpdate && <Alert type="info" showIcon message="当前账号无处置权限" description="可查看证据，但不能提交解决或驳回。" />}
        {canUpdate && !brands.length && <Alert type="error" showIcon role="alert" message="没有可验证的品牌范围" description="当前工作区没有返回品牌选择，已阻止提交，避免把冲突归入错误品牌。" />}
        ...
        {resolutionError && <Alert type="error" role="alert" showIcon message="未提交处理结果" description={resolutionError} />}
      </Space>}
    </Modal>
  </Card>;
}
