...
export function ProductSpreadsheetImport({ workspaceId, canWrite, platformScope }: { workspaceId?: string; canWrite: boolean; platformScope: boolean }) {
  ...
  return <Card title="商品与 SKU · Excel 导入">
    <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
      ...
      {!!rows.length && <><Typography.Text>预览：{products.length} 个商品，{rows.length} 行 SKU / 商品记录</Typography.Text><Table size="small" dataSource={rows} pagination={{ pageSize: 8 }} scroll={{ x: 950 }} columns={[{ title: "商品", dataIndex: "title" }, { title: "货号", dataIndex: "productKey" }, { title: "SKU编码", dataIndex: "sku" }, { title: "颜色", dataIndex: "color" }, { title: "尺码", dataIndex: "size" }, { title: "价格（元）", dataIndex: "price" }, { title: "库存", dataIndex: "stock" }, { title: "图片数", dataIndex: "images" }]} /><Button type="primary" disabled={!enabled || busy || !!imported.length} loading={busy} onClick={() => void commit()}>{imported.length ? "已导入" : "确认预览并导入"}</Button></>}
