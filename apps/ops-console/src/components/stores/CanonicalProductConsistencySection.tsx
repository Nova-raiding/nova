...
export function CanonicalProductConsistencySection({ report, onRefresh, onNextAction, loading = false, canRead = true }: { report?: PresentationReport; onRefresh?: () => void; onNextAction?: (finding: CanonicalProductConsistencyReport["findings"][number]) => void; loading?: boolean; canRead?: boolean }) {
  ...
  return <>
    <Card title={<Space>规范商品一致性 <Tag color={loading ? "processing" : hasAttention ? "warning" : "success"}>{loading ? "检查中" : hasAttention ? "需处理" : "已验证"}</Tag></Space>} extra={<Button icon={<ReloadOutlined />} loading={loading} onClick={onRefresh}>重新检查</Button>}>
      ...
      {report.error && <Alert role="alert" showIcon type="error" icon={<ExclamationCircleOutlined />} title="一致性报告读取失败" description={report.error.message ?? report.error.code ?? "服务端未返回可用报告"} action={<Button size="small" aria-label="重试" onClick={onRefresh} loading={loading}>重试</Button>} style={{ marginBottom: 12 }} />}
      ...
      <Space orientation="vertical" style={{ width: "100%", marginTop: 20 }} size={12}>
        ...
        {orphanFindings.length > 0 && <>
          ...
          <Table rowKey={(row) => `${row.entityType}:${row.entityId}`} size="small" pagination={{ pageSize: 10, showSizeChanger: false }} dataSource={orphanFindings} columns={[
            ...
            { title: "操作", render: (_: unknown, row: CanonicalProductConsistencyReport["orphanFindings"][number]) => <Button type="link" aria-label={`查看 ${row.entityId} 关系详情`} onClick={() => setSelectedOrphan(row)}>查看详情</Button> },
          ]} />
        </>}
      </Space>
    </Card>
