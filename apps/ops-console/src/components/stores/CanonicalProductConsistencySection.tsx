...
export function CanonicalProductConsistencySection({ report, onRefresh, onNextAction, loading = false, canRead = true }: { report?: PresentationReport; onRefresh?: () => void; onNextAction?: (finding: CanonicalProductConsistencyReport["findings"][number]) => void; loading?: boolean; canRead?: boolean }) {
  ...
  return <>
    <Card className="canonical-consistency-card" title={<Space>规范商品一致性 <Tag color={loading ? "processing" : hasAttention ? "warning" : "success"}>{loading ? "检查中" : hasAttention ? "需处理" : "已验证"}</Tag></Space>} extra={<Button className="canonical-consistency-action" aria-label="重新检查一致性报告" icon={<ReloadOutlined />} loading={loading} onClick={onRefresh}>重新检查</Button>}>
      ...
      {report.error && <Alert role="alert" showIcon type="error" icon={<ExclamationCircleOutlined />} title="一致性报告读取失败" description={report.error.message ?? report.error.code ?? "服务端未返回可用报告"} action={<Button className="canonical-consistency-action" size="small" aria-label="重试一致性报告" onClick={onRefresh} loading={loading}>重试</Button>} style={{ marginBottom: 12 }} />}
      ...
      {(unavailable || uncertain) && <Alert role="alert" showIcon type="error" title={unavailable ? "一致性数据暂不可读取" : "一致性数据尚未确认"} description={unavailable ? "这不是零结果；服务端没有返回可验证数据。请重试或联系具备权限的运营人员，当前禁止继续相关发布动作。" : "当前读取结果尚不确定，不能视为已验证。请重新检查后再处理。"} action={<Button className="canonical-consistency-action" size="small" aria-label="重新检查一致性数据" onClick={onRefresh} loading={loading}>重新检查</Button>} style={{ marginBottom: 16 }} />}
      {hasAttention && <Alert showIcon type="warning" icon={<ExclamationCircleOutlined />} title="存在未验证关系" description={<div><div>请打开具体商品查看稳定错误码、影响范围和下一步；不要把数量摘要当作全部一致。</div>{(errorCodes.length > 0 || report.error) && <div id="canonical-consistency-error-summary" ref={errorSummaryRef} className="canonical-error-summary" role="alert" aria-live="assertive" aria-atomic="true" tabIndex={-1} aria-labelledby="canonical-consistency-error-summary-label"><span id="canonical-consistency-error-summary-label">错误摘要：</span>{errorCodes.map((code) => <Tag key={code}>{codeMessage(code)}</Tag>)}{report.error && <Tag>{report.error.code ?? "CONSISTENCY_READ_FAILED"}</Tag>}<Typography.Text type="secondary">请修复后重新检查；错误状态不会自动放行后续操作。</Typography.Text></div>}</div>} style={{ marginBottom: 16 }} />}
