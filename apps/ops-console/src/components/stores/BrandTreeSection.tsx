...
export function BrandTreeSection({ brands = [], canRead = true, canCreate = false, stores = [], canBind = false, loading = false, error = "", onRetry, onOpenStore, onCreateBrand, onBindStore }: BrandTreeSectionProps) {
  ...
  const submitCreate = async () => {
    ...
    setCreateError("");
    try {
      const completed = await onCreateBrand(name);
      if (completed) setBrandName("");
      else setCreateError("请求未完成，品牌尚未创建；请检查权限或稍后重试");
    } catch (cause) {
    ...
    <Card title={<Space><ApartmentOutlined aria-hidden="true" />品牌、平台与店铺</Space>}>
      {!canRead ? (
        <div className="ops-data-state" data-state="forbidden">
          <Alert role="alert" type="warning" showIcon title="当前账号无权读取品牌树" description="这不是空结果；服务端已按品牌访问权限隐藏品牌明细。请切换到已授权的工作区或联系管理员授予品牌读取权限。" />
        </div>
      ) : error ? (
        ...
        <OpsDataState state="empty" description="当前工作区还没有可访问的品牌">
          {canCreate && onCreateBrand && <Space orientation="vertical" size={8} style={{ width: "100%" }}>
            ...
            {createError && <Alert role="alert" type="error" showIcon title="创建品牌失败" description={createError} />}
          </Space>}
        </OpsDataState>
      ) : (
