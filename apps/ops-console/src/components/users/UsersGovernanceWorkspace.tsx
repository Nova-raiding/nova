...
export function RegistrationApplications({ model }: { model: OpsConsoleModel }) {
  ...
  const load = async () => { setLoading(true); setLoadError(""); try { const result = await opsRestGet<{ items: Registration[] }>("/v1/ops/merchant-registration-applications"); setItems(result?.items ?? []); } catch (e) { const failure = describeOpsError(e); setLoadError(failure); message.error(failure); } finally { setLoading(false); } };
