...
import { campaignActionParams, campaignActionRecoveryCopy, parseCampaignControlSnapshot } from './CampaignLifecycleControl.js'
...
const blockedCampaign = parseCampaignControlSnapshot({ id: 'campaign-blocked', state: 'draft', revision: 3, delivery_manifest: { state: 'blocked', validation: { valid: false, code: 'CAMPAIGN_MANIFEST_INVALID', path: 'items[0].listingId' } }, items: [{ id: 'item-1', state: 'pending' }] })
...
describe('Ops campaign lifecycle control', () => {
  ...
  it('preserves a blocked delivery manifest instead of presenting draft as runnable', () => { expect(blockedCampaign.delivery).toEqual({ state: 'blocked', valid: false, code: 'CAMPAIGN_MANIFEST_INVALID', path: 'items[0].listingId' }) })
  it('provides an explicit, action-specific recovery label without auto-retrying', () => {
    expect(campaignActionRecoveryCopy('pause')).toBe('再次提交暂停操作')
    expect(campaignActionRecoveryCopy('resume')).toBe('再次提交恢复操作')
    expect(campaignActionRecoveryCopy('retry_failed')).toBe('再次提交失败项重试')
    expect(campaignActionRecoveryCopy(undefined)).toBe('再次提交操作')
  })
})
