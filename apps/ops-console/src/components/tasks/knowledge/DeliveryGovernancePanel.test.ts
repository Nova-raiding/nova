...
describe('ops delivery readiness', () => {
  const gate = { status: 'unverified' as const, authenticityGate: { status: 'unverified' as const, reason: '未返回' }, realRender: { status: 'unverified' as const, reason: '未返回' }, ocr: { status: 'unverified' as const, reason: '未返回' }, humanAttestation: { status: 'unverified' as const, reason: '未返回' }, bundleHash: { status: 'unverified' as const, reason: '未返回' } }
  ...
  it('distinguishes approved and expired evidence', () => {
    ...
    expect(opsMediaReadiness({ ...base, expiresAt: '2026-09-01T00:00:00Z' }, Date.parse('2026-08-29T00:00:00Z'), 'passed')).toBe('approved')
  ...
  it('renders explicit API empty states without a success label', () => {
    const html = renderToStaticMarkup(createElement(DeliveryReadinessCards, { state: { loaded: true, loading: false, data: { generatedAt: '2026-08-29T00:00:00Z', status: 'unverified', dimensions: { mapping: 'unverified', bundles: 'unverified', authenticity: 'unverified' }, gate, mappingPreflights: [], bundles: [], authenticity: [] } } }))
