...
export function assetStatusLabel(value: string | undefined): string {
  return ({
    clean: "已通过",
    blocked: "已阻断",
    failed: "检查失败",
    ...
    <Card title="上传素材治理动作" size="small">
        ...
        columns={[
          { title: "素材", dataIndex: "name" },
          {
            ...
            render: (_: unknown, row: UploadedAssetRisk) =>
              `${assetStatusLabel(row.scanStatus)} / ${assetStatusLabel(row.parseStatus)} / ${assetStatusLabel(row.rightsStatus)}`,
          },
          {
            title: "就绪状态",
            ...
            render: (_: unknown, row: UploadedAssetRisk) => (
              ...
              >
                {assetStatusLabel(row.readiness.status)}
              </Tag>
            ),
          },
          {
      ...
      <Modal title={`执行素材治理 · ${target?.name ?? ""}`} open={Boolean(target)} okText="提交治理动作" cancelText="取消" confirmLoading={submitting} okButtonProps={{ disabled: !canSubmit }} onCancel={close} onOk={() => void submit()}>
        ...
        {method === "asset.rights.update" && <Space orientation="vertical" style={{ width: "100%" }}>
          ...
          <label htmlFor="asset-rights-scope">权益范围</label><Select id="asset-rights-scope" value={rightsScope} onChange={setRightsScope} options={[{ value: "owned", label: "自有" }, { value: "commercial_authorized", label: "商业授权" }, { value: "limited_use", label: "限制使用" }, { value: "internal_only", label: "仅内部" }, { value: "unknown", label: "待确认" }, { value: "unusable", label: "不可用" }]} />
        </Space>}
