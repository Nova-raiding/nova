...
export function OpsDataState({ state, children, title, description, onRetry, retryLabel = "重试" }: OpsDataStateProps) {
  ...
  if (state === "error") {
    return <div className="ops-data-state" data-state="error"><Alert role="alert" aria-live="assertive" type="error" showIcon title={title || "数据加载失败"} description={description || "暂时无法读取运营数据，请重试。"} action={onRetry ? <Button aria-label={retryLabel} onClick={onRetry}>{retryLabel}</Button> : undefined} /></div>;
  }
