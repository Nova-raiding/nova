...
interface Props { items: FeatureFlag[]; loading: boolean; canWrite: boolean; canEmergency: boolean; onEdit(flag: FeatureFlag): void; onAudit(flag: FeatureFlag, trigger: HTMLElement): void; onEmergency(flag: FeatureFlag): void }
...
export function FeatureFlagsTable({ items, loading, canWrite, canEmergency, onEdit, onAudit, onEmergency }: Props) {
  const columns: ColumnsType<FeatureFlag> = [
    ...
    { title: "操作", key: "actions", render: (_, row) => <Space wrap>
      <Button aria-label={`查看 ${row.key} 审计`} style={{ minHeight: 44 }} onClick={event => onAudit(row, event.currentTarget)}>审计</Button>
