...
describe("ReconciliationSection finance actions", () => {
  ...
  it("renders unknown reconciliation states as blocked errors, never success", () => {
    ...
    expect(html).toContain("存在运行阻断");
    expect(html).toContain("查看详情");
    expect(html).not.toContain("外部中转站用量：尚未核验");
  });
  ...
  it("never presents fixture or unknown payment providers as ready", () => {
    ...
    expect(fixture).toContain("存在运行阻断");
    expect(fixture).not.toContain("当前为 fixture（已阻断真实支付判断）");
    const unknown = renderSection({ reconciliation: { ...reconciliationFixture, provider: { mode: "unknown", ready: true, reasons: [] } } as never });
    expect(unknown).toContain("存在运行阻断");
    expect(unknown).not.toContain("未就绪或待确认");
  });
