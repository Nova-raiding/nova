...
import { Alert, Button, Card, Checkbox, Drawer, Empty, Form, Input, Progress, Select, Space, Steps, Table, Tag, Typography, Upload, message } from "antd";
...
export function CustomerDeliverySection({ records = [], onOpen, onSave, onCreate, onChecklistSave, onChecklistLoad, onTrainingSave, onVideoAdd }: { records?: CustomerDeliveryRecord[]; onOpen?: (record: CustomerDeliveryRecord, step: DeliveryStepKey) => Promise<void> | void; onSave?: (record: CustomerDeliveryRecord) => Promise<CustomerDeliveryRecord | void> | CustomerDeliveryRecord | void; onCreate?: (companyName: string) => Promise<CustomerDeliveryRecord>; onChecklistSave?: (payload: CustomerDeliveryChecklistSave) => Promise<CustomerDeliveryRecord | void> | CustomerDeliveryRecord | void; onChecklistLoad?: (record: CustomerDeliveryRecord, checklistKey: "system_integration" | "functional_acceptance") => Promise<CustomerDeliveryChecklistItem[]>; onTrainingSave?: (record: CustomerDeliveryRecord, completed: boolean) => Promise<CustomerDeliveryRecord | void>; onVideoAdd?: (record: CustomerDeliveryRecord, input: { title: string; assetRef: string; sortOrder: number }) => Promise<CustomerDeliveryRecord | void> }) {
  ...
  const openStep = async (row: CustomerDeliveryRecord, next: DeliveryStepKey) => {
    if (isDeliveryStepBlocked(row.paymentStatus, next)) {
      setBlockedCompany(row.companyName);
      return;
    }
    setBlockedCompany(undefined);
    setSelected(row); setStep(next); form.setFieldsValue({ ...row, integrationItems: row.integrationItems ?? [], acceptanceItems: row.acceptanceItems ?? [], integrationEvidence: row.integrationEvidence ?? {}, acceptanceEvidence: row.acceptanceEvidence ?? {} });
    if ((next === "integration" || next === "acceptance") && onChecklistLoad) {
      const key = next === "integration" ? "system_integration" : "functional_acceptance";
      let items: CustomerDeliveryChecklistItem[];
      try { items = await onChecklistLoad(row, key); } catch (error) { setSelected(undefined); message.error(error instanceof Error ? error.message : "清单读取失败"); return; }
      const selectedItems = items.filter((item) => item.completed).map((item) => item.itemKey);
      const evidence = Object.fromEntries(items.map((item) => [item.itemKey, item.evidence]));
      form.setFieldsValue(next === "integration" ? { integrationItems: selectedItems, integrationEvidence: evidence } : { acceptanceItems: selectedItems, acceptanceEvidence: evidence });
    }
  ...
  const create = async (values: { companyName?: string }) => {
    ...
    setCreating(true);
    try { const record = await onCreate(values.companyName.trim()); setSelected(record); setStep("profile"); form.setFieldsValue(record); createForm.resetFields(); message.success("客户档案已创建"); }
  ...
  const save = async (values: Record<string, unknown>) => {
    ...
    setSaving(true); try {
      ...
      if (step === "integration" || step === "acceptance") {
        ...
        persisted = await onChecklistSave({ record: next, checklistKey: step === "integration" ? "system_integration" : "functional_acceptance", items });
        ...
        const refs = String(values.videoAssetRefs ?? "").split(/[\n,]/u).map((value) => value.trim()).filter(Boolean);
        if (!refs.length) throw new Error("请填写至少一个已上传视频的 asset_ref");
        for (const [index, assetRef] of refs.entries()) persisted = await onVideoAdd(selected, { title: `${selected.companyName} 交付视频 ${selected.videos + index + 1}`, assetRef, sortOrder: selected.videos + index });
      } else if (step === "profile") {
        if (!onSave) throw new Error("客户档案保存接口未配置");
        persisted = await onSave(next);
      }
      const finalRecord = persisted && typeof persisted === "object" ? persisted : next;
      setSelected(finalRecord); message.success("已保存");
  ...
  return <Card title="客户建档" extra={<Space><Typography.Text type="secondary">完成全部交付项后，客户主体才会变为生效</Typography.Text><Button type="primary" onClick={() => { createForm.resetFields(); setShowCreate(true); }}>新建客户</Button></Space>}>
    ...
    {showCreate ? <div style={{ marginTop: 12 }}><Card size="small" title="新建客户档案"><Form form={createForm} layout="inline" onFinish={create}><Form.Item name="companyName" rules={[{ required: true, message: "请输入公司名称" }]}><Input placeholder="公司名称" /></Form.Item><Button htmlType="submit" type="primary" loading={creating}>创建</Button><Button onClick={() => setShowCreate(false)}>取消</Button></Form></Card></div> : null}
    ...
    <Drawer title={selected ? `${selected.companyName} · ${stepLabels[step]}` : "客户交付详情"} open={Boolean(selected)} onClose={() => setSelected(undefined)} width={560}>
      {selected ? <Space orientation="vertical" size="large" style={{ width: "100%" }}><Steps current={Object.keys(stepLabels).indexOf(step)} items={Object.values(stepLabels).map((title, index) => { const key = Object.keys(stepLabels)[index] as DeliveryStepKey; const complete = key === "video" ? selected.videos > 0 : selected[key]; return ({ title, status: complete ? "finish" : index === Object.keys(stepLabels).indexOf(step) ? "process" : "wait" }); })} />
        <Form form={form} layout="vertical" onFinish={save} initialValues={selected}>
          {step === "profile" && <><Form.Item name="companyName" label="公司名称" rules={[{ required: true, message: "请输入公司名称" }]}><Input /></Form.Item><Form.Item name="contractNo" label="合同编号" rules={[{ required: true, message: "请输入合同编号" }]}><Input /></Form.Item><Form.Item name="paymentStatus" label="付款状态" rules={[{ required: true }]}><Select options={[{ label: "已完成付款核验", value: "paid" }, { label: "未支付", value: "unpaid" }]} /></Form.Item><Form.Item noStyle shouldUpdate={(prev, cur) => prev.paymentStatus !== cur.paymentStatus}>{({ getFieldValue }) => <Form.Item name="paymentDate" label="付款日期" rules={[{ required: getFieldValue("paymentStatus") === "paid", message: "已付款客户必须填写付款日期" }]}><Input type="date" /></Form.Item>}</Form.Item><Form.Item name="contractFile" label="合同文件" rules={[{ required: true, message: "请填写合同 asset_ref 或 HTTPS 链接" }, { validator: async (_, value) => { if (typeof value === "string" && (/^https:\/\//u.test(value.trim()) || /^asset[:_]/u.test(value.trim()))) return; throw new Error("合同必须是 HTTPS 链接或已扫描的 asset_ref"); } }]}><Input placeholder="asset_ref 或 https://... 合同链接" /></Form.Item><Typography.Text type="secondary">合同文件请先通过素材中心安全上传；此处只保存已验证的 asset_ref/HTTPS 链接。</Typography.Text><Form.Item name="owner" label="项目负责人" rules={[{ required: true, message: "请输入项目负责人" }]}><Input /></Form.Item><Form.Item name="afterSalesOwner" label="售后负责人" rules={[{ required: true, message: "请输入售后负责人" }]}><Input /></Form.Item><Form.Item name="requiredLaunchAt" label="要求上线时间" rules={[{ required: true, message: "请选择要求上线时间" }]}><Input type="datetime-local" /></Form.Item></>}
          ...
          {step === "training" && <Form.Item name="training" valuePropName="checked"><Checkbox>客户培训已完成（独立记录培训结果）</Checkbox></Form.Item>}
          {step === "video" && <><Form.Item name="videoAssetRefs" label="交付视频（支持多段）"><Input.TextArea rows={4} placeholder="每行一个已上传视频的 asset_ref；支持多段" /></Form.Item><Typography.Text type="secondary">视频必须先通过素材中心安全上传并完成扫描，再填写 asset_ref。页面不会把本地选文件误报为已上传。</Typography.Text><Upload beforeUpload={() => { message.info("请先通过素材中心安全上传，再填写 asset_ref"); return Upload.LIST_IGNORE; }} showUploadList={false}><Button icon={<PlusOutlined />}>选择视频（需安全上传）</Button></Upload></>}
