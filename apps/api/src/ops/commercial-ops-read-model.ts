...
export function projectCommercialCatalogItem(item: CommercialCatalogSkuSnapshot) {
  ...
  return { id: item.versionId, sku_code: item.code, name, type: item.kind, visibility: item.visibility, version: `v${item.version}`, price_fen: item.priceFen, price_label: priceLabel, cycle_label: item.durationDays === null ? null : `${item.durationDays} 天`, benefits_summary: benefitsSummary, benefits: item.benefits.map(benefit => ({ code: benefit.code, quantity: benefit.quantity, raw_value: benefit.rawValue, raw_unit: benefit.rawUnit })), approval_state: item.lifecycle, valid_from: item.effectiveAt, valid_to: null, unresolved: Array.isArray(item.payload.blockers) ? item.payload.blockers.filter((value): value is string => typeof value === 'string') : [], checksum: item.checksum, executable: item.executable }
}
