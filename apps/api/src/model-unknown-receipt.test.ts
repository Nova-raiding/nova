...
describe('unknown model provider receipt', () => {
  it('persists the stable relay attempt key when no provider response id exists', () => {
    ...
    expect(receipt).toMatchObject({ workspaceId: 'ws_one', operationId: 'cpo_one', provider: 'model-relay', providerRequestId: 'unknown:mm-attempt-1', outcome: 'unknown' })
  ...
  it('prefers the provider response id and never represents unknown as success', () => {
    expect(unknownModelProviderReceipt({ code: 'MODEL_PROVIDER_OUTCOME_UNKNOWN', providerOutcome: 'unknown', providerRequestId: 'relay-response', providerIdempotencyKey: 'mm-attempt' }, 'ws_one', 'cpo_one')).toMatchObject({ providerRequestId: 'unknown:relay-response', outcome: 'unknown' })
