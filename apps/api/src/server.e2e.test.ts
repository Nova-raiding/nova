...
describe('API HTTP vertical slice', () => {
  ...
  it('bootstraps a first-run workspace before the Codex plugin binds its workspace id', async () => {
    ...
    expect(startResult).toMatchObject({ greeting: '欢迎使用 Store Nova。', currentStep: { id: 'bind-store', entryMethod: 'platform.connect' }, nextPrompt: '选择一个平台连接我的店铺', modelAccess: { userKeyRequired: false }, wallet: { balance_cny: '0.00', unlocked: true } })
    ...
    expect(billingResult).toMatchObject({ balance_cny: '0.00', plugin_access: { unlocked: true }, model_access: { access_state: 'included_quota_available' }, capability_entitlements: { balance: { state: 'recharge_required' }, package_quota: { state: 'available' }, generation: { state: 'blocked', code: 'model_configuration' }, platform_publish: { state: 'blocked', code: 'store_authorization' } }, action_cards: [
  ...
  it('does not credit a wallet when the provider rejects a refund', async () => {
    ...
    const providerRefund = vi.fn(async () => ({ providerRefundId: 'refund-rejected', state: 'rejected' }))
    setPaymentProviderForTests({
      createCheckout: async () => ({ paymentUrl: 'https://payments.example/pay/order' }),
      queryStatus: async () => ({ state: 'paid' as const, providerTradeId: `trade-${workspaceId}`, amountFen: 1000 }),
  ...
  it('keeps an unknown provider refund reserved until reconciliation confirms success', async () => {
    vi.stubEnv('PAYMENT_MODE', 'provider')
    ...
    const providerRefund = vi.fn(async (_input: { refundRequestId?: string }) => ({ providerRefundId: 'refund-unknown', state: 'processing' }))
    ...
    const headers = { 'content-type': 'application/json', 'x-workspace-id': workspaceId, 'x-actor-id': 'finance_1' }
    const call = (id: number, method: string, params: Record<string, unknown>) => fetch(`${base}/mcp`, { method: 'POST', headers, body: JSON.stringify({ jsonrpc: '2.0', id, method, params: { workspace_id: workspaceId, ...params } }) }).then(json)
    const created = await call(1, 'billing.recharge.create', { channel: 'alipay', amount_cny: '10.00', idempotency_key: `refund-unknown-${workspaceId}` })
    const order = (created.data as { result: { id: string } }).result
    expect((await call(2, 'billing.recharge.get', { order_id: order.id })).error).toBeNull()

    const refund = await call(3, 'billing.refund', { order_id: order.id, reason: 'provider unknown test' })
    ...
    const refundRequestId = providerRefund.mock.calls[0]?.[0].refundRequestId
    expect(refundRequestId).toMatch(/^billing_tx_/u)

    const held = await call(4, 'billing.transactions', {})
    const heldWallet = held.data?.result as { balance_cny: string; transactions: Array<{ type: string; orderId?: string }> }
    expect(heldWallet.balance_cny).toBe('0.00')
    expect(heldWallet.transactions.filter(item => item.type === 'debit' && item.orderId?.startsWith(`recharge-refund:${order.id}:`))).toHaveLength(1)
    expect(heldWallet.transactions.filter(item => item.type === 'refund')).toHaveLength(0)

    const reconciliation = await call(5, 'billing.reconciliation.run', { limit: '10' })
    expect(reconciliation.error).toBeNull()
    expect(reconciliation.data?.result).toMatchObject({ state: 'completed', checked: 1, payment_checked: 0, refund_checked: 1, refund_settled: [{ order_id: order.id, refund_request_id: refundRequestId }], refund_pending: [], refund_failed: [] })
    expect(providerRefundQuery).toHaveBeenCalledWith(expect.objectContaining({ orderId: order.id, refundRequestId, amountFen: 1000 }))

    const replay = await call(6, 'billing.reconciliation.run', { limit: '10' })
  ...
  it('creates a price-snapshotted subscription, activates it after callback, and persists member/audit operations', async () => {
    ...
    expect((subscription.data as { result: { status: string; planCode: string; priceCny: number; commercial_entitlement: { status: string; plan: string; sourceVersion: string; checksum: string } } }).result).toMatchObject({ status: 'active', planCode: 'growth', priceCny: 599, commercial_entitlement: { status: 'available', plan: 'growth', sourceVersion: expect.stringMatching(/^growth:revision-\d+$/u), checksum: expect.stringMatching(/^[a-f0-9]{64}$/u) } })
    ...
    const userDetail = await fetch(`${base}/mcp`, { method: 'POST', headers, body: JSON.stringify({ jsonrpc: '2.0', id: 3.15, method: 'ops.user.detail', params: { workspace_id: workspaceId, external_subject: 'user_1' } }) }).then(json)
    expect((userDetail.data as { result: { memberships: Array<{ workspaceId: string; commercial: { planCode: string; planName: string; subscriptionStatus: string; usedTasks: number; includedTasks: number; remainingTasks: number; walletBalanceCny: string } }> } }).result).toMatchObject({ memberships: [expect.objectContaining({ workspaceId, commercial: { planCode: 'growth', planName: 'Growth', subscriptionStatus: 'active', usedTasks: 0, includedTasks: 150, remainingTasks: 150, walletBalanceCny: '0.00' } })] })
  ...
  it('runs task creation through publish confirmation over HTTP', async () => {
    ...
    expect(modelStatus.data.result.capabilities.knowledge_vector_indexing).toBe(false)
    expect(modelStatus.data.result.embedding_model).toBeNull()
    expect(modelStatus.data.result.model_readiness).toMatchObject({ ocr: { ready: false }, video: { ready: false } })
    expect(modelStatus.data.result.model_readiness.embedding).toMatchObject({ ready: false, provider_configured: false, reasons: ['knowledge_vector_indexing_disabled'] })
