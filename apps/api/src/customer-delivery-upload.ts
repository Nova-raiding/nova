...
const contractMimes = new Set(['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'image/png', 'image/jpeg'])
const videoMimes = new Set(['video/mp4', 'video/webm'])
export type CustomerDeliveryUploadPurpose = 'contract' | 'video'

export function customerDeliveryUploadPurpose(value: unknown): CustomerDeliveryUploadPurpose {
  if (value !== 'contract' && value !== 'video') throw new DomainError('INVALID_REQUEST', '上传用途必须是合同或交付视频', 400)
  return value
}
...
export function validateCustomerDeliveryUpload(input: Record<string, unknown>) {
  ...
  if (!(purpose === 'contract' ? contractMimes : videoMimes).has(mimeType)) throw new DomainError('CUSTOMER_DELIVERY_UPLOAD_TYPE_UNSUPPORTED', purpose === 'contract' ? '合同支持 PDF、DOCX、PNG 和 JPEG' : '交付视频支持 MP4 和 WebM', 400)
...
export function customerDeliveryUploadView(asset: Partial<AssetMetadata> | undefined, purpose: CustomerDeliveryUploadPurpose) {
  ...
  if (!asset || typeof asset.id !== 'string' || typeof asset.name !== 'string' || typeof asset.sizeBytes !== 'number'
    || !(purpose === 'contract' ? contractMimes : videoMimes).has(mimeType)) throw new DomainError('CUSTOMER_DELIVERY_UPLOAD_NOT_FOUND', '文件不存在、不属于当前工作区或上传用途不匹配', 404)
