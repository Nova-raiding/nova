...
describe('authorization decision audit evidence', () => {
  it('fails closed when an enforced decision cannot be attributed to a workspace actor', () => {
    ...
    expect(authorizationDecisionAuditContextIsValid(enforcedDeniedDecision, 'ws_a', undefined)).toBe(false)
    expect(authorizationDecisionAuditContextIsValid(enforcedDeniedDecision, 'ws_a', 'actor_a')).toBe(true)
  })

  it.each([
    ['workspace whitespace', ' ws_a ', 'actor_a'],
    ['workspace control character', 'ws\nforged', 'actor_a'],
    ['actor whitespace', 'ws_a', ' actor_a'],
    ['actor control character', 'ws_a', 'actor\u0000forged'],
    ['non-string actor', 'ws_a', 42],
  ])('rejects %s before durable audit attribution', (_label, workspaceId, actorId) => {
    const result = authorizationDecisionAuditContextIsValid(enforcedDeniedDecision, workspaceId, actorId as string | undefined)
  ...
  it('persists reconstructible scope and obligation evidence without changing the decision', () => {
    const decision: AuthorizationDecision = {
      decision_id: 'decision_audit_1', policy_version: AUTHZ_POLICY_VERSION, method: 'ops.member.upsert',
      capability: 'workspace.member.manage', workbench: 'workspace',
    ...
    expect(authorizationDecisionAuditEvidence(decision, { requestId: 'req_a', traceId: 'trace_a' })).toEqual({
      decision_id: 'decision_audit_1', request_id: 'req_a', trace_id: 'trace_a', policy_version: AUTHZ_POLICY_VERSION,
  ...
  it('copies arrays so audit serialization cannot mutate the authorization decision', () => {
    ...
    const decision: AuthorizationDecision = {
      decision_id: 'decision_audit_2', policy_version: AUTHZ_POLICY_VERSION, method: 'ops.member.upsert' as const,
