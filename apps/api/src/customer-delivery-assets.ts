 ...
 */
export async function requireCustomerDeliveryAsset(input: {
  workspaceId: string
  assetRef: string
  ...
  const ready = asset?.id === input.assetRef && asset.workspaceId === input.workspaceId
    && typeof asset.storageKey === 'string' && typeof asset.scanReceiptId === 'string'
