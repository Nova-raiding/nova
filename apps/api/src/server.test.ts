...
describe('central commercial access gate', () => {
  ...
  it('returns ops commercial DTOs in the desktop parser shape', () => {
    ...
    const adjustment = source.slice(source.indexOf("case 'ops.commercial.points.adjust.propose':"), source.indexOf("case 'ops.commercial.orders-v2.list':"))
    expect(adjustment).toContain("case 'ops.commercial.points.adjust.decide':")
    expect(adjustment).toContain('proposeCommercialPointAdjustment')
    expect(adjustment).toContain('decideCommercialPointAdjustment')
    expect(adjustment).toContain('requestActor(req)')
...
describe('API application wiring', () => {
  ...
  it('projects real Provider dispatch states through the image list and reconciliation APIs', () => {
    ...
    expect(source).toContain("reconciliation_required: !candidatesReadable || execution?.state === 'provider_reserved' || execution?.state === 'provider_dispatching'")
  ...
  it('requires exact image settlement evidence before callbacks, candidate reads, reviews or bindings', () => {
    ...
    const callbackStart = source.indexOf('const imageGenerationResultMatch = path.match')
    const callbackEnd = source.indexOf('const imageGenerationExecutionMatch = path.match', callbackStart)
    const reconciliationStart = source.indexOf('const imageGenerationEvidenceMatch = path.match')
    const reconciliationEnd = source.indexOf("if (req.method === 'POST' && path === '/v1/internal/model-usage')", reconciliationStart)
    const selectStart = source.indexOf("case 'catalog.image.select':")
    const selectEnd = source.indexOf("case 'catalog.image.review':", selectStart)
    const reviewStart = selectEnd
    const reviewEnd = source.indexOf("case 'content.visual.select':", reviewStart)
    const bindingStart = reviewEnd
    const bindingEnd = source.indexOf("case 'sync.retry_failed':", bindingStart)
    ...
    expect(source).toContain('publicImageJobForCommercialRead(job, candidatesReadable)')
    expect(source).toContain('!candidatesReadable || execution?.state')
  })
