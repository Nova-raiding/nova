...
describe('storage reconciliation API contract', () => {
  it('returns only redacted workspace status and never object identifiers', () => {
    const result = redactStorageReconciliation({
      ...
      counts: { references: 1, inventoryObjects: 2, matched: 0, missing: 1, metadataMismatches: 0, orphans: 1, crossWorkspace: 0, duplicates: 0 },
  ...
  it('marks old reports as expired instead of implying current health', () => {
    const result = redactStorageReconciliation({
      ...
      counts: { references: 1, inventoryObjects: 1, matched: 1, missing: 0, metadataMismatches: 0, orphans: 0, crossWorkspace: 0, duplicates: 0 }, findings: [],
    })
