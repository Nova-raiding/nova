...
describe('Codex plugin installation package', () => {
  it('packages the connection helper source without shipping an unsigned custom-scheme app', () => {
    ...
    try {
      ...
      expect(installer.stdout).toContain('scripts\\ensure-chatgpt-windows.ps1')
      expect(installer.stdout).toContain('install-plugin.ps1')
      const pluginInstaller = spawnSync('tar', ['-xOzf', artifact, 'install-plugin.ps1'], { encoding: 'utf8' })
      expect(pluginInstaller.status, pluginInstaller.stderr).toBe(0)
      expect(pluginInstaller.stdout).toContain('scripts\\login-local-windows.mjs')
      expect(pluginInstaller.stdout).not.toContain('SetEnvironmentVariable("MERCHANT_WORKSPACE_ID"')
      expect(pluginInstaller.stdout).not.toContain('ws_guirenniaoniao')
      expect(pluginInstaller.stdout).toContain('StoreNovaCredentialHelper.exe')
      expect(pluginInstaller.stdout).toContain('Get-FileHash')
      expect(pluginInstaller.stdout).toContain('Get-AuthenticodeSignature')
      expect(pluginInstaller.stdout).toContain('windows\\credential-signer.txt')
      expect(pluginInstaller.stdout.indexOf('Get-AuthenticodeSignature')).toBeLessThan(pluginInstaller.stdout.indexOf('install-chatgpt-bundled.mjs'))
      expect(pluginInstaller.stdout).toContain('install-chatgpt-bundled.mjs')
