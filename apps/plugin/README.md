...
当前商家插件源码运行态 `tools/list` 为 141 个 MCP 工具；33 个 `ops.*` 运营工具只保留在独立运营后台，不向商家插件暴露。安装缓存更新后必须重新验证，以运行态清单为准。

当前 0.1.0 的商家侧 Codex MCP 工具共 141 个。
当前源码运行态 `tools/list` 应返回 141 个 MCP 工具，且不得包含任何 `ops.*` 工具。

当前 0.1.0 的商家侧 Codex MCP 工具共 141 个（包含 `merchant.start` 和只读的 `merchant.first_value`），覆盖完整主流程；运营后台工具不在商家插件展示。商家可在 Codex 内按六个平台、店铺、商品、任务和版本查找已批准/已交付内容摘要，以及与版本精确绑定的已归档主图候选摘要。列表不返回正文、图片字节或 URL；选中代表候选后由 `catalog.image.get` 按需读取。生成候选不会覆盖商品当前图片，也不能被称为平台已发布图。OAuth 授权回调仍由服务端 REST/官方页面承载；Merchant Studio 仅用于开发调试，商家不需要打开它。平台统一承担模型中转站费用，商家只需充值/购买额度，不需要提供自己的 Key。小红书/抖音在官方连接器证据齐全前仅显示为 fixture/API 可用，不代表生产可写。

当前源码运行态 `tools/list` 应返回 141 个 MCP 工具；安装缓存更新后必须重新验证，以运行态清单为准。
