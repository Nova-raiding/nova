...
describe('Windows helper source contract', () => {
  ...
  it('requires an external signed installer to bind the full ZIP before extraction', () => {
    ...
    expect(bootstrap.indexOf('$bindingScript prepare')).toBeLessThan(bootstrap.indexOf('& $runtime $installer'))
    expect(bootstrap.indexOf('& $runtime $installer')).toBeLessThan(bootstrap.indexOf('$bindingScript commit'))
    expect(bootstrap).toContain('previous binding remains the last-known-good upgrade receipt')
  })
})
