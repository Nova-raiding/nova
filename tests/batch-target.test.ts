...
describe('batch target identity', () => {
  it('keeps the same product selectable in multiple stores', () => {
    const storeA = { productId: 'p1', platform: 'jd' as const, accountId: 'store-a' }
    const storeB = { productId: 'p1', platform: 'jd' as const, accountId: 'store-b' }
    expect(toggleBatchTarget(toggleBatchTarget([], storeA), storeB)).toEqual([storeA, storeB])
  })

  it('toggles only the exact listing target', () => {
    const first = { productId: 'p1', platform: 'taobao' as const, accountId: 'store-a', listingId: 'listing-1' }
