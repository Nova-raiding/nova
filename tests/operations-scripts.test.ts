...
describe('deployment operation scripts', () => {
  ...
  it('requires an explicit pilot release identity before compose can start', () => {
    ...
    expect(preflight).toContain(': "${OBJECT_STORAGE_EVIDENCE_PATH:?OBJECT_STORAGE_EVIDENCE_PATH is required}"')
    expect(preflight).toContain('tests/object-storage-evidence-gate.ts')
    ...
    expect(preflight).toContain("trust_dir='/run/release-security/evidence-trust'")
    for (const binding of ['--release-git-sha', '--manifest-sha256', '--image-set-digest', '--deployment-nonce', '--expected-config-checksum', '--public-key', '--key-id']) expect(preflight).toContain(binding)
  ...
  it('keeps role bootstrap atomic and verifies runtime ACL/RLS before seeding', () => {
    ...
    expect(compose).toContain('ALERT_RECEIVER_DATABASE_URL: postgres://merchant_alert_receiver:merchant_alert_receiver_local_only@postgres:5432/merchant')
    expect(roleSql).toContain("CREATE ROLE merchant_alert_receiver LOGIN PASSWORD 'merchant_alert_receiver_local_only'")
    expect(roleSql.indexOf('REVOKE ALL ON TABLE public.alert_webhook_receipts')).toBeGreaterThan(roleSql.indexOf('GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO merchant_app'))
  })
  ...
  it('plans ECS capacity capture without network activity and blocks production load', () => {
    ...
    expect(source).toContain('CAPACITY_CAPTURE_EXPECTED_GIT_SHA is required for capture')
    expect(source).toContain("new URL('/releasez', base)")
    expect(source).toContain("redirect: 'manual'")
    expect(source).toContain('release_id does not match the requested release')
  ...
  it('blocks deployment without immutable image and production connection metadata', () => {
    ...
    const imageDigests = {
      ...
      'merchant-alert-receiver': 'sha256:' + 'e'.repeat(64),
    ...
    writeFileSync(hostEvidence, JSON.stringify({
      ...
      scenarios: ['plugin_discovery', 'merchant_start', 'merchant_payment_status', 'manual_publish_workflow', 'asset_attachment', 'error_recovery', 'image_generation', 'automatic_scan', 'candidate_images_rendered', 'candidate_primary_cta', 'candidate_selection_persisted', 'selection_not_reviewed', 'selection_not_published', 'automation_read_only', 'automation_host_absent'].map(id => ({ id, state: 'passed', evidence_ref: `artifact://production/codex-host/${id}#${'a'.repeat(64)}`, console_errors: 0, network_errors: 0, ...(id === 'error_recovery' ? { error_recovery: { trigger_http_status: 503, trigger_error_code: 'MODEL_PROVIDER_OUTCOME_UNKNOWN', request_id: 'request-503', trace_id: 'trace-503', recovery_action: 'query_provider', retry_allowed: false, before_state: 'outcome_unknown', after_state: 'reconciled_failed', reconciliation_required: true, outcome_evidence_ref: `artifact://production/codex-host/error-recovery-outcome#${'a'.repeat(64)}` } } : {}) })),
    }))
    ...
    const scannerRuntime = {
      ...
      PAYMENT_MODE: 'provider', PAYMENT_PROVIDER_ADAPTERS: 'alipay,wechat', PAYMENT_CHECKOUT_BASE_URL: 'https://payments.yxsona.com/checkout', PAYMENT_PROVIDER_CHECKOUT_API_URL: 'https://payments.yxsona.com/v1/checkout', PAYMENT_PROVIDER_QUERY_API_URL: 'https://payments.yxsona.com/v1/query', PAYMENT_PROVIDER_REFUND_QUERY_API_URL: 'https://payments.yxsona.com/v1/refund/query', PAYMENT_PROVIDER_REFUND_API_URL: 'https://payments.yxsona.com/v1/refund', PAYMENT_PROVIDER_MERCHANT_ID: 'merchant-yxsona-2026', PAYMENT_CALLBACK_BASE_URL: 'https://merchant.example.com/v1', PAYMENT_RECONCILIATION_ENABLED: 'true', PAYMENT_REFUND_ENABLED: 'true', COMMERCIAL_PAYMENT_PROVIDER: 'alipay', PLATFORM_RULE_SYNC_MANIFEST_URL: 'https://rules.example.com/platform-rules/v1/manifest.json',
    }
    ...
    // The scan Pod runs as uid 10001, so the image's root-only `sed -i` cannot
    // materialise CLAMD_CONF_*, and the daemon limits have to come from a
    // mounted ConfigMap. See infra/kubernetes/base/clamav-config.yaml.
    const clamdLimitLines = ['StreamMaxLength 100M', 'MaxFileSize 100M', 'MaxScanSize 100M', 'AlertExceedsMax yes']
    const clamdConfBody = ['LocalSocket /tmp/clamd.sock', 'TCPSocket 3310', ...clamdLimitLines, ''].join('\n')
    const clamdConfGuards = clamdLimitLines.map(line => `grep -qx '${line}' /etc/clamav/clamd.conf`).join(' && ')
    ...
    writeFileSync(manifest, JSON.stringify({ apiVersion: 'v1', kind: 'List', items: [
      ...
      { apiVersion: 'apps/v1', kind: 'Deployment', metadata: { name: 'merchant-worker-scan' }, spec: { template: { metadata: { annotations: scannerConfigAnnotation }, spec: { nodeSelector: { 'kubernetes.io/arch': 'amd64' }, volumes: [{ name: 'clamav-config', configMap: { name: 'merchant-clamav-config' } }], containers: [
        ...
        { name: 'clamav', image: `registry.example.com/clamav@${imageDigests.clamav}`, env: [{ name: 'FRESHCLAM_CHECKS', value: '24' }], volumeMounts: [{ name: 'clamav-config', mountPath: '/etc/clamav/clamd.conf', subPath: 'clamd.conf', readOnly: true }], startupProbe: { exec: { command: ['sh', '-c', `clamdscan --ping 1 && ${clamdConfGuards}`] } }, readinessProbe: { exec: { command: ['sh', '-c', `clamdscan --ping 1 && find /var/lib/clamav -mmin -1440 && ${clamdConfGuards}`] } }, livenessProbe: { exec: { command: ['sh', '-c', `clamdscan --ping 1 && ${clamdConfGuards}`] } } },
      ] } } } },
      ...
      { apiVersion: 'v1', kind: 'ConfigMap', metadata: { name: 'merchant-clamav-config' }, data: { 'clamd.conf': clamdConfBody } },
    ] }))
    ...
    expect(() => run(script, [config], { ...base, ALERT_RECEIVER_DATABASE_URL: '' })).toThrow(/ALERT_RECEIVER_DATABASE_URL.*required/)
    expect(() => run(script, [config], { ...base, ALERT_RECEIVER_DATABASE_URL: 'postgresql://alert-db.internal/merchant' })).toThrow(/ALERT_RECEIVER_DATABASE_URL.*TLS/)
    expect(() => run(script, [config], { ...base, ALERT_RECEIVER_DATABASE_URL: 'postgresql://127.0.0.8/merchant?sslmode=verify-full' })).toThrow(/ALERT_RECEIVER_DATABASE_URL.*local/)
  ...
  it('provides one fail-closed launch preflight entrypoint', () => {
    ...
    expect(ecsDeployPreflight).toContain(': "${CAPACITY_PROFILE:?CAPACITY_PROFILE must be explicit: no_load for this release, or an approved load profile}"')
    ...
    expect(ecsDeployPreflight).toContain('--expected-manifest-sha256 "$manifest_sha256"')
    ...
    expect(deployPreflight.match(/codex-app-host-evidence-gate\.ts[^\n]*/g)?.every(line => line.includes('--require-artifacts') && line.includes('--require-production'))).toBe(true)
    expect(ecsDeployPreflight.match(/codex-app-host-evidence-gate\.ts[^\n]*/g)?.every(line => line.includes('--require-artifacts') && !line.includes('--require-production'))).toBe(true)
    expect(deployPreflight).toContain('--expected-mcp-base-url')
    expect(deployPreflight).toContain('--expected-bridge-sha256')
    expect(deployPreflight).toContain('--expected-manifest-sha256 "$manifest_sha256"')
    expect(deployPreflight).toContain('release-manifest-gate.ts')
    expect(deployPreflight).toContain('--expected-candidate-manifest-sha256 "$manifest_sha256"')
    const legacyBundleGate = deployPreflight.indexOf('tests/release-evidence-bundle-gate.ts')
    expect(deployPreflight).toContain('${RELEASE_EVIDENCE_BUNDLE_PATH:?RELEASE_EVIDENCE_BUNDLE_PATH is required}')
    expect(legacyBundleGate).toBeGreaterThan(deployPreflight.indexOf('tests/production-evidence-gate.ts --kind restore'))
    expect(legacyBundleGate).toBeLessThan(deployPreflight.indexOf('echo "deploy preflight passed:'))
    ...
    expect(ecsDeployPreflight).toContain('--candidate-manifest-sha256 "$manifest_sha256"')
  ...
  it('runs the same full migration-chain verification on the k8s path as on ECS', () => {
    // Checking only the tail version proves the workspace and the release agree
    // on a number. A rewritten, reordered or deleted migration keeps the same
    // tail and still passes. The ECS preflight has always also verified the live
    // chain (contiguous numbering plus per-migration name/checksum equality
    // against both the tenant and the Ops schema_migrations history); the
    // Kubernetes preflight used to stop at the tail.
    const kubernetesPreflight = readFileSync('infra/scripts/deploy-preflight.sh', 'utf8')
    const ecsPreflight = readFileSync('infra/scripts/deploy-preflight-ecs.sh', 'utf8')
    for (const preflight of [kubernetesPreflight, ecsPreflight]) {
    ...
    ]) expect(kubernetesPreflight).toContain(requirement)
  })

  it('fails closed for the retired Kubernetes production deploy entrypoint', () => {
    const script = readFileSync('infra/scripts/deploy-verified-manifest.sh', 'utf8')
    expect(script).toContain('deploy-verified-manifest.sh is retired; production deployments must use deploy-verified-ecs-compose.sh')
  ...
  it('isolates owner migration credentials from runtime pods and disables startup migrations', () => {
    ...
    expect(readFileSync('infra/kubernetes/base/ingress.yaml', 'utf8')).toContain('hosts: [yxsona.com, ops.yxsona.com, alerts.yxsona.com]')
    for (const profile of ['pilot-50', 'wave-100', 'wave-250', 'target-500']) {
      ...
      expect(overlay).toContain('digest: SET_ALERT_RECEIVER_IMAGE_DIGEST')
    }
  })
  ...
  it('keeps Merchant Studio browser auth on the HttpOnly session boundary', () => {
    ...
    expect(nginx).toContain('proxy_set_header X-Workspace-Id $http_x_workspace_id;')
    expect(nginx).not.toContain('proxy_set_header X-Workspace-Id "${MERCHANT_WORKSPACE_ID}"')
  ...
  it('keeps the API image build context complete for the TypeScript project references', () => {
    ...
    const localCompose = readFileSync('infra/local/docker-compose.yml', 'utf8')
    ...
    expect(dockerfile).toContain('apk add --no-cache postgresql16-client')
    expect(workerDockerfile).toMatch(/^FROM --platform=\$BUILDPLATFORM node:22-alpine@sha256:[0-9a-f]{64} AS build$/mu)
    expect(workerDockerfile).toMatch(/^FROM node:22-alpine@sha256:[0-9a-f]{64} AS runtime$/mu)
    ...
    expect(workerDockerfile).toContain('npx tsc -p apps/worker/tsconfig.build.json')
    expect(workerDockerfile).not.toContain('npx tsc -p tsconfig.json --noEmitOnError false')
    expect(localCompose.split('x-worker-common: &worker-common')[1]?.split('services:')[0]).toContain('image: local-worker')
  ...
  it('writes database backups through a verified temporary file and atomic rename', () => {
    ...
    expect(script).toContain("checksum=$(sha256sum \"$temporary\" | awk '{print $1}')")
  ...
  it('refuses same-second backup collisions instead of overwriting a verified artifact', () => {
    ...
    expect(execFileSync('sha256sum', ['-c', `${backup}.sha256`], { encoding: 'utf8' })).toContain(`${backup}: OK`)
  ...
  it('rejects rendered Kubernetes images without the release digest', () => {
    ...
    const deployment = (image: string) => ['apiVersion: apps/v1', 'kind: Deployment', 'metadata: {name: api}', 'spec:', '  template:', '    spec:', `      containers: [{name: api, image: ${JSON.stringify(image)}}]`].join('\n')
