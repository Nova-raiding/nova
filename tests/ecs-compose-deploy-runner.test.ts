...
describe('verified ECS Compose deployment runner', () => {
  ...
  it('validates frozen network and volume names against the selected project', () => {
    ...
    writeFileSync(compose, JSON.stringify({
      networks: { default: { name: 'merchant-candidate-test_default' } },
      volumes: { data: { name: 'merchant-candidate-test_data' } },
    }))
    const valid = spawnSync('node', ['infra/scripts/validate-ecs-compose-project.mjs', compose, 'merchant-candidate-test'], { encoding: 'utf8' })
    expect(valid.status, valid.stderr).toBe(0)

    writeFileSync(compose, JSON.stringify({ networks: { default: { name: 'local_default' } } }))
  ...
  it('binds the committed candidate before consuming the nonce or mutating Compose', () => {
    ...
    const preidentityCapture = script.indexOf('set -- capture --state "$state_path"')
  ...
  it('uses a separate signed B code-only branch and never treats a partial switch as ordinary rollback', () => {
    const bridgeScript = source()
    expect(bridgeScript).toContain('ECS_BRIDGE_CODE_ONLY')
    expect(bridgeScript).toContain('bridge code cutover requires candidate migration tail 244')
    expect(bridgeScript).toContain('bridge code cutover requires the reviewed 242/244 schema mode')
    expect(bridgeScript).toContain('B code-only capsule requires old schema 242 and candidate tail 244')
    expect(bridgeScript).toContain('set -- capture --state')
    expect(bridgeScript).toContain('--mode bridge_code_only --candidate-compose')
    expect(bridgeScript).toContain('bridge-begin --state "$state_path"')
    expect(bridgeScript).toContain('bridge-verify --state "$state_path"')
    expect(bridgeScript).toContain('bridge-recover --state "$state_path"')
    expect(bridgeScript).toContain('recovery_succeeded" = false ] && [ "${ECS_BRIDGE_CODE_ONLY:-NO}" != YES')
    const bridgeBegin = bridgeScript.indexOf('bridge-begin --state "$state_path"')
    const mutation = bridgeScript.indexOf('mutation_started=true', bridgeBegin)
    const migration = bridgeScript.indexOf('run --rm --no-deps --pull never migrate', bridgeBegin)
    expect(bridgeBegin).toBeLessThan(mutation)
    expect(mutation).toBeLessThan(migration)
    expect(bridgeScript).toContain('if [ "${ECS_BRIDGE_CODE_ONLY:-NO}" = YES ]; then\n  # B is first proven healthy against DB 242')
  })
