...
describe('verified ECS Compose deployment runner', () => {
  ...
  it('captures an external gateway before nonce consumption and restores it only after business rollback', () => {
    ...
    expect(script.indexOf('sh "$root/infra/scripts/invoke-ecs-automatic-rollback.sh"')).toBeLessThan(script.lastIndexOf('restore_external_gateway ||'))
  ...
  it('uses a separate signed B code-only branch and never treats a partial switch as ordinary rollback', () => {
    ...
    expect(script).toContain('bridge code cutover with an external gateway requires a separately signed topology contract')
    expect(script.indexOf('bridge code cutover with an external gateway requires a separately signed topology contract')).toBeLessThan(script.indexOf('consume-production-evidence-nonce.sh'))
    ...
    expect(script).toContain('bridge-finalize --state "$state_path"')
    const recoverBranch = script.slice(script.indexOf("echo 'B code cutover failed"), script.indexOf('elif [ "$runtime_cutover_started" = false ]'))
    expect(recoverBranch.indexOf('bridge-recover --state')).toBeLessThan(recoverBranch.indexOf('restore_external_gateway || gateway_restored=false'))
    expect(recoverBranch.indexOf('restore_external_gateway || gateway_restored=false')).toBeLessThan(recoverBranch.indexOf('bridge-finalize --state'))
    expect(recoverBranch).toContain('if [ "$gateway_restored" = true ]; then')
