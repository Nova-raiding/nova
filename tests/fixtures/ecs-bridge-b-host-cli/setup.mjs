...
function compose(identity, digest) {
  return { services: { api: { image: `registry.invalid/api@${digest}`, environment: { RELEASE_ID: identity.release_id, RELEASE_GIT_SHA: identity.release_git_sha, RELEASE_MANIFEST_SHA256: identity.manifest_sha256, RELEASE_IMAGE_SET_DIGEST: identity.image_set_digest, RUN_MIGRATIONS_ON_STARTUP: 'false' }, depends_on: {} } } }
}
...
writeFileSync('/state/runtime.json', JSON.stringify({ runtime: 'old', containerId: 'a'.repeat(64) }))
