...
describe('API replica credential rotation dry-run', () => {
  ...
  it('CLI accepts protected files and never emits secret values on success or parse failure', () => {
    ...
    chmodSync(grantsPath, 0o640)
    const groupReadable = spawnSync('node', args, { encoding: 'utf8' })
    expect(groupReadable.status).not.toBe(0)
    expect(groupReadable.stderr).toContain('UNPROTECTED_INPUT')
    expect(groupReadable.stderr).not.toContain(secret)
    chmodSync(grantsPath, 0o700)
    const ownerExecutable = spawnSync('node', args, { encoding: 'utf8' })
    expect(ownerExecutable.status).not.toBe(0)
    expect(ownerExecutable.stderr).toContain('UNPROTECTED_INPUT')
