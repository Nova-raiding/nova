...
describe('OpenAPI security contract', () => {
  ...
  it('documents conditional workspace scope without weakening the production requirement', () => {
    const source = readFileSync(resolve(process.cwd(), 'apps/api/openapi.yaml'), 'utf8')
    const workspaceId = source.slice(source.indexOf('    WorkspaceId:'), source.indexOf('    ActorId:'))
    expect(workspaceId).toContain('required: false')
    expect(workspaceId).toContain('x-conditional-required:')
    expect(workspaceId).toContain('production:')
    expect(workspaceId).toContain('required: true')
    expect(workspaceId).toContain('missing_header_error: WORKSPACE_SCOPE_REQUIRED')
    expect(workspaceId).toContain('local_non_production:')
    expect(workspaceId).toContain('omission_resolution: ws_demo')
    expect(workspaceId).toContain('oidc_bootstrap:')
    expect(workspaceId).toContain('method: workspace.bootstrap')
    expect(workspaceId).toContain('header: X-Workspace-Bootstrap')
    expect(workspaceId).toContain('applies_to_other_methods: false')

    const bootstrap = source.slice(source.indexOf('    WorkspaceBootstrap:'), source.indexOf('    ActorId:'))
    expect(bootstrap).toContain('name: X-Workspace-Bootstrap')
    expect(bootstrap).toContain("enum: ['true']")
