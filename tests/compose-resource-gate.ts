...
assert.equal(scannerEnv.CLAMAV_MAX_FILE_BYTES, '104857600')
...
assert.equal(clamavEnv.CLAMD_CONF_StreamMaxLength, '100M', 'ClamAV INSTREAM must accept the full 100 MiB scanner boundary')
assert.equal(clamavEnv.CLAMD_CONF_MaxFileSize, '100M', 'ClamAV file limit must match the 100 MiB scanner boundary')
assert.equal(clamavEnv.CLAMD_CONF_MaxScanSize, '100M', 'ClamAV scan limit must match the 100 MiB scanner boundary')
assert.equal(clamavEnv.CLAMD_CONF_AlertExceedsMax, 'yes', 'ClamAV must fail closed when expanded archive contents exceed a scan limit')
...
for (const setting of ['StreamMaxLength', 'MaxFileSize', 'MaxScanSize']) {
  assert.ok(clamavSupervisor.includes(setting), `ClamAV supervisor must verify effective ${setting}`)
}
