...
function main() {
  ...
  const pluginDescriptorPath = arg('--plugin-descriptor') ?? process.env.PLUGIN_RELEASE_DESCRIPTOR_PATH
  const pluginTestAttestationPath = arg('--plugin-test-attestation') ?? process.env.PLUGIN_RELEASE_TEST_ATTESTATION_PATH
