...
const cloudRequiredArtifacts = [...requiredArtifacts.filter(path => !path.startsWith('apps/plugin/') && !path.startsWith('.codex-marketplace/')), 'scripts/plugin-release-descriptor.mjs', 'scripts/plugin-release-descriptor.d.mts']
...
export function validateReleaseManifest(document: unknown, options: { root?: string; expectedReleaseId?: string } & EvidenceBindingOptions = {}): string[] {
  ...
  if (value.schemaVersion === 2) {
    if (!options.pluginDescriptorPath || !options.pluginPublicKeyPem || !options.pluginKeyId) errors.push('plugin-release/2 requires a trusted descriptor file, public key and key ID')
    else try {
...
function main() {
  ...
  const pluginDescriptorPath = arg('--plugin-descriptor'); const pluginPublicKeyPath = arg('--plugin-public-key'); const pluginKeyId = arg('--plugin-key-id')
  const errors = validateReleaseManifest(document, { expectedReleaseId: releaseId, artifactRoot, evidenceFiles, publicKeyPem: readFileSync(publicKeyPath, 'utf8'), trustedKeyId,
    pluginDescriptorPath, pluginPublicKeyPem: pluginPublicKeyPath ? readFileSync(pluginPublicKeyPath, 'utf8') : undefined, pluginKeyId })
