...
describe('local relay start contract', () => {
  ...
  it('loads the protected launchctl key and exports all five business model contracts', () => {
    ...
    expect(compose).toContain('MODEL_DAILY_CNY_LIMIT: ${MODEL_DAILY_CNY_LIMIT:-5000.00}')
    expect(compose).toContain('MODEL_MAX_TASK_COST_CNY: ${MODEL_MAX_TASK_COST_CNY:-2000.00}')
    expect(compose).toContain('MODEL_VIDEO_MAX_REQUEST_CNY: ${MODEL_VIDEO_MAX_REQUEST_CNY:-20.00}')
