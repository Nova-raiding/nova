...
describe('verified ECS release staging', () => {
  ...
  it('fails closed on v2 descriptor tampering or absent protected plugin trust before extraction', () => {
    const value = fixture()
    const descriptor = join(value.bundle, 'plugin-release-descriptor-darwin.json')
    const testRecord = join(value.bundle, 'local-plugin-test-attestation-darwin.json')
    const winDescriptor = join(value.bundle, 'plugin-release-descriptor-win32.json')
    const winTest = join(value.bundle, 'local-plugin-test-attestation-win32.json')
    for (const path of [descriptor, winDescriptor]) writeFileSync(path, '{"schema_version":"plugin-release/2"}\n')
    for (const path of [testRecord, winTest]) writeFileSync(path, '{"schema_version":"local-plugin-tests/2"}\n')
    const identity = join(value.bundle, 'candidate-identity.txt')
    const v1 = readFileSync(identity, 'utf8')
    writeFileSync(identity, `${v1}schema_version=candidate-identity/2\nrelease_id=release-1\nplugin_darwin_descriptor_sha256=sha256:${sha(readFileSync(descriptor))}\nplugin_darwin_test_sha256=sha256:${sha(readFileSync(testRecord))}\nplugin_win32_descriptor_sha256=sha256:${sha(readFileSync(winDescriptor))}\nplugin_win32_test_sha256=sha256:${sha(readFileSync(winTest))}\nplugin_key_id=plugin-test-key\n`)
    writeFileSync(descriptor, 'tampered')
    const tampered = run(value)
    expect(tampered.status).not.toBe(0)
    expect(tampered.stderr).toContain('darwin plugin descriptor digest mismatch')
    expect(existsSync(join(value.releases, 'release-1'))).toBe(false)
    writeFileSync(descriptor, '{"schema_version":"plugin-release/2"}\n')
    const noTrust = run(value)
    expect(noTrust.status).not.toBe(0)
    expect(noTrust.stderr).toContain('trusted plugin public key is missing or unsafe')
    expect(existsSync(join(value.releases, 'release-1'))).toBe(false)
  })
