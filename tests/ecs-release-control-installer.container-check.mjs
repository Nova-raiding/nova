...
const bridgeSource = '/source/infra/protected/ecs-preidentity-recovery.mjs';
const bridgeBytes = readFileSync(bridgeSource);
const bridgeArgs = [installer, '--control', 'preidentity', '--source', bridgeSource, '--source-sha256', hash(bridgeBytes), '--node', runtime, '--node-sha256', hash(readFileSync(runtime))];
result = run(bridgeArgs);
assert.equal(result.status, 0, result.stderr);
const bridgeReceipt = JSON.parse(result.stdout);
const bridgeHelper = `${bin}/ecs-preidentity-recovery`;
const bridgeDigest = `${trust}/production-preidentity-recovery-sha256`;
assert.equal(hash(readFileSync(bridgeHelper)), bridgeReceipt.installed_sha256);
assert.equal(readFileSync(bridgeDigest, 'utf8').trim(), bridgeReceipt.installed_sha256);
assert.equal(readFileSync(bridgeHelper, 'utf8').split('\n').slice(1).join('\n'), bridgeBytes.toString('utf8').split('\n').slice(1).join('\n'));
const bridgeVerifier = '/source/infra/scripts/verify-ecs-bridge-control-install.mjs';
const verifyBridge = () => spawnSync(runtime, [bridgeVerifier, bridgeSource, bridgeHelper, bridgeDigest], { encoding: 'utf8', env: { PATH: '/usr/bin:/bin' } });
assert.equal(verifyBridge().status, 0);
// Simulate interruption between executable and trust-digest replacement.
writeFileSync(bridgeDigest, `${'0'.repeat(64)}\n`, { mode: 0o444 });
assert.notEqual(verifyBridge().status, 0);
result = spawnSync(bridgeHelper, ['capture'], { encoding: 'utf8', env: { PATH: '/usr/bin:/bin' } });
assert.notEqual(result.status, 0);
assert.match(result.stderr, /self digest mismatch/u);
result = run(bridgeArgs);
assert.equal(result.status, 0, result.stderr);
assert.equal(readFileSync(bridgeDigest, 'utf8').trim(), hash(readFileSync(bridgeHelper)));
assert.equal(verifyBridge().status, 0);
